...
def build_sports_live_config(env: dict[str, str]) -> dict[str, Any]:
    ...
    return {
        ...
        "order_sizing_mode": str(env.get("JANUS_LIVE_TEST_ORDER_SIZING_MODE", "operator_minimum_order")).strip()
        or "operator_minimum_order",
        "max_buy_notional_usd": env_float(env, "JANUS_LIVE_TEST_MAX_BUY_NOTIONAL_USD", default=10.0),
        "max_order_buy_notional_usd": env_float(
            env,
            "JANUS_LIVE_TEST_MAX_ORDER_BUY_NOTIONAL_USD",
            default=env_float(env, "JANUS_LIVE_TEST_MAX_BUY_NOTIONAL_USD", default=10.0),
        ),
        "event_cap_usd": env_float(
            env,
            "JANUS_LIVE_TEST_EVENT_CAP_USD",
            default=env_float(env, "JANUS_LIVE_TEST_MAX_BUY_NOTIONAL_USD", default=10.0),
        ),
        "side_budget_mode": str(env.get("JANUS_LIVE_TEST_SIDE_BUDGET_MODE", "")).strip() or None,
        "max_same_side_exposure_pct": env_float(
            env,
            "JANUS_LIVE_TEST_MAX_SAME_SIDE_EXPOSURE_PCT",
            default=0.70,
        ),
...
def evaluate_sports_live_activation(
    ...
    *,
    ...
    add("minimum_size_policy", (config.get("min_size") or 0) > 0, config.get("min_size"), blocker="JANUS_LIVE_TEST_MIN_SIZE must be > 0")
    fixed_min_shares = str(config.get("order_sizing_mode") or "").strip().lower() in {
        "fixed_min_shares",
        "fixed_min_size",
        "minimum_shares",
    }
    add(
        ...
        fixed_min_shares or (config.get("min_buy_notional_usd") or 0) >= 1,
        {"order_sizing_mode": config.get("order_sizing_mode"), "min_buy_notional_usd": config.get("min_buy_notional_usd")},
        blocker="JANUS_LIVE_TEST_MIN_BUY_NOTIONAL_USD must be >= 1",
    )
    add(
        "maximum_notional_policy",
        fixed_min_shares
        or (config.get("max_order_buy_notional_usd") or config.get("max_buy_notional_usd") or 0)
        >= (config.get("min_buy_notional_usd") or 0),
        config.get("max_order_buy_notional_usd") or config.get("max_buy_notional_usd"),
        blocker="JANUS_LIVE_TEST_MAX_ORDER_BUY_NOTIONAL_USD must be >= JANUS_LIVE_TEST_MIN_BUY_NOTIONAL_USD",
    )
    add(
        "event_cap_policy",
        (config.get("event_cap_usd") or 0)
        >= ((config.get("min_size") or 0) if fixed_min_shares else (config.get("max_order_buy_notional_usd") or config.get("max_buy_notional_usd") or 0)),
        {
            "event_cap_usd": config.get("event_cap_usd"),
            "min_size": config.get("min_size"),
            "max_order_buy_notional_usd": config.get("max_order_buy_notional_usd"),
        },
        blocker="JANUS_LIVE_TEST_EVENT_CAP_USD must cover the configured per-order policy",
    )
    if not fixed_min_shares:
        add(
            "event_cap_legacy_notional_policy",
            (config.get("event_cap_usd") or 0)
        >= (config.get("max_order_buy_notional_usd") or config.get("max_buy_notional_usd") or 0),
            {
                "event_cap_usd": config.get("event_cap_usd"),
                "max_order_buy_notional_usd": config.get("max_order_buy_notional_usd"),
            },
            blocker="JANUS_LIVE_TEST_EVENT_CAP_USD must be >= per-order buy cap",
        )
    add(
...
def build_sports_live_next_commands(config: dict[str, Any]) -> dict[str, str]:
    ...
    base.extend(
        [
            ...
            "--order-sizing-mode",
            str(config.get("order_sizing_mode") or "operator_minimum_order"),
            "--max-buy-notional-usd",
            str(config.get("max_order_buy_notional_usd") or config.get("max_buy_notional_usd") or 10),
            "--event-cap-usd",
            str(config.get("event_cap_usd") or config.get("max_buy_notional_usd") or 10),
        ]
    )
    if config.get("side_budget_mode"):
        base.extend(["--side-budget-mode", str(config.get("side_budget_mode"))])
    if config.get("max_same_side_exposure_pct") is not None:
        base.extend(["--max-same-side-exposure-pct", str(config.get("max_same_side_exposure_pct"))])
    rehearsal = list(base)
...
def _build_worker_tick_command(config: dict[str, Any], *, execute: bool) -> str:
    ...
    if config.get("side_budget_mode"):
        parts.extend(["--side-budget-mode", str(config.get("side_budget_mode"))])
    if config.get("max_same_side_exposure_pct") is not None:
        parts.extend(["--max-same-side-exposure-pct", str(config.get("max_same_side_exposure_pct"))])
    if execute:
