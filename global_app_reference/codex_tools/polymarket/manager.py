...
_HIGH_VOLUME_USD = Decimal("100000")
_LOW_PRICE_CATALOG_PRICE_CEILING = Decimal("0.40")
...
def _catalog_score(
    ...
    *,
    ...
    if category in _CATALYST_CATEGORIES:
        score += 2
    score += _catalog_low_price_score(price)
    score += _catalog_cashout_score(row)
...
def _catalog_low_price_score(price: Decimal | None) -> int:
    if price is None or price <= 0:
        return 0
    if Decimal("0.02") <= price < Decimal("0.05"):
        return 5
    if Decimal("0.05") <= price < Decimal("0.15"):
        return 8
    if Decimal("0.15") <= price <= _LOW_PRICE_CATALOG_PRICE_CEILING:
        return 5
...
def _catalog_cashout_score(row: dict[str, Any]) -> int:
    ...
    liquidity = _first_decimal(row, ("liquidity_usd", "liquidity", "depth_usd", "cashout_depth_usd"))
    spread = _first_decimal(row, ("spread_cents", "spread"))
    score = 0
    ...
    if liquidity is not None:
        if liquidity >= Decimal("250"):
            score += 5
        elif liquidity >= Decimal("10"):
            score += 2
