...
def _train_numpy_logistic(
    ...
    *,
    ...
    weights = rng.normal(0.0, 0.01, size=x_train.shape[1])
    bias = 0.0
    positives = float(np.sum(y_train))
    negatives = float(len(y_train) - positives)
    pos_weight = min(max(negatives / positives, 1.0), 8.0) if positives > 0 else 1.0
    best_weights = weights.copy()
    best_bias = bias
    best_loss = float("inf")
    history: list[dict[str, Any]] = []
    patience = max(12, min(40, int(epochs) // 4))
    stale_epochs = 0
    for epoch in range(int(epochs)):
        logits = x_train @ weights + bias
        probs = 1.0 / (1.0 + np.exp(-np.clip(logits, -30.0, 30.0)))
        row_weights = np.where(y_train > 0.5, pos_weight, 1.0)
        error = (probs - y_train) * row_weights
        denom = max(1.0, float(np.sum(row_weights)))
        grad_w = (x_train.T @ error) / denom + float(weight_decay) * weights
        grad_b = float(np.sum(error) / denom)
        weights = weights - float(learning_rate) * grad_w
        bias = bias - float(learning_rate) * grad_b
        train_loss = _weighted_binary_loss(y_train, probs, row_weights)
        calibration_probs = _predict_numpy_logistic({"weights": weights, "bias": bias}, x_calibration)
        calibration_loss = _weighted_binary_loss(y_calibration, calibration_probs, np.ones_like(y_calibration))
        history.append(
            {
                "epoch": epoch + 1,
...
def _weighted_binary_loss(y_true: np.ndarray, probs: np.ndarray, row_weights: np.ndarray) -> float:
    clipped = np.clip(probs, 1e-6, 1.0 - 1e-6)
    losses = -(y_true * np.log(clipped) + (1.0 - y_true) * np.log(1.0 - clipped))
    return float(np.average(losses, weights=row_weights)) if len(losses) else 0.0


def _predict_numpy_logistic(model: dict[str, Any], x_values: np.ndarray) -> np.ndarray:
...
def _predict_torch_mlp(model: Any, x_values: np.ndarray) -> np.ndarray:
    if isinstance(model, dict) and model.get("backend") == "numpy_logistic":
        return _predict_numpy_logistic(model, x_values)
    import torch
