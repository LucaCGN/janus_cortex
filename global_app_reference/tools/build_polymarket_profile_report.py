...
PROFILES = [
    ...
    {
        "handle": "ScottyNooo",
        "url": "https://polymarket.com/@ScottyNooo",
        "wallet": "0xbacd00c9080a82ded56f504ee8810af732b0ab35",
    },
    {
        "handle": "ImJustKen",
        "url": "https://polymarket.com/@ImJustKen",
...
def compact_timestamp(ts: int) -> str:
    if not ts:
        return ""
    return datetime.fromtimestamp(ts, UTC).strftime("%Y-%m-%d %H:%M UTC")


def latest_position_rows(positions: list[dict[str, Any]], *, limit: int = 5) -> list[dict[str, Any]]:
    def sort_key(row: dict[str, Any]) -> tuple[float, float, float]:
        current = safe_float(first_present(row, ("currentValue", "visible_position_value", "value")))
        pnl_abs = abs(safe_float(first_present(row, ("cashPnl", "visible_pnl", "pnl"))))
        size = safe_float(row.get("size"))
        return current, pnl_abs, size

    rows: list[dict[str, Any]] = []
    for row in sorted(positions, key=sort_key, reverse=True)[:limit]:
        slug = str(first_present(row, ("eventSlug", "marketSlug", "market_slug", "slug")) or "")
        title = clean_text(first_present(row, ("title", "marketTitle", "question")) or slug or "Unknown market")
        meta = infer_event_meta(slug, title, {}, "heuristic")
        rows.append(
            {
                "title": title,
                "market_slug": slug,
                "market_domain": category_path(meta),
                "outcome": clean_text(first_present(row, ("outcome", "side", "outcomeName")) or ""),
                "size": safe_float(row.get("size")),
                "avg_price": safe_float(first_present(row, ("avgPrice", "price", "averagePrice"))),
                "current_price": safe_float(first_present(row, ("curPrice", "currentPrice", "current_price", "price"))),
                "current_value": safe_float(first_present(row, ("currentValue", "visible_position_value", "value"))),
                "cash_pnl": safe_float(first_present(row, ("cashPnl", "visible_pnl", "pnl"))),
                "token_id": str(first_present(row, ("asset", "tokenId", "token_id")) or ""),
                "source_confidence": ["profile_observed", "api_limited", "unresolved_open_pnl"],
            }
        )
    return rows


def latest_trade_rows(trades: list[dict[str, Any]], *, limit: int = 5) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for row in sorted(trades, key=parse_timestamp, reverse=True)[:limit]:
        slug = str(first_present(row, ("eventSlug", "marketSlug", "market_slug", "slug")) or "")
        title = clean_text(first_present(row, ("title", "marketTitle", "question")) or slug or "Unknown market")
        ...
        rows.append(
            {
                ...
                "market_domain": category_path(meta),
                "outcome": clean_text(first_present(row, ("outcome", "side", "outcomeName")) or ""),
                "trade_side": clean_text(str(row.get("side") or "")),
                "size": safe_float(row.get("size")),
                "price": safe_float(row.get("price")),
                "notional": trade_cash(row),
                "timestamp": ts,
                "observed_time_utc": compact_timestamp(ts),
                "token_id": str(first_present(row, ("asset", "tokenId", "token_id")) or ""),
                "source_confidence": ["profile_observed", "api_limited"],
            }
        )
    return rows


def source_caveats(summary: dict[str, Any]) -> list[str]:
    caveats = [
        "public_profile_data_not_strategy_proof",
        "missing_full_trade_sequence",
        "fillability_and_queue_position_unknown",
    ]
    if summary["closed_capped"]:
        caveats.append("closed_positions_api_capped")
    if summary["trades_capped"]:
        caveats.append("trade_history_api_capped")
    if summary["open"]["positions"]:
        caveats.append("unresolved_open_pnl")
    if summary["open"]["capped"]:
        caveats.append("open_positions_api_capped")
    return caveats


def render_latest_observations(summary: dict[str, Any]) -> str:
    observation = summary["latest_observations"]
    position_rows = [
        [
            row["title"][:72] + ("..." if len(row["title"]) > 72 else ""),
            row["market_domain"],
            row["outcome"],
            number(row["size"], 2),
            f"{pct(row['current_price'] * 100, 1)} price",
            money(row["current_value"]),
            money(row["cash_pnl"]),
        ]
        for row in observation["active_positions"]
    ]
    trade_rows = [
        [
            row["observed_time_utc"] or "n/a",
            row["title"][:62] + ("..." if len(row["title"]) > 62 else ""),
            row["market_domain"],
            row["trade_side"],
            row["outcome"],
            number(row["size"], 2),
            f"{pct(row['price'] * 100, 1)} price",
            money(row["notional"]),
        ]
        for row in observation["recent_trades"]
    ]
    return "\n".join(
        [
            "### Latest Observable Moves",
            markdown_table(
                ["Open market", "Domain", "Outcome", "Size", "Current", "Value", "Open P/L"],
                position_rows,
            ),
            "",
            markdown_table(
                ["Observed time", "Recent trade", "Domain", "Trade", "Outcome", "Size", "Price", "Notional"],
                trade_rows,
            ),
            "",
            "Source confidence: "
            + ", ".join(observation["source_confidence"])
            + ". Caveats: "
            + ", ".join(observation["source_caveats"])
            + ".",
        ]
    )


def performance_pattern(summary: dict[str, Any]) -> str:
    win_rate = summary["closed_stats"].get("win_rate")
    period_map = {row["period"]: row for row in summary["period_rows"]}
    all_pnl = safe_float(period_map.get("ALL", {}).get("pnl"))
    if win_rate is not None and win_rate >= 90:
        return "high_win_rate"
    if win_rate is not None and 45 <= win_rate <= 55 and all_pnl > 25000:
        return "near_50_win_rate_high_return"
    if all_pnl > 100000:
        return "moderate_win_rate_high_return"
    return "low_sample_or_conservative_return"


def profile_ref(summary: dict[str, Any]) -> dict[str, Any]:
    profile = summary["profile"]
    period_map = {row["period"]: row for row in summary["period_rows"]}
    return {
        "handle": profile["handle"],
        "url": profile["input_url"],
        "wallet": profile["wallet"],
        "specialty": summary["specialty"],
        "performance_pattern": performance_pattern(summary),
        "win_rate": summary["closed_stats"].get("win_rate"),
        "all_time_pnl": period_map.get("ALL", {}).get("pnl"),
        "position_value": summary["position_value"],
        "source_caveats": source_caveats(summary),
    }


def by_handle(summaries: list[dict[str, Any]]) -> dict[str, dict[str, Any]]:
    return {summary["profile"]["handle"].lower(): summary for summary in summaries}


def collect_refs(summaries_by_handle: dict[str, dict[str, Any]], handles: list[str]) -> list[dict[str, Any]]:
    refs: list[dict[str, Any]] = []
    for handle in handles:
        summary = summaries_by_handle.get(handle.lower())
        if summary:
            refs.append(profile_ref(summary))
    return refs


def derive_benchmark_hypotheses(summaries: list[dict[str, Any]], now: datetime) -> dict[str, Any]:
    summaries_by_handle = by_handle(summaries)
    common_promotion_path = ["idea", "research", "shadow", "min-size test"]
    common_status = "not_live_authorized"
    common_requirements = [
        "fresh profile observation",
        "direct CLOB token and orderbook proof",
        "independent market thesis",
        "historical sample or replay substrate",
        "fillability and slippage model",
        "concentration and risk-cap review",
    ]
    hypotheses = [
        {
            "id": "PROFILE-HYP-GEOPOL-CATALYST-REPRICING",
            "title": "Geopolitics catalyst-cluster repricing",
            "profile_sources": collect_refs(summaries_by_handle, ["aenews2", "classified", "car", "ScottyNooo"]),
            "market_domain": ["geopolitics", "politics", "elections", "economics"],
            "setup_pattern": "High-performing generalist profiles repeatedly appear around related political and geopolitical event clusters; benchmark whether fresh catalyst news plus low-slippage books produces repeatable repricing windows.",
            "cadence_time_horizon": "event-driven; hours to weeks, with explicit deadline reviews",
            "win_rate_return_pattern": ["moderate_win_rate_high_return", "clustered_thesis_exposure"],
            "data_requirements": common_requirements
            + ["both-side top-holder conflict scan", "official-source timeline for the catalyst"],
            "backtest_requirements": [
                "event-cluster sample with timestamped news/catalysts",
                "orderbook spread/depth at candidate time",
                "entry, target, stop, and rebid policy replay",
            ],
            "fillability_liquidity_caveats": "Profile sizes are not transferable; Janus must assume only micro-size fillability until orderbook depth and queue position are modeled.",
            "expected_failure_modes": [
                "copying stale active positions",
                "both sides supported by credible high-PnL holders",
                "correlated Iran/geopolitics concentration",
                "headline risk invalidates thesis before exit",
            ],
            "validation_plan": "Research artifact first, then shadow-score candidate rows for at least one week before any min-size test.",
            "promotion_path": common_promotion_path,
            "status": common_status,
            "backlog_route": "planned research item under #46/#48 only after source and sample requirements are explicit",
        },
        {
            "id": "PROFILE-HYP-GLOBAL-LADDERED-PORTFOLIO",
            "title": "Laddered global portfolio thesis management",
            "profile_sources": collect_refs(summaries_by_handle, ["predictfolio", "car", "aenews2"]),
            "market_domain": ["global-portfolio", "economics", "politics", "culture"],
            "setup_pattern": "Profiles with broad or conservative exposure suggest benchmarking slow thesis positions with target, rebuy, and opportunity-cost rules rather than simple hold-to-resolution.",
            "cadence_time_horizon": "days to months; reviewed on scheduled portfolio passes",
            "win_rate_return_pattern": ["high_win_rate_low_turnover", "moderate_win_rate_high_return"],
            "data_requirements": common_requirements
            + ["position lifecycle snapshots", "target/rebuy history", "category concentration history"],
            "backtest_requirements": [
                "simulate target and lower-rebid ladders on historical market price paths",
                "compare passive hold, profit-capture, and stop/rebid variants",
            ],
            "fillability_liquidity_caveats": "Long-horizon books can look liquid at top of book but fail on exits; benchmark must include exit depth and stale-target behavior.",
            "expected_failure_modes": [
                "capital trapped in low-velocity positions",
                "sunk-cost holding after thesis failure",
                "targets too far from current mark to be actionable",
            ],
            "validation_plan": "Shadow every current global slot with hold, target-tighten, cut/rebid, and no-action alternatives before promotion.",
            "promotion_path": common_promotion_path,
            "status": common_status,
            "backlog_route": "planned research item under #46 and portfolio-manager follow-up only after lifecycle data exists",
        },
        {
            "id": "PROFILE-HYP-TECH-TOP-HOLDER-CONFLICT",
            "title": "Tech and AI top-holder conflict screen",
            "profile_sources": collect_refs(summaries_by_handle, ["ImJustKen"]),
            "market_domain": ["tech", "ai", "corporate-events"],
            "setup_pattern": "A high-PnL verified holder can improve candidate discovery, but benchmark value comes from comparing credible Yes and No holder conflict before any side is selected.",
            "cadence_time_horizon": "catalyst-driven; daily to event outcome",
            "win_rate_return_pattern": ["high_pnl_holder_context", "conflict_screen_required"],
            "data_requirements": common_requirements
            + ["Yes and No holder lists", "profile-side conflict score", "fresh catalyst calendar"],
            "backtest_requirements": [
                "historical top-holder snapshots around tech/AI catalysts",
                "outcome and mark-to-market comparison when high-PnL holders disagree",
            ],
            "fillability_liquidity_caveats": "Top-holder visibility is not trade timing; public holder data can be stale and does not expose resting exits.",
            "expected_failure_modes": [
                "treating a famous holder as proof",
                "adding duplicate OpenAI/AI concentration",
                "ignoring stronger opposing-holder evidence",
            ],
            "validation_plan": "Keep as watch-only until at least five conflicted tech/AI candidate cases are scored with direct book evidence.",
            "promotion_path": common_promotion_path,
            "status": common_status,
            "backlog_route": "planned benchmark row under #46; no execution authority",
        },
        {
            "id": "PROFILE-HYP-CRYPTO-UPDOWN-MICROSTRUCTURE",
            "title": "Crypto up/down microstructure timing",
            "profile_sources": collect_refs(
                summaries_by_handle,
                ["0xb55fa1296e6ec55d0ce53d93b9237389f11764d4-1777575277609", "pbot-6", "baloneigh"],
            ),
            "market_domain": ["crypto", "short-horizon-binary"],
            "setup_pattern": "Near-50% win-rate crypto profiles can still show high return, implying the benchmark should test entry timing, payoff asymmetry, and interval selection rather than raw directional accuracy.",
            "cadence_time_horizon": "5-minute to 4-hour markets; high-frequency",
            "win_rate_return_pattern": ["near_50_win_rate_high_return", "high_throughput"],
            "data_requirements": common_requirements
            + ["tick-level crypto reference prices", "per-interval orderbook snapshots", "fees and settlement timing"],
            "backtest_requirements": [
                "interval-by-interval replay with queue and slippage stress",
                "separate 5m, 15m, hourly, and 4h cohorts",
                "capital-turnover and loss-tail accounting",
            ],
            "fillability_liquidity_caveats": "This edge can disappear under latency, fees, and queue priority; public profile rows do not prove executable fills.",
            "expected_failure_modes": [
                "overfitting visible winning intervals",
                "ignoring maker/taker and queue position",
                "assuming settlement marks are tradable prices",
            ],
            "validation_plan": "Route to #47 as research/backtest only after a replay substrate exists.",
            "promotion_path": common_promotion_path,
            "status": common_status,
            "backlog_route": "planned future-domain research under #47; #46 supplies benchmark hypothesis shape",
        },
        {
            "id": "PROFILE-HYP-HIGH-WIN-TAIL-RISK",
            "title": "High-win-rate tail-risk and accounting audit",
            "profile_sources": collect_refs(summaries_by_handle, ["mikeaddon", "wuhuuuuuuli"]),
            "market_domain": ["crypto", "risk-accounting"],
            "setup_pattern": "Extremely high reported win rates may represent small-edge harvesting, but the benchmark must first prove that unresolved open PnL, settlement accounting, and rare losses are understood.",
            "cadence_time_horizon": "high-frequency or near-settlement; minutes to hours",
            "win_rate_return_pattern": ["high_win_rate", "tail_loss_sensitive"],
            "data_requirements": common_requirements
            + ["biggest-loss audit", "open-PnL reconciliation", "settlement-state classification"],
            "backtest_requirements": [
                "loss-tail distribution by interval and market type",
                "stress test for rare but large adverse outcomes",
                "capital efficiency after fees and unresolved exposure",
            ],
            "fillability_liquidity_caveats": "High win rate is not enough; benchmark must show losses are bounded and exits are actually fillable.",
            "expected_failure_modes": [
                "mistaking accounting artifacts for edge",
                "rare loss overwhelms many small wins",
                "unresolved positions hide active risk",
            ],
            "validation_plan": "Audit data semantics before any strategy score is allowed into shadow mode.",
            "promotion_path": common_promotion_path,
            "status": common_status,
            "backlog_route": "planned future-domain risk research under #47",
        },
    ]
    return {
        "schema_version": "profile_benchmark_hypotheses_v1",
        "generated_at_utc": now.isoformat(),
        "issue": "https://github.com/LucaCGN/janus_cortex/issues/46",
        "source_profile_report_schema": "polymarket_profile_rentability_report_json_v1",
        "source_caveats": [
            "public profile data is sampled and can hit API caps",
            "public rows do not expose complete trade sequencing, resting orders, or queue position",
            "unresolved open PnL is not proof of realized edge",
            "profile behavior is benchmark inspiration only and creates no live trading authority",
        ],
        "hypotheses": hypotheses,
    }


def render_hypotheses_report(payload: dict[str, Any]) -> str:
    rows = []
    for item in payload["hypotheses"]:
        rows.append(
            [
                item["id"],
                ", ".join(item["market_domain"]),
                item["cadence_time_horizon"],
                ", ".join(item["win_rate_return_pattern"]),
                item["status"],
            ]
        )
    body = [
        "# Profile Benchmark Hypotheses",
        "",
        f"Generated: {payload['generated_at_utc']}",
        f"Issue: {payload['issue']}",
        "",
        "These hypotheses convert public profile observations into research benchmarks only. They do not authorize live trading, copy-trading, runtime setting changes, or Janus NBA/WNBA StrategyPlan mutation.",
        "",
        "## Shared Caveats",
        "",
        *[f"- {caveat}" for caveat in payload["source_caveats"]],
        "",
        "## Summary",
        markdown_table(["Hypothesis", "Domain", "Cadence", "Pattern tags", "Status"], rows),
        "",
        "## Details",
    ]
    for item in payload["hypotheses"]:
        profiles = ", ".join(f"@{ref['handle']}" for ref in item["profile_sources"]) or "n/a"
        body.extend(
            [
                "",
                f"### {item['id']} - {item['title']}",
                "",
                f"- Profile/source reference: {profiles}",
                f"- Market domain: {', '.join(item['market_domain'])}",
                f"- Setup pattern: {item['setup_pattern']}",
                f"- Cadence/time horizon: {item['cadence_time_horizon']}",
                f"- Data requirements: {'; '.join(item['data_requirements'])}",
                f"- Backtest requirements: {'; '.join(item['backtest_requirements'])}",
                f"- Fillability/liquidity caveats: {item['fillability_liquidity_caveats']}",
                f"- Expected failure modes: {'; '.join(item['expected_failure_modes'])}",
                f"- Validation plan: {item['validation_plan']}",
                f"- Promotion path: {' -> '.join(item['promotion_path'])}",
                f"- Status: {item['status']}",
                f"- Backlog route: {item['backlog_route']}",
            ]
        )
    return "\n".join(body).rstrip() + "\n"
...
def build_profile_report(
    ...
    now: datetime,
    *,
    max_categorized_events: int = 800,
) -> dict[str, Any]:
    ...
    metas, categorized_events, total_events = fetch_event_metadata(client, event_rows, max_events=max_categorized_events)
    ...
    latest_observations = {
        "active_positions": latest_position_rows(positions),
        "recent_trades": latest_trade_rows(trades),
        "source_confidence": list(
            dict.fromkeys(
                ["profile_observed", "api_limited"]
                + (["unresolved_open_pnl"] if positions else [])
                + (["trade_history_capped"] if trades_capped else [])
                + (["closed_positions_capped"] if closed_capped else [])
            )
        ),
        "source_caveats": [],
    }
    ...
    latest_observations["source_caveats"] = source_caveats(summary)
    return summary
...
def render_profile_section(summary: dict[str, Any]) -> str:
    ...
    lines.extend(["", render_latest_observations(summary)])
...
def main() -> None:
    ...
    parser.add_argument(
        "--max-categorized-events",
        type=int,
        default=800,
        help="Maximum closed event groups per profile to enrich with Gamma metadata.",
    )
    ...
    summaries = [
        build_profile_report(client, item, now, max_categorized_events=args.max_categorized_events)
        for item in PROFILES
    ]
    report = render_report(summaries, now)
    hypotheses = derive_benchmark_hypotheses(summaries, now)
    hypotheses_report = render_hypotheses_report(hypotheses)
    ...
    hypothesis_md_path = output_dir / f"profile_benchmark_hypotheses_{stamp}.md"
    hypothesis_json_path = output_dir / f"profile_benchmark_hypotheses_{stamp}.json"
    md_path.write_text(report, encoding="utf-8")
    json_path.write_text(json.dumps(summaries, indent=2, ensure_ascii=True), encoding="utf-8")
    hypothesis_md_path.write_text(hypotheses_report, encoding="utf-8")
    hypothesis_json_path.write_text(json.dumps(hypotheses, indent=2, ensure_ascii=True), encoding="utf-8")
    print(f"Wrote {md_path}")
    print(f"Wrote {json_path}")
    print(f"Wrote {hypothesis_md_path}")
    print(f"Wrote {hypothesis_json_path}")
