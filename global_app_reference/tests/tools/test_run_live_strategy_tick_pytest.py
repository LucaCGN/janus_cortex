...
def test_resolve_wnba_game_matches_la_sparks_official_las_tricode_pytest(monkeypatch) -> None:
    from app.data.nodes.wnba.live import live_stats
            ...
            [
                {
                    "game_id": "1022600061",
                    "game_status": 2,
                    "game_status_text": "Q2 2:00",
                    "period": 2,
                    "game_clock": "PT02M00.00S",
                    "game_date": "2026-05-30",
                    "home_team_tricode": "CON",
                    "away_team_tricode": "LAS",
                    "home_score": 31,
                    "away_score": 33,
                }
            ]
    ...
    assert result is not None
    assert result["resolved"] is True
    assert result["game_id"] == "1022600061"
    assert result["resolution_source"] == "wnba_cdn_scoreboard_event_slug"
...
def test_resolve_nba_game_uses_adjacent_utc_date_and_reports_diagnostics_pytest(monkeypatch) -> None:
    def fake_api_json(api_root: str, method: str, path: str, payload: dict[str, Any] | None = None, **kwargs):
        if path == "/v1/nba/games":
            return {
                "items": [
    ...
    result = live_tick._resolve_game("http://test", "nba-cle-nyk-2026-05-31", "2026-05-31")

    assert result["resolved"] is True
    assert result["game_id"] == "0042500314"
    resolution = result["scoreboard_resolution"]
    assert resolution["schema_version"] == "scoreboard_resolution_diagnostics_v1"
    assert resolution["status"] == "resolved"
    assert resolution["selected_game_id"] == "0042500314"
    assert "2026-06-01" in resolution["date_windows_searched"]
    assert resolution["candidate_games_considered"][0]["team_pair_match"] is True
    assert resolution["execution_safely_blocked"] is False


def test_resolve_nba_game_reports_safe_blocker_and_candidates_when_missing_pytest(monkeypatch) -> None:
    def fake_api_json(api_root: str, method: str, path: str, payload: dict[str, Any] | None = None, **kwargs):
        if path == "/v1/nba/games":
            return {
                "items": [
    ...
    assert resolution["feed_blocker_reason"] == "game_not_found"
    assert resolution["execution_safely_blocked"] is True
    assert resolution["total_games_considered"] == 1
    assert resolution["candidate_games_considered"][0]["date_match"] is True
    assert resolution["candidate_games_considered"][0]["team_pair_match"] is False
...
def test_event_tick_does_not_replace_current_strategy_plan_with_candidate_without_review_pytest(monkeypatch) -> None:
    calls: list[dict[str, Any]] = []
    plan = {
        "market_id": "market-1",
        "active_strategies": [
    ...
    plan_submit_calls = [call for call in calls if call["path"] == "/v1/events/nba-sas-min-2026-05-10/strategy-plan"]
    assert plan_submit_calls == []
    assert evaluate_index >= 0
    candidate = StrategyPlan.model_validate(result["operator_reaction"]["candidate_strategy_plan"])
    assert candidate.active_strategies[0].family == "operator_position_management"
    assert candidate.active_strategies[0].entry_rules["entry_disabled"] is True
    assert result["operator_reaction"]["candidate_strategy_plan_submission"] == {
        "enabled": True,
        "submitted": False,
        "reason": "current_strategy_plan_replacement_requires_explicit_review",
        "current_plan_strategy_count": 1,
    }
...
def test_event_tick_classifies_manual_open_order_before_direct_mirror_pytest(monkeypatch) -> None:
    calls: list[dict[str, Any]] = []
    portfolio_order_get_count = 0
    plan = {
        "market_id": "market-1",
        "generated_at_utc": "2026-05-30T22:00:00Z",
        "active_strategies": [
            {
                "strategy_id": "okc-micro-grid",
                "family": "nba_evening_micro_grid",
                "side": "Thunder",
                "entry_rules": {
                    "market_id": "market-1",
                    "outcome_id": "outcome-okc",
    ...
    def fake_api_json(api_root: str, method: str, path: str, payload: dict[str, Any] | None = None, **kwargs):
        nonlocal portfolio_order_get_count
        calls.append({"method": method, "path": path, "payload": payload, "kwargs": kwargs})
        if path == "/v1/events/nba-sas-okc-2026-05-30/agent-context":
            return {
                "ok": True,
        ...
        if path == "/v1/nba/games":
            return {"ok": True, "items": []}
        if path == "/v1/outcomes/by-token/token-okc":
            return {
                "ok": True,
                "market_id": "market-1",
                "outcome_id": "outcome-okc",
        ...
        if path == "/v1/sync/polymarket/orderbook":
            return {"ok": True}
        if path == "/v1/outcomes/outcome-okc/orderbook/latest":
            return {
                "ok": True,
                "snapshot": {
                    "best_bid": 0.41,
                    "best_ask": 0.43,
                    "captured_at": "2026-05-31T00:25:00+00:00",
                },
            }
        if path == "/v1/portfolio/orders":
            portfolio_order_get_count += 1
            items = []
            if portfolio_order_get_count > 1:
                items = [
                    {
                        "external_order_id": "0xmanual-sell",
                        "event_slug": "nba-sas-okc-2026-05-30",
                        "market_id": "market-1",
                        "outcome_id": "outcome-okc",
                        "side": "sell",
                        "status": "live",
        ...
        if path == "/v1/portfolio/orders/direct-open-mirror":
            return {
                ...
                "applied": [{"external_order_id": "0xmanual-sell"}],
            }
        ...
        if path == "/v1/events/nba-sas-okc-2026-05-30/strategy-plan/evaluate":
    ...
    reaction = result["operator_reaction"]
    assert reaction["pre_mirror_known_order_lookup"]["external_order_ids"] == []
    assert reaction["known_order_lookup"]["external_order_ids"] == ["0xmanual-sell"]
    assert reaction["order_reactions"][0]["action"] == "adopt_operator_open_order"
    assert reaction["order_reactions"][0]["direct_order_id"] == "0xmanual-sell"
    assert reaction["manual_interference_rebase"]["status"] == "required"
    assert reaction["manual_interference_rebase"]["requested_model_tier"] == "mini"
    assert reaction["manual_interference_rebase"]["open_order_reaction_count"] == 1
    assert result["market_state"]["llm_runtime_triggers"][0]["trigger_type"] == "manual_operator_order"
    assert result["market_state"]["llm_runtime_triggers"][0]["selected_model"] == "gpt-5.4-mini"
    assert result["llm_runtime_trace"]["model_routing"]["selected_tier"] == "mini"
    assert (
        "frontier_downgraded_operator_minimum_order_policy"
        in result["llm_runtime_trace"]["model_routing"]["critical_reasons"]
    )
...
def test_event_scoped_direct_clob_includes_sibling_outcome_inventory_pytest() -> None:
    scoped = live_tick._event_scoped_direct_clob(
                ...
                "trades": [
                    {
                        ...
                        "price": 0.76,
                        "size": 5,
                        "timestamp": int(datetime(2026, 5, 18, 1, 0, tzinfo=timezone.utc).timestamp()),
                    }
                ]
            },
        },
        {
            "context_summary": {
                "event_slug": "nba-sas-okc-2026-05-18",
                "game_start_utc": "2026-05-18T00:00:00Z",
            },
...
def test_event_scoped_direct_clob_quarantines_zero_timestamp_trades_pytest() -> None:
    scoped = live_tick._event_scoped_direct_clob(
        {
            "current_token_trades": {
                "trades": [
    ...
    assert scoped["current_token_trade_count"] == 1
    assert scoped["current_token_trades"]["trusted_trade_count"] == 1
    assert scoped["current_token_trades"]["trades"][0]["id"] == "live-fill-row"
    assert scoped["untrusted_current_token_trade_count"] == 1
    assert scoped["current_token_trades"]["untrusted_trade_count"] == 1
    untrusted = scoped["current_token_trades"]["untrusted_trades"][0]
    assert untrusted["id"] == "oversized-history-row"
    assert untrusted["current_account_evidence_status"] == "untrusted"
    assert untrusted["current_account_evidence_reason"] == "direct_trade_timestamp_missing_or_invalid"
...
def test_live_signal_aggregation_budget_uses_dynamic_risk_state_pytest() -> None:
    ...
    assert budget["realized_event_pnl_usd"] == 0.0
    assert budget["unresolved_loss_exposure_usd"] == 1.0
    assert budget["profit_ratcheted_addon_usd"] == 0.0
    assert budget["loss_cut_usd"] == 2.5
    assert budget["event_cap_usd"] == 7.5
    assert budget["risk_promotion_evidence"]["risk_promotion_allowed"] is False
    assert budget["risk_promotion_evidence"]["blocker_codes"] == [
        "realized_pnl_source_not_account_or_db_confirmed",
        "open_unrealized_pnl_not_risk_promotable",
    ]


def test_live_signal_aggregation_budget_promotes_only_account_confirmed_closed_pnl_pytest() -> None:
    aggregation = live_tick._build_live_signal_aggregation_evidence(
        event_id="wnba-phx-nyl-2026-05-27",
        session_date="2026-05-27",
        source="pytest",
    ...
    assert budget["unresolved_loss_exposure_usd"] == 0.0
    assert budget["event_cap_before_profit_addon_usd"] == 10.0
    assert budget["profit_ratcheted_requested_addon_usd"] == 8.0
    assert budget["profit_ratcheted_addon_usd"] == 8.0
    assert budget["profit_ratcheted_blocked_addon_usd"] == 0.0
    assert budget["event_cap_usd"] == 18.0
    assert budget["risk_promotion_evidence"]["risk_promotion_allowed"] is True
    assert budget["risk_promotion_evidence"]["source_confidence"] == "account_confirmed"
    assert budget["risk_promotion_evidence"]["requested_profit_addon_usd"] == 8.0
    assert budget["risk_promotion_evidence"]["allowed_profit_addon_usd"] == 8.0
    assert budget["risk_promotion_evidence"]["profit_addon_policy"]["mode"] == "dollar_for_dollar_realized_profit"


def test_live_signal_aggregation_separates_order_cap_from_event_cap_pytest() -> None:
    aggregation = live_tick._build_live_signal_aggregation_evidence(
        event_id="wnba-sea-tor-2026-05-30",
        session_date="2026-05-30",
        source="pytest",
        plan={"active_strategies": []},
        evaluation={},
        market_state={"live_game_context": {"schema_version": "live_game_context_evidence_v1"}},
        portfolio_state={},
        direct_clob={},
        min_size=1.0,
        min_buy_notional_usd=1.0,
    ...
    budget = aggregation["event_risk_budget"]
    assert budget["base_event_cap_usd"] == 16.0
    assert budget["event_cap_usd"] == 16.0
    assert budget["policy"]["absolute_event_cap_usd"] == 16.0
    assert budget["policy"]["side_budget_mode"] == "balanced_50_50"
    assert budget["side_budget_caps_usd"] == {"side_a": 8.0, "side_b": 8.0}


def test_live_signal_aggregation_rebases_manual_ladder_budget_without_freezing_parallel_sleeves_pytest() -> None:
    aggregation = live_tick._build_live_signal_aggregation_evidence(
        event_id="nba-sas-okc-2026-05-30",
        session_date="2026-05-30",
        source="pytest",
        plan={
            ...
            "active_strategies": [
                {
                    "strategy_id": "okc-comeback-grid",
                    "family": "comeback_micro_grid",
                    "side": "Thunder",
        ...
        market_state={
            "live_game_context": {
                ...
                "classification_snapshot": {
                    "period": 3,
                    "clock_seconds_remaining": 280,
                    "score_gap": -7,
                    "underdog_price": 0.35,
                    "star_foul_trouble": True,
                    "recent_run_margin": 8,
                },
                "game_scenario": {"scenario_level": "B", "confidence": 0.73},
                "outcome_contexts": [{"side": "Thunder", "price": 0.35}],
            }
        },
        portfolio_state={
        ...
        direct_clob={
            ...
            "open_orders": {
                "orders": [
                    {"id": "0xsell43", "token_id": "token-okc", "side": "SELL", "size": 10.0, "price": 0.43},
                    {"id": "0xsell50", "token_id": "token-okc", "side": "SELL", "size": 6.8, "price": 0.50},
                    {"id": "0xbuy30", "token_id": "token-sas", "side": "BUY", "size": 10.0, "price": 0.30},
                ]
            },
        },
        min_size=5.0,
    ...
    budget = aggregation["event_risk_budget"]
    rebase = budget["manual_interference_budget_rebase"]
    buckets = rebase["budget_buckets"]
    decision = aggregation["decision"]
    blocker_reasons = {item["reason_code"] for item in decision["blocker_artifacts"]}
    assert rebase["status"] == "active"
    assert rebase["account_total_notional_usd"] == 17.16
    assert rebase["autonomous_used_notional_usd"] == 0.0
    assert buckets["manual_imported"]["account_notional_usd"] == 17.16
    assert buckets["manual_imported"]["consumes_autonomous_budget"] is False
    assert buckets["micro_band_scalp"]["status"] == "available_after_rebase"
    assert buckets["rebound_snipe"]["status"] == "review_ready"
    assert buckets["rebound_snipe"]["configured_sleeve_count"] == 1
    assert buckets["rebound_snipe"]["mini_review_recommended"] is True
    assert "star_foul_trouble" in buckets["rebound_snipe"]["reason_codes"]
    assert "low_price_close_game_dislocation" in buckets["rebound_snipe"]["reason_codes"]
    assert budget["used_notional_usd"] == 0.0
    assert decision["decision_type"] == "order_intent_candidate"
    assert decision["order_intent_candidates"][0]["requested_shares"] == 5.0
    assert "duplicate_exposure_risk" not in blocker_reasons
    assert "event_budget_exceeded" not in blocker_reasons
