...
def test_janus_worker_restart_readback_flags_stale_running_api_contract_gap(tmp_path: Path) -> None:
    status = {
        "ok": True,
        "status": "stopped",
        ...
        "config": {
            ...
            "source": "old-live-window",
        },
    }

    readback = janus_worker.build_live_strategy_worker_restart_readback(
        status,
        api_root="http://janus.local",
        session_date="2026-06-01",
        expected_event_ids=["nba-cle-nyk-2026-06-01"],
    )
    artifact = janus_worker.write_live_strategy_worker_restart_readback_artifact(readback, root=tmp_path)

    assert readback["schema_version"] == "live_strategy_worker_restart_readback_v1"
    assert readback["schema_contract"]["schema_version"] == "live_strategy_worker_restart_readback_contract_v1"
    assert readback["status_endpoint_has_config_trust"] is False
    assert readback["restart_allowed"] is False
    assert readback["gate"] == "YELLOW"
    assert readback["execution_authority"] is False
    assert readback["order_endpoint_call_allowed"] is False
    assert readback["worker_start_allowed_by_this_artifact"] is False
    assert readback["issues"] == [
        "config_trust_missing_from_running_api",
        "expected_event_scope_mismatch",
    ]
    assert Path(artifact["path"]).exists()
    assert json.loads(Path(artifact["current_path"]).read_text(encoding="utf-8"))["gate"] == "YELLOW"


def test_janus_worker_restart_readback_accepts_fail_closed_contract_status() -> None:
    status = {
        "ok": True,
        "status": "stopped",
        "worker_thread_alive": False,
        "config": {"session_date": "2026-06-01", "event_ids": ["nba-cle-nyk-2026-06-01"]},
        "config_trust": {
            "config_trust_status": "stale_inert_snapshot",
            "start_allowed": False,
            "restart_allowed": False,
            "stale_config_reason": "worker_stopped_requires_explicit_control_payload",
            "effective_event_ids": ["nba-cle-nyk-2026-06-01"],
            "schema_contract": {"schema_version": "live_strategy_worker_config_trust_contract_v1"},
        },
    }
    ...
    assert readback["status_endpoint_has_config_trust"] is True
    assert readback["status_endpoint_has_config_trust_contract"] is True
    assert readback["gate"] == "GREEN"
    assert readback["issues"] == []
    assert readback["restart_allowed"] is False
...
def test_janus_live_strategy_tick_bridge_preserves_legacy_cli_shape() -> None:
    ...
    assert janus_live_strategy_tick.build_live_strategy_tick_kwargs(args) == {
        ...
        "order_sizing_mode": "operator_minimum_order",
        "share_precision": 3,
        "auto_protect_manual_positions": False,
...
def test_risk_budget_minimum_order_and_kill_switch_checks_block_missing_inputs() -> None:
    ...
    assert "minimum_limit_order_size_not_met" in minimum.blockers
    assert "minimum_order_size_not_met" in minimum.blockers
    assert "minimum_buy_notional_not_met" not in minimum.blockers
    assert minimum.evidence["policy_schema_version"] == "polymarket_order_type_minimum_policy_v1"
    assert minimum.evidence["order_type_minimum_rule"] == "limit_min_shares"
    assert minimum.evidence["share_minimum_applies"] is True
    assert minimum.evidence["notional_minimum_applies"] is False
    assert kill_switch.passed is False
    assert "kill_switch_not_clear" in kill_switch.blockers


def test_minimum_order_policy_is_order_type_aware_for_limit_and_market_buys() -> None:
    low_notional_limit = evaluate_minimum_order_policy(
        PolymarketFallbackIntent(
            action="open_position",
            account_id="account-1",
            market_slug="test-market",
            ...
            price="0.10",
            size="5",
            reason="limit order below one dollar but at five shares",
            metadata={"order_type": "limit"},
        )
    )
    low_share_market = evaluate_minimum_order_policy(
        PolymarketFallbackIntent(
            action="open_position",
            account_id="account-1",
            market_slug="test-market",
            ...
            price="0.50",
            size="2",
            reason="market order below five shares but one dollar notional",
            metadata={"order_type": "market"},
        ),
        market_order_exception_approved=True,
    )
    under_notional_market = evaluate_minimum_order_policy(
        PolymarketFallbackIntent(
            action="open_position",
            account_id="account-1",
            market_slug="test-market",
            ...
            price="0.20",
            size="4",
            reason="market order below one dollar",
            metadata={"order_type": "market"},
        ),
        market_order_exception_approved=True,
    )

    assert low_notional_limit.passed is True
    assert low_notional_limit.evidence["order_type_minimum_rule"] == "limit_min_shares"
    assert low_notional_limit.evidence["required_min_size"] == 5.0
    assert low_notional_limit.evidence["required_market_notional_usd"] is None

    assert low_share_market.passed is True
    assert low_share_market.evidence["order_type_minimum_rule"] == "market_min_notional"
    assert low_share_market.evidence["required_min_size"] is None
    assert low_share_market.evidence["required_market_notional_usd"] == 1.0

    assert under_notional_market.passed is False
    assert "minimum_order_size_not_met" not in under_notional_market.blockers
    assert "minimum_market_notional_not_met" in under_notional_market.blockers
    assert "minimum_buy_notional_not_met" in under_notional_market.blockers
