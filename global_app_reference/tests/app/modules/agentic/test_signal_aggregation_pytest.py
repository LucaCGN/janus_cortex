...
def test_aggregator_merges_confirming_buy_signals_into_one_candidate_pytest() -> None:
    ...
    decision = aggregate_live_signals(
        [
            _signal(source="deterministic", confidence=0.71, payload={"minimum_order_policy": minimum_order_policy}),
    ...
    assert candidate.minimum_order_policy == minimum_order_policy
...
def test_aggregator_keeps_duplicate_cooldown_from_suppressing_reduce_exit_pytest() -> None:
    reduce = _signal(
        signal_type="reduce",
        side="Thunder",
        token="token-thunder",
    ...
    reduce.payload = {"sleeve_id": "okc-grid", "trigger_source": "reduce_stop_lifecycle"}
    duplicate_block = _signal(
        signal_type="block",
        side="Thunder",
        token="token-thunder",
        reason="position_limit_reached",
    )
    duplicate_block.payload = {"aggregation_scope": "sleeve", "sleeve_id": "okc-grid"}
    duplicate_block_2 = _signal(
        signal_type="block",
        side="Thunder",
        token="token-thunder",
        reason="position_limit_reached",
    )
    duplicate_block_2.payload = {"aggregation_scope": "sleeve", "sleeve_id": "okc-grid"}

    decision = aggregate_live_signals(
        [duplicate_block, duplicate_block_2, reduce],
        event_id=EVENT_ID,
        generated_at_utc=BASE_TIME + timedelta(seconds=5),
    )
    ...
    assert decision.order_intent_candidates[0].signal_type == "reduce"
    assert any(blocker.reason_code == "duplicate_signal_cooldown" for blocker in decision.blocker_artifacts)


def test_aggregator_keeps_paired_duplicate_buy_block_from_suppressing_reduce_exit_pytest() -> None:
    reduce = _signal(
        signal_type="reduce",
        side="Thunder",
        token="token-thunder",
    ...
    reduce.payload = {"sleeve_id": "okc-grid", "trigger_source": "reduce_stop_lifecycle"}
    duplicate_buy_block = _signal(
        signal_type="block",
        side="Thunder",
        token="token-thunder",
        reason="open_paired_sell_blocks_duplicate_buy",
    )
    duplicate_buy_block.payload = {
        "aggregation_scope": "sleeve",
        "sleeve_id": "okc-grid",
        "trigger_source": "paired_microcycle",
    }

    decision = aggregate_live_signals(
        [duplicate_buy_block, reduce],
        event_id=EVENT_ID,
        generated_at_utc=BASE_TIME + timedelta(seconds=5),
    )
...
def test_aggregator_keeps_sleeve_local_blocker_from_suppressing_independent_candidate_pytest() -> None:
    ...
    blocked_sleeve.payload = {
        "aggregation_scope": "sleeve",
        "sleeve_id": "sas-grid",
        "trigger_id": "sas-grid-price-band-gate",
        "trigger_type": "price_band_gate",
        "trigger_source": "sleeve_gate",
    }
    ...
    blocker_detail = decision.blocker_artifacts[0].detail
    assert blocker_detail["blocker_scope_schema_version"] == "live_signal_blocker_scope_v1"
    assert blocker_detail["affected_sleeve_ids"] == ["sas-grid"]
    assert blocker_detail["candidate_sleeve_ids"] == ["okc-grid"]
    assert blocker_detail["unaffected_sleeve_ids"] == ["okc-grid"]
    assert blocker_detail["trigger_id"] == "sas-grid-price-band-gate"
    assert blocker_detail["trigger_type"] == "price_band_gate"
    assert blocker_detail["trigger_source"] == "sleeve_gate"
    assert blocker_detail["expected_next_action"] == "preserve_unrelated_order_candidates"
...
def test_aggregator_keeps_local_block_local_without_candidate_pytest() -> None:
    ...
    assert decision.order_intent_candidates == []
    blocker_detail = decision.blocker_artifacts[0].detail
    assert blocker_detail["scope"] == "local_sleeve"
    assert blocker_detail["candidate_blocking"] is False
    assert blocker_detail["affected_sleeve_ids"] == ["sas-grid"]
    assert blocker_detail["expected_next_action"] == "preserve_unrelated_order_candidates"


def test_aggregator_keeps_stale_unrelated_signal_from_suppressing_fresh_candidate_pytest() -> None:
    stale_spurs = _signal(signal_type="buy", side="Spurs", token="token-spurs", stale=True)
    stale_spurs.risk_request = LiveSignalRiskRequest(sleeve_id="sas-grid", requested_shares=5, max_price=0.54)
    ...
    assert decision.order_intent_candidates[0].side == "Thunder"
    blocker_detail = decision.blocker_artifacts[0].detail
    assert blocker_detail["scope"] == "signal"
    assert blocker_detail["candidate_blocking"] is False
    assert blocker_detail["affected_sleeve_ids"] == ["sas-grid"]
    assert blocker_detail["expected_next_action"] == "suppress_blocked_signal_and_continue"
...
def test_aggregator_records_same_band_duplicate_scope_pytest() -> None:
    decision = aggregate_live_signals(
        [
            _signal(source="deterministic", emitted_offset_seconds=0, payload={"cycle_id": "cycle-okc-1"}),
            _signal(source="deterministic", emitted_offset_seconds=10, payload={"cycle_id": "cycle-okc-1"}),
        ],
        event_id=EVENT_ID,
        control=LiveSignalAggregationControl(cooldown_seconds=90),
        generated_at_utc=BASE_TIME + timedelta(seconds=15),
    ...
    duplicate = next(blocker for blocker in decision.blocker_artifacts if blocker.reason_code == "duplicate_signal_cooldown")
    assert decision.duplicate_signal_count == 1
    assert duplicate.detail["duplicate_scope"] == "same_source_side_sleeve_band_cycle"
    assert duplicate.detail["duplicate_budget_claim_blocked"] is True
    assert duplicate.detail["sleeve_id"] == "okc-grid"
    assert duplicate.detail["cycle_id"] == "cycle-okc-1"
    assert duplicate.detail["price_band"]["current_price"] == 0.22


def test_aggregator_does_not_cooldown_distinct_cycles_on_same_band_pytest() -> None:
    decision = aggregate_live_signals(
        [
            _signal(source="deterministic", emitted_offset_seconds=0, payload={"cycle_id": "cycle-okc-1"}),
            _signal(source="deterministic", emitted_offset_seconds=10, payload={"cycle_id": "cycle-okc-2"}),
        ],
        event_id=EVENT_ID,
        control=LiveSignalAggregationControl(cooldown_seconds=90),
        generated_at_utc=BASE_TIME + timedelta(seconds=15),
    ...
    assert decision.decision_type == "order_intent_candidate"
    assert decision.duplicate_signal_count == 0
    assert {blocker.reason_code for blocker in decision.blocker_artifacts} == set()
    assert len(decision.order_intent_candidates[0].supporting_signal_ids) == 2


def test_aggregator_allows_reviewed_multi_lot_same_band_ladder_pytest() -> None:
    payload = {
        "reviewed_multi_lot_ladder": True,
        "allow_same_band_multi_lot": True,
        "multi_lot_ladder_id": "okc-reviewed-ladder",
    }
    decision = aggregate_live_signals(
        [
            _signal(source="deterministic", emitted_offset_seconds=0, payload=payload),
            _signal(source="deterministic", emitted_offset_seconds=10, payload=payload),
        ],
        event_id=EVENT_ID,
        control=LiveSignalAggregationControl(cooldown_seconds=90),
        generated_at_utc=BASE_TIME + timedelta(seconds=15),
