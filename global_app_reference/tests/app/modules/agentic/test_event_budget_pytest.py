...
def test_profit_ratcheted_event_budget_adds_realized_profit_without_counting_open_losses_pytest() -> None:
    ...
    assert winner.event_cap_before_profit_addon_usd == 10.0
    assert winner.profit_ratcheted_requested_addon_usd == 7.0
    assert winner.profit_ratcheted_addon_usd == 7.0
    assert winner.profit_ratcheted_blocked_addon_usd == 0.0
    assert winner.event_cap_usd == 17.0
    assert winner.profit_ratcheted_policy["mode"] == "dollar_for_dollar_realized_profit"
    assert winner.profit_ratcheted_policy["source_confidence_required"] == ["account_confirmed", "db_confirmed"]
    assert winner.profit_ratcheted_policy["blocker_codes"] == ["unresolved_loss_exposure_deducted_from_profit_addon"]
...
def test_calibrate_event_risk_policy_from_history_promotes_winning_sample_pytest() -> None:
    ...
    assert calibration["risk_promotion_allowed"] is True
    assert "increase_realized_profit_addon_cap" in calibration["recommended_changes"]
    assert calibration["policy"]["profit_ratcheted_reinvestment_pct"] == 1.0
    assert calibration["policy"]["max_profit_ratcheted_addon_usd"] > 10.0
...
def test_calibrate_event_risk_policy_ignores_unconfirmed_realized_rows_pytest() -> None:
    ...
    assert calibration["calibration_input_count"] == 1
    assert calibration["risk_promotion_eligible_count"] == 1
    assert calibration["risk_promotion_allowed"] is False
    assert calibration["policy"]["profit_ratcheted_reinvestment_pct"] == 1.0


def test_calibrate_event_risk_policy_holds_promotion_for_unresolved_lifecycle_pytest() -> None:
    calibration = calibrate_event_risk_policy_from_history(
        [
            {
                "actual_pnl_usd": 1.4,
                "source_confidence": "account_confirmed",
                "unresolved_evidence": {"unknown_lifecycle_count": 25},
            },
            {
                "actual_pnl_usd": 1.299515,
                "source_confidence": "account_confirmed",
                "final_cleanup_reason_counts": {
                    "final_settled_open_order_reconciliation_required": 2,
                },
            },
        ],
        risk_mode="development",
    ...
    assert calibration["status"] == "promotion_blocked"
    assert calibration["calibration_input_count"] == 2
    assert calibration["risk_promotion_eligible_count"] == 0
    assert calibration["risk_promotion_allowed"] is False
    assert calibration["policy"]["profit_ratcheted_reinvestment_pct"] == 1.0
    assert calibration["recommended_changes"] == ["hold_risk_promotion_until_lifecycle_clean"]
    assert calibration["risk_promotion_blocker_counts"] == {
        "unresolved_final_cleanup_blocks_risk_promotion": 1,
        "unresolved_lifecycle_blocks_risk_promotion": 1,
    }


def test_calibrate_event_risk_policy_treats_may30_profit_as_input_not_cap_authority_pytest() -> None:
    calibration = calibrate_event_risk_policy_from_history(
        [
            {
                "event_id": "wnba-sea-tor-2026-05-30",
                "actual_pnl_usd": -1.150220,
                "peak_cash_at_risk_usd": 3.300220,
                "source_confidence": "account_confirmed",
            },
            {
                "event_id": "wnba-la-conn-2026-05-30",
                "actual_pnl_usd": 1.400000,
                "peak_cash_at_risk_usd": 6.384560,
                "source_confidence": "account_confirmed",
                "unresolved_evidence": {"unknown_lifecycle_count": 25},
            },
            {
                "event_id": "wnba-ind-por-2026-05-30",
                "actual_pnl_usd": 1.299515,
                "peak_cash_at_risk_usd": 9.930486,
                "source_confidence": "account_confirmed",
                "final_cleanup_reason_counts": {
                    "final_settled_open_order_reconciliation_required": 2,
            ...
            },
            {
                ...
                "actual_pnl_usd": 5.067219,
                "peak_cash_at_risk_usd": 16.293076,
                "source_confidence": "account_confirmed",
            },
        ],
        risk_mode="development",
    ...
    assert calibration["status"] == "promotion_blocked"
    assert calibration["calibration_input_count"] == 4
    assert calibration["sample_size"] == 2
    assert calibration["risk_promotion_eligible_count"] == 2
    assert calibration["risk_promotion_allowed"] is False
    assert calibration["policy"]["absolute_event_cap_usd"] == 10.0
    assert calibration["policy"]["profit_ratcheted_reinvestment_pct"] == 1.0
    assert calibration["policy"]["max_profit_ratcheted_addon_usd"] == 10.0
    assert calibration["recommended_changes"] == ["hold_risk_promotion_until_lifecycle_clean"]
    assert calibration["risk_promotion_blocker_counts"] == {
