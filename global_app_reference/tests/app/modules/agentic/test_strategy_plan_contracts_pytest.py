...
def test_strategy_plan_blocks_same_price_buy_across_sleeves_pytest() -> None:
    plan = StrategyPlan(
        event_id="event-1",
        market_id="market-1",
        active_strategies=[
            ActiveStrategy(
                strategy_id="storm-grid",
                family="price_stability_micro_grid",
                side="Seattle Storm",
                sleeve_id="storm-grid",
                sleeve_role="grid_scalp",
                budget_usd=4.0,
                entry_rules={
                    "outcome_id": "outcome-sea",
                    "token_id": "token-sea",
                    "side": "buy",
                    "price": 0.40,
                    "size": 5,
                    "price_band": [0.30, 0.45],
                },
                exit_rules={"target_policy": "micro_grid_scaled", "min_target_cents": 2},
            ),
            ActiveStrategy(
                strategy_id="storm-core",
                family="core_hold_live_validation",
                side="Seattle Storm",
                sleeve_id="storm-core",
    ...
    result = evaluate_strategy_plan(
        plan,
        market_state={"token_states": {"token-sea": {"price": 0.40}}},
        dry_run=True,
    )

    assert result.intent_count == 1
    assert result.intents[0].strategy_id == "storm-grid"
    assert any(
        blocker["strategy_id"] == "storm-core"
        and blocker["reason"] == "economic_duplicate_buy_intent_blocked"
        and blocker["duplicate_of_strategy_id"] == "storm-grid"
...
def test_strategy_plan_evaluator_promotes_live_aggregation_candidate_pytest() -> None:
    ...
    assert result.intents[0].metadata["minimum_order_policy"]["schema_version"] == "polymarket_order_type_minimum_policy_v1"
    assert result.intents[0].metadata["minimum_order_policy"]["order_type_minimum_rule"] == "limit_min_shares"
...
def test_strategy_plan_evaluator_prioritizes_reduce_aggregation_over_plan_buy_pytest() -> None:
    plan = StrategyPlan(
        event_id="event-1",
        market_id="market-1",
        active_strategies=[
                    ...
                    "order_intent_candidates": [
                        {
                            ...
                            "trigger_type": "stop_triggered",
                            "trigger_source": "reduce_stop_lifecycle",
                            "requested_shares": 5,
                            "max_price": 0.37,
                            "reason_codes": ["reduce_stop_triggered"],
                        }
                    ]
                }
            }
        },
        portfolio_state={"open_positions": 1, "open_orders": 0, "pending_intents": 0},
        dry_run=True,
    )

    assert result.intent_count == 1
    assert result.intents[0].side == "sell"
    assert result.intents[0].price == 0.37
    assert result.intents[0].metadata["aggregation_candidate"]["signal_type"] == "reduce"
    assert any(blocker["reason"] == "exit_intent_priority_blocks_entry" for blocker in result.blockers)
...
def test_strategy_plan_evaluator_blocks_economic_duplicate_live_aggregation_buy_pytest() -> None:
    plan = StrategyPlan(
        event_id="event-1",
        market_id="market-1",
        active_strategies=[
                    ...
                    "order_intent_candidates": [
                        {
                            ...
                            "sleeve_id": "okc-core",
                            "strategy_id": "grid-1",
                            "strategy_family": "core_hold_live_validation",
                            "requested_shares": 5,
                            "max_price": 0.2,
                        }
                    ]
    ...
    assert result.intents[0].reason == "strategy_plan_entry"
    assert any(
        blocker["scope"] == "live_signal_aggregation"
        and blocker["reason"] == "economic_duplicate_buy_intent_blocked"
        and blocker["duplicate_of_strategy_id"] == "grid-1"
        for blocker in result.blockers
    )


def test_strategy_plan_evaluator_blocks_live_aggregation_buy_with_pending_same_sleeve_pytest() -> None:
    plan = StrategyPlan(
        event_id="wnba-la-conn-2026-05-30",
        market_id="market-1",
        active_strategies=[
            ActiveStrategy(
                strategy_id="observer",
                family="live_validation",
                side="Connecticut Sun",
                budget_usd=0.0,
                entry_rules={},
                    ...
                    "order_intent_candidates": [
                        {
                            ...
                            "side": "Connecticut Sun",
                            "market_id": "market-1",
                            "outcome_id": "outcome-con",
                            "market_token_id": "token-con",
                            "sleeve_id": "connecticut_sun-opportunistic-tail",
                            "strategy_id": "conn-tail",
                            "strategy_family": "opportunistic_profit_ratcheted_entry",
                            "requested_shares": 5,
                            "max_price": 0.36,
                            "lifecycle_policy": {"target_delta_cents": 2.0, "max_loss_usd": 0.0},
                        }
                    ]
                }
            }
    ...
    assert result.intent_count == 0
    assert any(
        blocker.get("scope") == "live_signal_aggregation"
        and blocker.get("reason") == "pending_intent_limit_reached"
        and blocker.get("pending_order_ids") == ["0xpending"]
        for blocker in result.blockers
    )


def test_strategy_plan_evaluator_blocks_live_aggregation_buy_with_direct_open_buy_order_pytest() -> None:
    plan = StrategyPlan(
        event_id="wnba-la-conn-2026-05-30",
        market_id="market-1",
        active_strategies=[],
    ...
    assert any(
        blocker.get("scope") == "live_signal_aggregation"
        and blocker.get("reason") == "direct_open_buy_order_present"
        and blocker.get("open_order_ids") == ["0xdirect"]
        for blocker in result.blockers
    )


def test_strategy_plan_evaluator_blocks_plan_buy_before_game_is_live_pytest() -> None:
    plan = StrategyPlan(
    ...
    result = evaluate_strategy_plan(
        plan,
        market_state={"game": {"game_status": 1, "game_status_text": "9:00 pm ET", "period": 0}},
        dry_run=True,
    )

    assert result.intent_count == 0
    assert result.blockers[0]["reason"] == "game_not_live_no_entry"
    assert result.blockers[0]["game_status"] == 1


def test_strategy_plan_evaluator_blocks_aggregation_buy_before_game_is_live_pytest() -> None:
    plan = StrategyPlan(
        event_id="nba-sas-okc-2026-05-30",
        market_id="market-1",
        active_strategies=[
    ...
    result = evaluate_strategy_plan(
        plan,
        market_state={
            "game": {"game_status": 1, "game_status_text": "9:00 pm ET", "period": 0},
            "live_signal_aggregation": {
                "decision": {
                    "order_intent_candidates": [
                        {
    ...
    assert any(
        blocker.get("scope") == "live_signal_aggregation"
        and blocker.get("reason") == "game_not_live_no_entry"
        and blocker.get("game_status") == 1
        for blocker in result.blockers
    )
...
def test_strategy_plan_evaluator_uses_order_type_minimums_for_limit_buys_pytest() -> None:
    ...
    below_share_minimum_plan = StrategyPlan(
        event_id="event-1",
        market_id="market-1",
        active_strategies=[
            ActiveStrategy(
                strategy_id="below-share-minimum",
                family="grid",
                side="underdog",
                budget_usd=2.0,
                entry_rules={
    ...
    low_notional_limit_plan = StrategyPlan(
        event_id="event-1",
        market_id="market-1",
        active_strategies=[
            ActiveStrategy(
                strategy_id="low-notional-limit",
                family="grid",
                side="underdog",
                budget_usd=2.0,
                entry_rules={
    ...
    below_share_result = evaluate_strategy_plan(below_share_minimum_plan)
    low_notional_limit_result = evaluate_strategy_plan(low_notional_limit_plan)

    assert market_result.intent_count == 0
    assert market_result.blockers[0]["reason"] == "market_orders_disabled"
    assert below_share_result.intent_count == 0
    assert below_share_result.blockers[0]["reason"] == "minimum_size_not_met"
    assert below_share_result.blockers[0]["minimum_order_policy"]["order_type_minimum_rule"] == "limit_min_shares"
    assert below_share_result.blockers[0]["minimum_order_policy"]["share_minimum_applies"] is True
    assert below_share_result.blockers[0]["minimum_order_policy"]["notional_minimum_applies"] is False
    assert low_notional_limit_result.intent_count == 1
    assert low_notional_limit_result.intents[0].metadata["required_notional_usd"] == 0.95
    assert low_notional_limit_result.intents[0].metadata["minimum_order_policy"]["minimum_rule_status"] == "passed"
    assert low_notional_limit_result.intents[0].metadata["minimum_order_policy"]["required_market_notional_usd"] is None
...
def test_operator_minimum_buy_size_can_run_small_parallel_orders_pytest() -> None:
    plan = StrategyPlan(
        event_id="event-sea-tor",
        market_id="market-1",
        active_strategies=[
            ActiveStrategy(
                strategy_id="sea-favorite-scalp",
                family="price_stability_micro_grid",
                side="Storm",
                budget_usd=2.0,
                entry_rules={
                    ...
                    "price_policy": "current_ask",
                    "max_price": 0.82,
                    "price_band": [0.70, 0.82],
                    "max_scoreboard_age_seconds": 5,
                    "max_abs_score_gap": 8,
                },
                exit_rules={"target_policy": "micro_grid_scaled", "target_return_fraction": 0.05},
                stop_rules={"max_adverse_cents": 3},
            )
        ],
    )
    ...
    assert result.intent_count == 1
    assert result.intents[0].size == 1.263
    assert result.intents[0].metadata["required_notional_usd"] == 1.0104
    assert result.intents[0].metadata["sizing_policy"]["max_buy_notional_usd"] == 2.0


def test_fixed_min_shares_policy_keeps_buy_order_at_exchange_minimum_pytest() -> None:
    plan = StrategyPlan(
        event_id="event-sea-tor",
        market_id="market-1",
        active_strategies=[
            ActiveStrategy(
                strategy_id="sea-low-band-scalp",
                family="price_stability_micro_grid",
                side="Storm",
                budget_usd=20.0,
                entry_rules={
    ...
    assert result.intent_count == 1
    assert result.intents[0].size == 5
    assert result.intents[0].metadata["required_notional_usd"] == 0.9
    assert result.intents[0].metadata["sizing_policy"]["mode"] == "fixed_min_shares"
    assert result.intents[0].metadata["sizing_policy"]["min_buy_notional_usd"] == 0.0
