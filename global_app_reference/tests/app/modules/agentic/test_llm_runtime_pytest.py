...
def test_fixed_min_shares_policy_downgrades_manual_rebase_to_mini_pytest() -> None:
    triggers = detect_llm_runtime_triggers(
        event_id="nba-sas-okc-2026-05-30",
        operator_interventions=[
            {
                "action": "adopt_operator_open_order",
                "direct_order_id": "0xmanual-sell",
                "token_id": "token-okc",
                "order_side": "sell",
                "current_order": {"size": 5, "price": 0.50},
            }
        ],
        portfolio_state={
            "open_orders": 1,
    ...
    assert [trigger.trigger_type for trigger in triggers] == ["manual_operator_order"]
    assert decision.selected_model == MINI_MODEL
    assert decision.selected_tier == "mini"
    assert decision.fallback_alias == FRONTIER_MODEL
    assert "manual_operator_order" in decision.critical_reasons
    assert "frontier_downgraded_operator_minimum_order_policy" in decision.critical_reasons
...
def test_prompt_contract_and_revision_request_include_safety_sections_pytest() -> None:
                ...
                "db_context_sources": [
                    {
                        ...
                        "source_id": "nba_team_stats_snapshot",
                        "source_kind": "team_stats_snapshot",
                        "status": "current",
                        "source_timestamp_utc": "2026-05-11T12:00:00Z",
                        "included_in_llm_context": True,
                        "team_stat_snapshot_count": 2,
                        "player_stat_snapshot_count": 20,
                    }
                ],
    ...
    assert "db_context_sources" in request.prompt_contract["required_input_sections"]
    assert request.db_context_sources[0]["source_id"] == "nba_team_stats_snapshot"
    assert request.db_context_sources[0]["included_in_llm_context"] is True
    assert request.db_stat_context_trace["schema_version"] == "db_stat_context_trace_v1"
...
def test_quarter_end_runtime_trace_persists_to_artifact_pytest(tmp_path) -> None:
    ...
    assert payload["quarter_revision_reviews"][0]["schema_version"] == "strategy_plan_quarter_revision_review_v1"
    assert payload["quarter_revision_reviews"][0]["quarter_label"] == "q1_end"
    quarter_path = payload["quarter_revision_reviews"][0]["path"]
    quarter_review = json.loads(quarter_path and open(quarter_path, encoding="utf-8").read())
    assert quarter_review["event_id"] == "nba-det-cle-2026-05-11"
    assert quarter_review["review_status"] == "review_pending_unavailable"
    assert quarter_review["order_endpoint_call_allowed"] is False
    assert quarter_review["execution_authority"] is False


def test_quarter_revision_artifact_records_sleeve_decisions_and_shadow_results_pytest(tmp_path) -> None:
    class FakeResponses:
        def parse(self, **kwargs):
            ...
            revised_plan = {
                ...
                "generated_at_utc": "2026-05-11T00:30:00Z",
                "active_strategies": [
                    {"strategy_id": "core-det", "sleeve_id": "core-det", "family": "core_hold", "side": "DET"},
                    {"strategy_id": "rebound-cle", "sleeve_id": "rebound-cle", "family": "rebound_snipe", "side": "CLE"},
                ],
            }
            return SimpleNamespace(
                output_parsed=text_format(
                    ...
                    revised_strategy_plan_json=json.dumps(revised_plan),
                    reconciliation_actions_json=json.dumps([{"action": "keep", "strategy_id": "core-det"}]),
                    blocked_actions_json=json.dumps([{"reason": "remove_shadow_grid", "strategy_id": "grid-cle"}]),
                    confidence=0.81,
                    skipped_reason=None,
    ...
    trace = build_llm_runtime_trace(
        ...
        current_plan={
            ...
            "active_strategies": [
                {
                    "strategy_id": "core-det",
                    "sleeve_id": "core-det",
                    "family": "core_hold",
                    "side": "DET",
                },
                {
                    "strategy_id": "grid-cle",
                    "sleeve_id": "grid-cle",
                    "family": "grid_scalp",
                    "side": "CLE",
        ...
        strategy_decisions=[
            {
                "strategy_id": "core-det",
                "actual_result": {"status": "observed", "net_pnl_usd": 0.12},
                "shadow_result": {"status": "not_applicable"},
            },
            {
                "strategy_id": "grid-cle",
                "actual_result": {"status": "shadow_only"},
                "shadow_result": {"status": "observed", "would_have_net_pnl_usd": -0.04},
            },
        ],
    )
    ...
    assert persistence["quarter_revision_review_count"] == 1
    quarter_review = json.loads(open(persistence["quarter_revision_review_artifacts"][0]["path"], encoding="utf-8").read())
    decisions = quarter_review["revision_decisions"]
    sleeves = {row["strategy_id"]: row for row in quarter_review["reviewed_sleeves"]}
    assert quarter_review["schema_version"] == "strategy_plan_quarter_revision_review_v1"
    assert quarter_review["quarter_label"] == "halftime_q2_end"
    assert quarter_review["review_status"] == "revision_recorded"
    assert decisions["kept_strategy_ids"] == ["core-det"]
    assert decisions["removed_strategy_ids"] == ["grid-cle"]
    assert decisions["added_strategy_ids"] == ["rebound-cle"]
    assert sleeves["core-det"]["actual_result"]["net_pnl_usd"] == 0.12
    assert sleeves["grid-cle"]["shadow_only"] is True
    assert sleeves["grid-cle"]["shadow_result"]["would_have_net_pnl_usd"] == -0.04
    assert quarter_review["order_endpoint_call_allowed"] is False
...
def test_manual_fixed_min_share_rebase_can_call_mini_when_event_budget_exceeded_pytest(tmp_path, monkeypatch) -> None:
    class FakeResponses:
        def parse(self, **kwargs):
            request_id = json.loads(kwargs["input"][1]["content"])["request_id"]
            text_format = kwargs["text_format"]
            return SimpleNamespace(
                output_parsed=text_format(
                    ...
                    selected_model=kwargs["model"],
                    revised_strategy_plan_json=None,
                    reconciliation_actions_json=json.dumps(["rebase_current_inventory"]),
                    blocked_actions_json="[]",
                    trace_metadata_json=json.dumps({"manual_rebase": True}),
                ),
                usage=SimpleNamespace(input_tokens=9, output_tokens=4),
            )

    monkeypatch.setenv("JANUS_LLM_EVENT_TOKEN_BUDGET", "1")
    day_root = tmp_path / "llm-runtime" / "2026-05-30"
    day_root.mkdir(parents=True)
    (day_root / "prior.json").write_text(
        json.dumps(
            {
    ...
    assert trace.status == "response_recorded"
    assert trace.revision_response is not None
    assert trace.revision_response.selected_model == MINI_MODEL
    assert trace.revision_response.trace_metadata["openai_call_attempted"] is True
    assert trace.revision_response.trace_metadata["llm_budget_status"]["hard_block"] is True
    assert trace.revision_response.trace_metadata["llm_manual_interference_budget_override"]["applied"] is True
