...
def test_pbp_annotation_detects_player_catalyst_foul_trouble_pytest() -> None:
    evidence = build_pbp_annotation_evidence(
        event_id="nba-test",
        live_state={
            "latest_snapshot": {
            ...
            "recent_play_by_play": [
                {"description": "Victor Wembanyama personal foul, his 5th foul"},
                {"description": "Thunder defensive rebound"},
            ],
        },
        source="pytest",
    )

    foul_tags = [tag for tag in evidence["tags"] if tag["tag_type"] == "player_foul_trouble_watch"]
    assert foul_tags
    assert "rebound_snipe" in foul_tags[0]["sleeve_relevance"]
    assert evidence["execution_boundary"] == "evidence_only"
