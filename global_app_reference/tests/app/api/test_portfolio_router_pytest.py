...
def test_cancel_manual_order_dry_run_is_read_only_pytest(monkeypatch) -> None:
    ...
    def fail_rate_limit(**_):
        raise AssertionError("dry-run cancel must not consume the order rate limiter")

    monkeypatch.setattr(portfolio_router, "enforce_order_rate_limit", fail_rate_limit)
    ...
    try:
        response = client.request(
            "DELETE",
            f"/v1/portfolio/orders/{order_id}",
            json={"account_id": ACCOUNT_ID, "reason": "preview cancel only", "dry_run": True},
        )
    finally:
    ...
    assert payload["event_type"] == "manual_cancel_dry_run"
    assert payload["summary"]["side_effects"]["orders_updated"] is False
    assert not any("UPDATE portfolio.orders" in query for query, _ in queries)
    assert not any("INSERT INTO portfolio.order_events" in query for query, _ in queries)
...
def test_order_lifecycle_report_flags_direct_flat_unknown_and_actor_cashflow_pytest() -> None:
    ...
    assert report["unresolved_lifecycle_reason_counts"] == {"direct_flat_open_order_missing_terminal_status": 1}
    ...
    assert report["actor_summary"]["manual_target_exit"]["unresolved_lifecycle_reason_counts"] == {
        "direct_flat_open_order_missing_terminal_status": 1
    }
    assert report["items"][1]["lifecycle_status"] == "direct_flat_status_unknown"
    assert report["items"][1]["unresolved_lifecycle_reason"]["reason"] == "direct_flat_open_order_missing_terminal_status"
    assert report["items"][1]["unresolved_lifecycle_reason"]["required_evidence"] == [
        "terminal_order_status",
        "linked_trade_row",
        "direct_clob_trade_match",
    ]


def test_order_lifecycle_report_classifies_open_order_evidence_gaps_pytest() -> None:
    rows = [
        _order_row(
            "janus-open-no-direct-snapshot",
    ...
    assert report["unknown_lifecycle_count"] == 2
    assert report["unresolved_lifecycle_reason_counts"] == {
        "direct_account_snapshot_unavailable_for_open_order": 1,
        "local_open_order_missing_external_id": 1,
    }
    reasons = [item["unresolved_lifecycle_reason"]["reason"] for item in report["items"]]
    assert reasons == [
        "direct_account_snapshot_unavailable_for_open_order",
        "local_open_order_missing_external_id",
    ]
...
def test_portfolio_manager_price_memory_collect_endpoint_dry_run_smoke_pytest() -> None:
    client = TestClient(create_app())
    client.app.dependency_overrides[get_db_connection] = _unused_fake_db_connection

    try:
        response = client.post(
            "/v1/portfolio/manager/price-memory/collect",
            json={
                "dry_run": True,
                "market_rows": [
                    {
                        "market_title": "Global price-memory market",
                        "market_slug": "global-price-memory-market",
                        "token_id": "token-price-memory",
                    }
                ],
    ...
    assert response.status_code == 200
    payload = response.json()
    collection = payload["price_memory_collection"]
    assert payload["live_order_impact"] == "read-only"
    assert collection["featured_count"] == 1
    assert payload["side_effects"]["orders_submitted"] is False


def test_portfolio_manager_strategy_worker_endpoints_dry_run_smoke_pytest() -> None:
    client = TestClient(create_app())
    client.app.dependency_overrides[get_db_connection] = _unused_fake_db_connection

    try:
        upsert = client.post(
            "/v1/portfolio/manager/strategy-workers",
            json={
                "dry_run": True,
                "worker_config": {
        ...
        tick = client.post(
            "/v1/portfolio/manager/strategy-workers/tick",
            json={
                "dry_run": True,
                "worker_config": {
                    "worker_id": "worker-api",
    ...
    assert upsert.json()["strategy_worker"]["worker_id"] == "worker-api"
    assert upsert.json()["side_effects"]["live_worker_started"] is False
    assert tick.status_code == 200
    assert tick.json()["strategy_worker_tick"]["status"] == "dry_run_tick_recorded"
    assert tick.json()["side_effects"]["orders_prepared"] is False
...
def test_portfolio_manager_order_management_ready_plan_stays_preview_only_pytest() -> None:
    ...
    minimum_policy = preview["concrete_adapter_proof"]["minimum_order_policy"]
...
def test_portfolio_manager_order_management_live_path_places_order_when_gate_proven_pytest(monkeypatch) -> None:
    ...
    assert runtime_evidence["requested_order"]["minimum_order_policy"]["schema_version"] == "polymarket_order_type_minimum_policy_v1"
    assert runtime_evidence["requested_order"]["minimum_order_policy"]["required_min_size"] == 5.0
    assert runtime_evidence["requested_order"]["minimum_order_policy"]["required_market_notional_usd"] is None
    assert execution["execution_payload"]["concrete_adapter_proof"]["minimum_order_policy"]["minimum_rule_status"] == "passed"
