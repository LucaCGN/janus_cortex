...
REBOUND_SNIPE_SIGNAL_SCHEMA_VERSION = "live_game_rebound_snipe_signal_v1"
...
def build_live_game_context_evidence(
    *,
    ...
    rebound_snipe = _rebound_snipe_signal_candidates(
        event_id=event_id,
        plan=plan,
    ...
    return {
        ...
        "rebound_snipe_signal_candidates": rebound_snipe,
        "notes": [
            ...
            "Rebound-snipe candidates require a reviewed rebound/comeback sleeve and still execute only through Janus gates.",
        ],
    }
...
def live_signals_from_live_game_context(evidence: dict[str, Any] | None) -> list[LiveSignal]:
    ...
    for candidate in evidence.get("rebound_snipe_signal_candidates") or []:
        if not isinstance(candidate, dict) or candidate.get("status") != "signal_candidate":
            continue
        signals.append(
            LiveSignal(
                ...
                price_band=LiveSignalPriceBand(
                    current_price=_float(candidate.get("current_price")),
                    target_price=_float(candidate.get("max_price")),
                    band_role="rebound_snipe_catalyst_entry",
                ),
                confidence=_float(candidate.get("confidence")),
                confidence_source="live_game_context:rebound_snipe_review",
                freshness=LiveSignalFreshness(source_timestamp_utc=now, stale=False),
                reason_codes=[str(reason) for reason in candidate.get("reason_codes") or []],
                risk_request=LiveSignalRiskRequest(
                    sleeve_id=_clean(candidate.get("sleeve_id")),
                ...
                evidence_paths=[str(path) for path in evidence.get("evidence_paths") or []],
                payload={
                    "schema_version": REBOUND_SNIPE_SIGNAL_SCHEMA_VERSION,
                    "strategy_id": _clean(candidate.get("strategy_id")),
                    "strategy_family": _clean(candidate.get("strategy_family")),
                    "sleeve_id": _clean(candidate.get("sleeve_id")),
                    "sleeve_group": _clean(candidate.get("sleeve_group")),
                    "sleeve_role": _clean(candidate.get("sleeve_role")),
                    "trigger_type": "rebound_snipe_catalyst_entry",
                    "trigger_source": "live_game_context",
                    "position_limit_scope": "local_sleeve",
                    "allow_existing_position_add": True,
                    "allow_inventory_adding": True,
                    "standalone_signal": True,
                    "manual_rebase_status": candidate.get("manual_rebase_status"),
                    "lifecycle_policy": candidate.get("lifecycle_policy") or {},
                    "game_scenario": evidence.get("game_scenario") or {},
                    "dynamic_risk_state": evidence.get("dynamic_risk_state") or {},
                    "ml_confidence": candidate.get("ml_confidence") or {},
...
def _classification_snapshot(*, market_state: dict[str, Any], outcomes: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "star_bench_rest": _player_shock_contains(market_state, "bench")
        or _player_shock_contains(market_state, "rest")
        or _player_shock_contains(market_state, "rotation"),
...
def _opportunistic_signal_candidates(
    *,
    ...
    scoreboard_age = _min_number([outcome.get("scoreboard_age_seconds") for outcome in outcomes])
    score_gap = _score_gap_for_outcomes(outcomes)
    if scoreboard_age is None or _is_stale(scoreboard_age) or score_gap is None:
        return [_blocked_opportunistic(event_id, "scoreboard_freshness_required", scenario, max(tail_budget, profit_risk_budget))]
    if scenario in {"D", "U"}:
...
def _rebound_snipe_signal_candidates(
    *,
    event_id: str,
    plan: dict[str, Any],
    market_state: dict[str, Any],
    ...
    score_gap = _score_gap_for_outcomes(outcomes)
    if scoreboard_age is None or _is_stale(scoreboard_age) or score_gap is None:
        return [_blocked_rebound_snipe(event_id, "scoreboard_freshness_required", scenario)]
    explicit = [outcome for outcome in outcomes if _is_rebound_snipe_context(outcome)]
    if not explicit:
        return [_blocked_rebound_snipe(event_id, "reviewed_rebound_snipe_sleeve_required", scenario)]
    candidate = min(
        explicit,
        key=lambda row: _float(row.get("best_ask") or row.get("price") or row.get("best_bid")) or 1.0,
    )
    price = _float(candidate.get("best_ask") or candidate.get("price"))
    if price is None or price <= 0.0:
        return [_blocked_rebound_snipe(event_id, "fresh_rebound_snipe_ask_required", scenario)]
    catalysts = _rebound_snipe_catalyst_reason_codes(snapshot=snapshot, candidate=candidate)
    if not catalysts:
        return [_blocked_rebound_snipe(event_id, "rebound_snipe_catalyst_required", scenario)]
    if scenario in {"D", "U"} and "low_price_close_game_dislocation" not in catalysts:
        return [_blocked_rebound_snipe(event_id, "scenario_blocks_rebound_snipe_without_price_dislocation", scenario)]
    shares = _rebound_snipe_requested_shares(plan=plan, candidate=candidate)
    requested_notional = shares * price
    manual_rebase = _manual_rebase_status(market_state=market_state, portfolio_state=portfolio_state)
    reason_codes = [
        *catalysts,
        f"scenario_{scenario}",
        "reviewed_rebound_snipe_sleeve_present",
    ]
    if manual_rebase == "required":
        reason_codes.append("manual_interference_rebase_active")
    return [
        {
            "schema_version": REBOUND_SNIPE_SIGNAL_SCHEMA_VERSION,
            "event_id": event_id,
            ...
            "sleeve_group": candidate.get("sleeve_group"),
            "sleeve_role": candidate.get("sleeve_role"),
            "side": candidate.get("side"),
            "market_id": candidate.get("market_id") or plan.get("market_id"),
            "outcome_id": candidate.get("outcome_id"),
            "market_token_id": candidate.get("market_token_id"),
            "current_price": price,
            "max_price": price,
            "requested_notional_usd": round(requested_notional, 6),
            "requested_shares": round(shares, 6),
            "confidence": round(min(max(float(classification.get("confidence") or 0.0), 0.55), 0.88), 3),
            "manual_rebase_status": manual_rebase,
            "ml_confidence": _closest_ml_confidence(candidate, ml_confidence_by_sleeve),
            "lifecycle_policy": {
                "required": True,
                "target_delta_cents": 1.0 if price < 0.20 else 2.0,
                "target_policy": "rebound_snipe_ladder_target",
                "stop_policy": "catalyst_retest_or_score_gap_falsification",
                "max_loss_usd": round(requested_notional, 6),
                "rebuy_requires_fresh_review": True,
                "manual_ladder_coexistence": manual_rebase == "required",
            },
...
def _blocked_rebound_snipe(event_id: str, reason: str, scenario: str) -> dict[str, Any]:
    return {
        "schema_version": REBOUND_SNIPE_SIGNAL_SCHEMA_VERSION,
        "event_id": event_id,
        "status": "blocked",
...
def _is_rebound_snipe_context(outcome: dict[str, Any]) -> bool:
    text = _norm(
        " ".join(
            str(value or "")
            for value in (
                outcome.get("strategy_id"),
                outcome.get("strategy_family"),
                outcome.get("sleeve_id"),
                outcome.get("sleeve_role"),
            )
        )
    )
    return any(fragment in text for fragment in ("rebound", "snipe", "comeback", "tail", "ultra_low"))


def _rebound_snipe_catalyst_reason_codes(*, snapshot: dict[str, Any], candidate: dict[str, Any]) -> list[str]:
    reasons: list[str] = []
    ...
    if snapshot.get("star_ejection"):
        reasons.append("star_ejection_watch")
    ...
    price = _float(candidate.get("best_ask") or candidate.get("price") or snapshot.get("underdog_price"))
    gap = _float(candidate.get("score_gap") or snapshot.get("score_gap"))
    if price is not None and price <= 0.40 and (gap is None or abs(gap) <= 12.0):
        reasons.append("low_price_close_game_dislocation")
    return _unique(reasons)


def _rebound_snipe_requested_shares(*, plan: dict[str, Any], candidate: dict[str, Any]) -> float:
    strategy_id = _clean(candidate.get("strategy_id"))
    for strategy in plan.get("active_strategies") or []:
        if not isinstance(strategy, dict) or _clean(strategy.get("strategy_id")) != strategy_id:
            continue
        entry_rules = strategy.get("entry_rules") if isinstance(strategy.get("entry_rules"), dict) else {}
        return _first_float(entry_rules, ("size", "requested_shares", "min_size")) or 5.0
    return 5.0


def _manual_rebase_status(*, market_state: dict[str, Any], portfolio_state: dict[str, Any]) -> str:
    for values in (market_state, portfolio_state):
        rebase = values.get("manual_interference_rebase") if isinstance(values.get("manual_interference_rebase"), dict) else {}
        status = _clean(rebase.get("status"))
        if status:
            return status
    return "not_required"
...
__all__ = [
    ...
    "REBOUND_SNIPE_SIGNAL_SCHEMA_VERSION",
    "SCHEMA_VERSION",
