...
QUARTER_REVISION_REVIEW_SCHEMA_VERSION = "strategy_plan_quarter_revision_review_v1"
...
def build_llm_revision_request(
    *,
    ...
    return LLMRevisionRequest(
        ...
        db_context_sources=_db_context_sources_from_runtime_context(
            event_context=dict(event_context or {}),
            current_plan=plan,
        ),
        db_stat_context_trace=_db_stat_context_trace_from_runtime_context(
            event_context=dict(event_context or {}),
            current_plan=plan,
        ),
    )
...
def persist_llm_runtime_trace(
    ...
    *,
    ...
    quarter_revision_reviews = _build_quarter_revision_review_records(
        trace,
        session_date=resolved_day,
        trace_artifact_path=str(path),
        persisted_at_utc=now.isoformat(),
    )
    quarter_revision_artifacts = _persist_quarter_revision_review_records(
        ...
        "quarter_revision_reviews": [
            {
                ...
                "reviewed_sleeve_count": item.get("reviewed_sleeve_count"),
                "path": artifact.get("path"),
            }
            for item, artifact in zip(quarter_revision_reviews, quarter_revision_artifacts, strict=False)
        ],
        "quarter_revision_review_artifacts": quarter_revision_artifacts,
    ...
    return {
        ...
        "selected_model": trace.model_routing.selected_model,
        "quarter_revision_review_count": len(quarter_revision_artifacts),
        "quarter_revision_review_artifacts": quarter_revision_artifacts,
        "persisted_at_utc": now.isoformat(),
    }
...
def _build_llm_pre_dispatch_guard(
    ...
    *,
    ...
    manual_mini_override = _manual_interference_mini_override_allowed(trace)
    metadata = {
        "llm_runtime_safety_controls": build_llm_runtime_safety_controls_status(),
    ...
    if budget["hard_block"]:
        if manual_mini_override:
            metadata["llm_manual_interference_budget_override"] = {
                "schema_version": "manual_interference_budget_override_v1",
                "applied": True,
                "reason": "fixed_min_share_manual_interference_requires_mini_rebase",
                "selected_model": trace.model_routing.selected_model,
    ...
    if capped["hard_block"]:
        if manual_mini_override:
            metadata["llm_manual_interference_cap_override"] = {
                "schema_version": "manual_interference_cap_override_v1",
                "applied": True,
                "reason": "fixed_min_share_manual_interference_requires_mini_rebase",
                "selected_model": trace.model_routing.selected_model,
                "selected_tier": trace.model_routing.selected_tier,
...
def _manual_interference_mini_override_allowed(trace: LLMRuntimeTrace) -> bool:
    if trace.model_routing.selected_tier != "mini":
        return False
    manual_trigger_types = {"manual_operator_order", "manual_operator_trade", "manual_operator_position"}
    trigger_types = {str(trigger.trigger_type) for trigger in trace.triggers if trigger.requires_revision}
    if not trigger_types.intersection(manual_trigger_types):
        request = trace.revision_request
        portfolio = request.portfolio_state if request is not None else {}
        rebase = portfolio.get("manual_interference_rebase") if isinstance(portfolio.get("manual_interference_rebase"), dict) else {}
        if rebase.get("status") != "required":
            return False
    else:
        request = trace.revision_request
        portfolio = request.portfolio_state if request is not None else {}
    policy = portfolio.get("operator_sizing_policy") if isinstance(portfolio.get("operator_sizing_policy"), dict) else {}
    mode = str(policy.get("mode") or "").strip().lower()
    if mode not in {"fixed_min_shares", "fixed_min_size", "minimum_shares"}:
        return False
    min_size = _safe_float(policy.get("min_size") or policy.get("minimum_size"))
    return min_size is not None and min_size <= 5.0
...
def _status_from_artifact_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "quarter_revision_review_count": len(payload.get("quarter_revision_reviews") or []),
        "quarter_revision_reviews": payload.get("quarter_revision_reviews") or [],
        "quarter_revision_review_artifacts": payload.get("quarter_revision_review_artifacts") or [],
        "adoption_status": adoption["status"],
...
def _build_quarter_revision_review_records(
    trace: LLMRuntimeTrace,
    *,
    session_date: str,
    ...
    for trigger in trace.triggers:
        if trigger.trigger_type != "quarter_end":
            continue
        period = _trigger_period(trigger.evidence)
        decisions = _quarter_revision_decision_summary(
            request.current_plan,
            response.revised_strategy_plan if response and isinstance(response.revised_strategy_plan, dict) else None,
            response=response,
        )
        reviewed_sleeves = _quarter_revision_reviewed_sleeves(
            request,
            decisions=decisions,
            response=response,
        )
        records.append(
            {
                "schema_version": QUARTER_REVISION_REVIEW_SCHEMA_VERSION,
                "artifact_kind": "quarter_bound_strategy_plan_revision_review",
                ...
                "period": period,
                "quarter_label": _quarter_revision_label(period),
                "trigger_requires_revision": trigger.requires_revision,
                "trigger_evidence": trigger.evidence,
                "selected_model": trace.model_routing.selected_model,
                "selected_tier": trace.model_routing.selected_tier,
                "response_status": response.status if response else None,
                "skipped_reason": response.skipped_reason if response else None,
                "review_status": _quarter_revision_review_status(trigger=trigger, response=response),
                "revision_available": bool(response and isinstance(response.revised_strategy_plan, dict)),
                "current_plan_stale_reason": request.current_plan_stale_reason,
                "current_plan_generated_at_utc": request.current_plan.get("generated_at_utc"),
                "current_plan_strategy_count": len(_strategy_rows_for_revision_review(request.current_plan)),
                "reviewed_sleeve_count": len(reviewed_sleeves),
                "reviewed_sleeves": reviewed_sleeves,
                "revision_decisions": decisions,
                "reconciliation_actions": response.reconciliation_actions if response else [],
                "blocked_actions": response.blocked_actions if response else [],
                "scoreboard_pbp_summary": request.scoreboard_pbp_summary,
                "operator_sizing_policy": request.operator_sizing_policy,
                "direct_clob_truth_status": _quarter_revision_direct_truth_status(request.direct_clob_truth),
                "order_endpoint_call_allowed": False,
...
def _persist_quarter_revision_review_records(
    records: list[dict[str, Any]],
    *,
    day_root: Path,
    timestamp: str,
    ...
    for index, record in enumerate(records, start=1):
        event_id = str(record.get("event_id") or "event")
        period = record.get("period")
        period_label = f"q{period}" if period is not None else f"idx{index}"
        event_root = day_root / "quarter-revision-reviews" / _safe_filename(event_id)
        event_root.mkdir(parents=True, exist_ok=True)
        path = event_root / f"quarter_revision_{timestamp}_{period_label}_{trace_id[:8]}.json"
        record_with_path = {**record, "path": str(path)}
        _write_json(path, record_with_path)
        event_summary = {
            "schema_version": QUARTER_REVISION_REVIEW_SCHEMA_VERSION,
            "event_id": event_id,
            "trace_id": trace_id,
            ...
            "reviewed_sleeve_count": record.get("reviewed_sleeve_count"),
            "revision_available": record.get("revision_available"),
            "path": str(path),
            "persisted_at_utc": record.get("persisted_at_utc"),
        }
        _append_jsonl(day_root / "quarter_revision_reviews.jsonl", event_summary)
        _append_jsonl(event_root / "history.jsonl", event_summary)
        artifacts.append(event_summary)
    return artifacts


def _quarter_revision_reviewed_sleeves(
    request: LLMRevisionRequest,
    *,
    decisions: dict[str, Any],
    response: LLMRevisionResponse | None,
) -> list[dict[str, Any]]:
    decision_by_id: dict[str, str] = {
        str(item.get("strategy_id")): str(item.get("decision"))
        for item in decisions.get("strategy_decision_rows", [])
        if isinstance(item, dict) and item.get("strategy_id")
    }
    runtime_by_id = _strategy_runtime_decision_index(request.strategy_decisions)
    rows: list[dict[str, Any]] = []
    for strategy in _strategy_rows_for_revision_review(request.current_plan):
        strategy_id = str(strategy.get("strategy_id") or strategy.get("sleeve_id") or "")
        runtime = runtime_by_id.get(strategy_id, {})
        rows.append(
            {
                ...
                "status": strategy.get("status") or strategy.get("state") or "active",
                "shadow_only": _strategy_is_shadow_only(strategy),
                "actual_result": _runtime_result_payload(runtime, "actual"),
                "shadow_result": _runtime_result_payload(runtime, "shadow"),
                "decision": decision_by_id.get(strategy_id)
                or ("review_pending" if response is None or response.skipped_reason else "keep"),
                "decision_source": "llm_revision_response" if response and not response.skipped_reason else "runtime_trace",
            }
        )
    for item in decisions.get("strategy_decision_rows", []):
        if not isinstance(item, dict) or item.get("decision") != "add":
            continue
        rows.append(
            {
                ...
                "status": "proposed",
                "shadow_only": False,
                "actual_result": {"status": "not_applicable_new_strategy"},
                "shadow_result": {"status": "not_applicable_new_strategy"},
                "decision": "add",
                "decision_source": "llm_revision_response",
            }
        )
    return rows


def _quarter_revision_decision_summary(
    current_plan: dict[str, Any],
    revised_plan: dict[str, Any] | None,
    *,
    response: LLMRevisionResponse | None,
) -> dict[str, Any]:
    before = _strategy_map_for_revision_review(current_plan)
    after = _strategy_map_for_revision_review(revised_plan or {})
    rows: list[dict[str, Any]] = []
    if revised_plan:
        for strategy_id in sorted(before):
            strategy = before[strategy_id]
            if strategy_id not in after:
                rows.append(_strategy_decision_row(strategy, decision="remove"))
            elif _canonical_payload(strategy) != _canonical_payload(after[strategy_id]):
                rows.append(_strategy_decision_row(after[strategy_id], decision="modify"))
            else:
                rows.append(_strategy_decision_row(strategy, decision="keep"))
        for strategy_id in sorted(set(after) - set(before)):
            rows.append(_strategy_decision_row(after[strategy_id], decision="add"))
    else:
        fallback_decision = "review_pending" if response is None or response.skipped_reason else "keep"
        rows.extend(_strategy_decision_row(strategy, decision=fallback_decision) for strategy in before.values())
    return {
        "schema_version": "strategy_plan_quarter_revision_decisions_v1",
        "decision_source": "llm_revised_strategy_plan" if revised_plan else "llm_trace_without_revised_plan",
        "revision_available": bool(revised_plan),
        "added_strategy_ids": [str(item.get("strategy_id")) for item in rows if item.get("decision") == "add" and item.get("strategy_id")],
        "removed_strategy_ids": [str(item.get("strategy_id")) for item in rows if item.get("decision") == "remove" and item.get("strategy_id")],
        "changed_strategy_ids": [str(item.get("strategy_id")) for item in rows if item.get("decision") == "modify" and item.get("strategy_id")],
        "kept_strategy_ids": [str(item.get("strategy_id")) for item in rows if item.get("decision") == "keep" and item.get("strategy_id")],
        "pending_strategy_ids": [str(item.get("strategy_id")) for item in rows if item.get("decision") == "review_pending" and item.get("strategy_id")],
        "strategy_decision_rows": rows,
    }


def _strategy_decision_row(strategy: dict[str, Any], *, decision: str) -> dict[str, Any]:
    return {
        "strategy_id": strategy.get("strategy_id"),
        "sleeve_id": strategy.get("sleeve_id"),
        "family": strategy.get("family"),
...
def _strategy_map_for_revision_review(plan: dict[str, Any]) -> dict[str, dict[str, Any]]:
    rows = _strategy_rows_for_revision_review(plan)
    result: dict[str, dict[str, Any]] = {}
    for index, row in enumerate(rows):
        strategy_id = str(row.get("strategy_id") or row.get("sleeve_id") or f"strategy-{index}").strip()
        if strategy_id:
            result[strategy_id] = row
    return result


def _strategy_rows_for_revision_review(plan: dict[str, Any]) -> list[dict[str, Any]]:
    if not isinstance(plan, dict):
        return []
    rows = [dict(item) for item in plan.get("active_strategies") or [] if isinstance(item, dict)]
    rows.extend(dict(item) for item in plan.get("shadow_strategies") or [] if isinstance(item, dict))
    return rows


def _strategy_runtime_decision_index(rows: list[dict[str, Any]]) -> dict[str, dict[str, Any]]:
    result: dict[str, dict[str, Any]] = {}
    for row in rows:
        if not isinstance(row, dict):
            continue
        for key in ("strategy_id", "sleeve_id"):
...
def _runtime_result_payload(row: dict[str, Any], prefix: str) -> dict[str, Any]:
    keys = (
        f"{prefix}_result",
...
def _strategy_is_shadow_only(strategy: dict[str, Any]) -> bool:
    flags = strategy.get("shadow_flags") if isinstance(strategy.get("shadow_flags"), dict) else {}
    return _truthy(flags.get("shadow_only") or strategy.get("shadow_only") or strategy.get("shadow"))


def _quarter_revision_review_status(*, trigger: LLMRuntimeTrigger, response: LLMRevisionResponse | None) -> str:
    if response is None:
        return "detected_no_response"
    if not trigger.requires_revision and _truthy(trigger.evidence.get("reviewed_by_current_plan")):
        return "reviewed_by_current_plan"
    if isinstance(response.revised_strategy_plan, dict):
        return "revision_recorded"
    if response.skipped_reason:
        return "review_pending_unavailable"
    if response.status == "response_recorded":
        return "reviewed_no_plan_change"
    return "review_pending"


def _quarter_revision_label(period: int | None) -> str | None:
    labels = {1: "q1_end", 2: "halftime_q2_end", 3: "q3_end", 4: "q4_end"}
    if period is None:
        return None
    return labels.get(period, f"period_{period}_end")


def _quarter_revision_direct_truth_status(direct_clob_truth: dict[str, Any]) -> dict[str, Any]:
    inventory = direct_clob_truth.get("current_event_inventory_proof")
    if isinstance(inventory, dict):
        return {
            "schema_version": "quarter_revision_direct_truth_status_v1",
            "status": inventory.get("status"),
            "event_scoped": inventory.get("event_scoped"),
            "unresolved_inventory_present": inventory.get("unresolved_inventory_present"),
            "unresolved_inventory_count": inventory.get("unresolved_inventory_count"),
        }
    return {
        "schema_version": "quarter_revision_direct_truth_status_v1",
        "status": "missing_direct_clob_truth",
        "event_scoped": False,
        "unresolved_inventory_present": False,
        "unresolved_inventory_count": 0,
    }
...
def _db_context_sources_from_runtime_context(
    *,
    event_context: dict[str, Any],
    current_plan: dict[str, Any],
) -> list[dict[str, Any]]:
    trace = event_context.get("db_stat_context_trace")
    if isinstance(trace, dict) and isinstance(trace.get("db_context_sources"), list):
        return [dict(item) for item in trace["db_context_sources"] if isinstance(item, dict)]
    sources = event_context.get("db_context_sources")
    if isinstance(sources, list):
        return [dict(item) for item in sources if isinstance(item, dict)]
    for section_name in ("context_summary", "explainability"):
        section = current_plan.get(section_name)
        if not isinstance(section, dict):
            continue
        for key in ("db_context_sources", "db_stat_context_sources", "team_player_stat_context_sources"):
            section_sources = section.get(key)
            if isinstance(section_sources, list):
                return [dict(item) for item in section_sources if isinstance(item, dict)]
    return []


def _db_stat_context_trace_from_runtime_context(
    *,
    event_context: dict[str, Any],
    current_plan: dict[str, Any],
) -> dict[str, Any]:
    ...
    if isinstance(trace, dict):
        return dict(trace)
    sources = _db_context_sources_from_runtime_context(event_context=event_context, current_plan=current_plan)
    return {
        "schema_version": "db_stat_context_trace_v1",
        "db_context_sources": sources,
        "source_count": len(sources),
        "included_source_count": sum(1 for source in sources if source.get("included_in_llm_context") is True),
