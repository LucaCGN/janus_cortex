...
PremiseState = Literal["confirmed", "intact", "sideways_valid", "weakening", "invalidated", "unreviewed", "unknown"]
HygieneBucket = Literal[
    "targeted",
    "needs_target",
    "loss_recovery_scalp_candidate",
    "replace_candidate",
    "worker_allocated",
    "unknown",
]
PriceBucket = Literal["sub_2c", "2c_to_5c", "5c_to_15c", "15c_to_40c", "40c_to_75c", "over_75c", "unknown"]
...
DEFAULT_MIN_BUY_NOTIONAL_USD = Decimal("1")
...
DEFAULT_HIGH_VOLUME_USD = Decimal("100000")
DEFAULT_LOW_PRICE_MAX = Decimal("0.40")
DEFAULT_LOW_PRICE_STRONG_MAX = Decimal("0.15")
DEFAULT_GRID_1D_RANGE_THRESHOLD_CENTS = Decimal("5")
DEFAULT_GRID_7D_RANGE_THRESHOLD_CENTS = Decimal("10")
DEFAULT_GRID_30D_RANGE_THRESHOLD_CENTS = Decimal("15")
...
class GlobalPortfolioManagedSlot(BaseModel):
    ...
    premise_state: PremiseState = "unreviewed"
    hygiene_bucket: HygieneBucket = "unknown"
    market_volume_usd: float | None = None
    volume_24h_usd: float | None = None
    price_bucket: PriceBucket = "unknown"
    cashout_score: int = 0
    price_memory: dict[str, Any] = Field(default_factory=dict)
    worker_allocation: dict[str, Any] = Field(default_factory=dict)
...
class GlobalPortfolioBudgetSnapshot(BaseModel):
    ...
    base_sleeve_cap_usd: float = 50.0
    realized_global_pnl_usd: float = 0.0
    dynamic_sleeve_cap_usd: float = 50.0
    ...
    budget_source_confidence: str = "account_confirmed_or_zero"
    budget_source_caveats: list[str] = Field(default_factory=list)
...
class GlobalPortfolioCandidate(BaseModel):
    ...
    price_history_features: dict[str, Any] = Field(default_factory=dict)
    grid_backtest_summary: dict[str, Any] = Field(default_factory=dict)
...
class GlobalPortfolioGridEligibilityReview(BaseModel):
    ...
    market_slug: str | None = None
    token_id: str | None = None
    validator_profile: str = "legacy_30d"
    premise_state: PremiseState = "unreviewed"
    thirty_day_range_percent: float | None = None
    one_day_range_cents: float | None = None
    seven_day_range_cents: float | None = None
    thirty_day_range_cents: float | None = None
    ...
    one_day_backtest_positive: bool | None = None
    three_day_backtest_positive: bool | None = None
    seven_day_backtest_positive: bool | None = None
    recommended_worker_duration_hours: int | None = None
    ...
    def _summarize_grid_review(self) -> "GlobalPortfolioGridEligibilityReview":
        blockers: list[str] = []
        v2_window_inputs_present = any(
            value is not None
            for value in (
                self.one_day_range_cents,
                self.seven_day_range_cents,
                self.thirty_day_range_cents,
                self.one_day_backtest_positive,
                self.three_day_backtest_positive,
                self.seven_day_backtest_positive,
            )
        ) or bool(self.backtest_json)
        if v2_window_inputs_present:
            qualified_profiles: list[tuple[str, int]] = []
            if (
                (_decimal_value(self.one_day_range_cents) or Decimal("0")) >= DEFAULT_GRID_1D_RANGE_THRESHOLD_CENTS
                and self.one_day_backtest_positive is True
            ):
                qualified_profiles.append(("1d_5c_sideways_positive_backtest", 24))
            if (
                (_decimal_value(self.seven_day_range_cents) or Decimal("0")) >= DEFAULT_GRID_7D_RANGE_THRESHOLD_CENTS
                and self.three_day_backtest_positive is True
            ):
                qualified_profiles.append(("3d_10c_weekly_sideways_positive_backtest", 72))
            if (
                (_decimal_value(self.thirty_day_range_cents) or Decimal("0"))
                >= DEFAULT_GRID_30D_RANGE_THRESHOLD_CENTS
                and self.seven_day_backtest_positive is True
            ):
                qualified_profiles.append(("7d_15c_monthly_sideways_positive_backtest", 168))
            if qualified_profiles:
                self.validator_profile = qualified_profiles[0][0]
                if self.recommended_worker_duration_hours is None:
                    self.recommended_worker_duration_hours = qualified_profiles[0][1]
            else:
                blockers.append("grid_window_backtest_not_qualified")
            if self.days_to_resolution is not None and self.days_to_resolution < 1:
                blockers.append("resolution_window_below_1_day")
        else:
        ...
        if self.premise_state == "invalidated":
            blockers.append("premise_invalidated")
...
def build_global_portfolio_dynamic_budget(
    direct_truth_snapshot: dict[str, Any],
    *,
    base_sleeve_cap_usd: Decimal | float | str = DEFAULT_CODEX_SLEEVE_CAP_USD,
) -> dict[str, Any]:
    """Compute the global-portfolio budget from confirmed realized PnL only."""

    base_cap = _decimal_value(base_sleeve_cap_usd) or DEFAULT_CODEX_SLEEVE_CAP_USD
    realized = Decimal("0")
    caveats: list[str] = []
    source_count = 0
    excluded_count = 0
    skipped_unconfirmed_count = 0
    skipped_unrealized_count = 0

    explicit = _decimal_value(
        direct_truth_snapshot.get("realized_global_pnl_usd")
        or direct_truth_snapshot.get("global_portfolio_realized_pnl_usd")
    )
    if explicit is not None:
        confidence = str(
            direct_truth_snapshot.get("realized_global_pnl_source_confidence")
            or direct_truth_snapshot.get("global_portfolio_realized_pnl_source_confidence")
            or "account_confirmed"
        ).strip().lower()
        if _is_confirmed_realized_pnl_source(confidence):
            realized += explicit
            source_count += 1
        else:
            caveats.append(f"explicit_realized_global_pnl_skipped_unconfirmed:{confidence or 'unknown'}")
            skipped_unconfirmed_count += 1

    for section_name, default_confirmed in (
        ("closed_positions", True),
        ("closed_position_rows", True),
        ("resolved_positions", True),
        ("settled_positions", True),
        ("realized_pnl_rows", False),
        ("pnl_attribution", False),
    ):
        rows = direct_truth_snapshot.get(section_name)
        if isinstance(rows, dict):
            rows = rows.get("items") or rows.get("rows") or rows.get("positions")
        if not isinstance(rows, list):
            continue
        for index, row in enumerate(rows, start=1):
            if not isinstance(row, dict):
                caveats.append(f"{section_name}_row_{index}_not_object")
                continue
            if _is_covered_basketball_row(row) or _is_external_research_owned_row(row):
                excluded_count += 1
                continue
            pnl = _realized_pnl_value(row)
            if pnl is None:
                if _row_has_unrealized_pnl(row):
                    skipped_unrealized_count += 1
                else:
                    caveats.append(f"{section_name}_row_{index}_missing_realized_pnl")
                continue
            if not _row_realized_pnl_confirmed(row, default_confirmed=default_confirmed):
                skipped_unconfirmed_count += 1
                continue
            realized += pnl
            source_count += 1

    dynamic_cap = max(Decimal("0"), base_cap + realized)
    if source_count == 0 and realized == Decimal("0"):
        confidence = "account_confirmed_or_zero"
    elif skipped_unconfirmed_count or skipped_unrealized_count:
        confidence = "account_confirmed_with_caveats"
    else:
        confidence = "account_confirmed"
    if excluded_count:
        caveats.append(f"excluded_non_global_realized_pnl_rows:{excluded_count}")
    if skipped_unconfirmed_count:
        caveats.append(f"skipped_unconfirmed_realized_pnl_rows:{skipped_unconfirmed_count}")
    if skipped_unrealized_count:
        caveats.append(f"ignored_unrealized_pnl_rows:{skipped_unrealized_count}")
    return {
        "base_sleeve_cap_usd": base_cap,
        "realized_global_pnl_usd": realized,
        "dynamic_sleeve_cap_usd": dynamic_cap,
        "budget_source_confidence": confidence,
        "budget_source_caveats": sorted(set(caveats)),
        "realized_source_row_count": source_count,
        "excluded_realized_pnl_row_count": excluded_count,
        "skipped_unconfirmed_realized_pnl_row_count": skipped_unconfirmed_count,
        "ignored_unrealized_pnl_row_count": skipped_unrealized_count,
    }
...
def build_20_slot_board(
    ...
    *,
    ...
    inventory and externally owned research rows from the Codex global slot target.
    """
    ...
    budget_inputs = build_global_portfolio_dynamic_budget(
        direct_truth_snapshot,
        base_sleeve_cap_usd=codex_sleeve_cap_usd,
    )
    base_sleeve_cap = budget_inputs["base_sleeve_cap_usd"]
    realized_global_pnl = budget_inputs["realized_global_pnl_usd"]
    sleeve_cap = budget_inputs["dynamic_sleeve_cap_usd"]
    ...
    caveats.extend(str(item) for item in budget_inputs["budget_source_caveats"])
    ...
    for index, position in enumerate(_portfolio_list(direct_truth_snapshot, "open_positions"), start=1):
        ...
        if _is_covered_basketball_row(position):
            ignored_rows.append({"source": "open_positions", "ignore_reason": "covered_market_excluded", "row": dict(position)})
            continue
        if _is_external_research_owned_row(position):
            ignored_rows.append(
                {"source": "open_positions", "ignore_reason": "external_research_scope_excluded", "row": dict(position)}
            )
            continue
    ...
    for index, order in enumerate(order_rows, start=1):
        ...
        if _is_covered_basketball_row(order):
            ignored_rows.append({"source": "open_orders", "ignore_reason": "covered_market_excluded", "row": dict(order)})
            continue
        if _is_external_research_owned_row(order):
            ignored_rows.append(
                {"source": "open_orders", "ignore_reason": "external_research_scope_excluded", "row": dict(order)}
            )
            continue
    ...
    budget = GlobalPortfolioBudgetSnapshot(
        ...
        base_sleeve_cap_usd=_decimal_to_float(base_sleeve_cap) or 0.0,
        realized_global_pnl_usd=_decimal_to_float(realized_global_pnl) or 0.0,
        dynamic_sleeve_cap_usd=_decimal_to_float(sleeve_cap) or 0.0,
        ...
        budget_source_confidence=str(budget_inputs["budget_source_confidence"]),
        budget_source_caveats=[str(item) for item in budget_inputs["budget_source_caveats"]],
    )
...
def score_portfolio_candidates(
    ...
    *,
    ...
    for slot in board.slots:
        ...
        if slot.token_id:
            existing_tags.add(slot.token_id)
    candidates: list[GlobalPortfolioCandidate] = []
    reserved_candidate_tags = set(existing_tags)
    for index, row in enumerate(candidate_rows, start=1):
        if not isinstance(row, dict):
            continue
        candidate = _score_candidate_row(
            row,
            board=board,
            existing_tags=reserved_candidate_tags,
            index=index,
            generated_at_utc=generated_at,
        )
        candidates.append(candidate)
        if candidate.status == "ready_for_order_proof":
            reserved_candidate_tags.update(_concentration_tags(row))
            if candidate.market_slug:
                reserved_candidate_tags.add(candidate.market_slug)
...
def build_grid_eligibility_review(
    *,
    ...
    token_id: str | None = None,
    validator_profile: str = "legacy_30d",
    premise_state: str | None = None,
    thirty_day_range_percent: Decimal | float | str | None = None,
    one_day_range_cents: Decimal | float | str | None = None,
    seven_day_range_cents: Decimal | float | str | None = None,
    thirty_day_range_cents: Decimal | float | str | None = None,
    days_to_resolution: int | None = None,
    stable_thesis: bool = False,
    ...
    seven_day_backtest_positive: bool | None = None,
    recommended_worker_duration_hours: int | None = None,
    backtest_json: dict[str, Any] | None = None,
    ...
    return GlobalPortfolioGridEligibilityReview(
        ...
        premise_state=_premise_state({"premise_state": premise_state}),
        thirty_day_range_percent=_decimal_to_float(_decimal_value(thirty_day_range_percent)),
        one_day_range_cents=_decimal_to_float(_decimal_value(one_day_range_cents)),
        seven_day_range_cents=_decimal_to_float(_decimal_value(seven_day_range_cents)),
        thirty_day_range_cents=_decimal_to_float(_decimal_value(thirty_day_range_cents)),
        ...
        one_day_backtest_positive=one_day_backtest_positive,
        three_day_backtest_positive=three_day_backtest_positive,
        seven_day_backtest_positive=seven_day_backtest_positive,
        recommended_worker_duration_hours=recommended_worker_duration_hours,
        backtest_json=dict(backtest_json or {}),
...
def build_deep_pass_plan(
    ...
    *,
    ...
    enriched_candidate_rows = _attach_price_memory_to_candidate_rows(candidate_rows or [], direct_truth_snapshot)
    queue = score_portfolio_candidates(enriched_candidate_rows, board, generated_at_utc=generated_at)
    ...
    if board.empty_slot_count > 0 and selected is not None:
        ...
        if not enriched_candidate_rows:
...
def _realized_pnl_value(row: dict[str, Any]) -> Decimal | None:
    for key in (
        "realized_global_pnl_usd",
        "realized_pnl_usd",
        "realizedPnl",
        "realized_pnl",
        "closed_position_realized_pnl_usd",
        "pnl_usd",
        "pnl",
        "net_pnl_usd",
    ):
    ...
    if isinstance(account_pnl, dict):
        for key in ("realized_pnl_usd", "known_cashflow_usd", "net_pnl_usd"):
            value = _decimal_value(account_pnl.get(key))
            if value is not None:
                return value
    return None


def _row_has_unrealized_pnl(row: dict[str, Any]) -> bool:
    for key in (
        "unrealized_pnl_usd",
        "unrealizedPnl",
        "unrealized_pnl",
...
def _row_realized_pnl_confirmed(row: dict[str, Any], *, default_confirmed: bool) -> bool:
    confidence = str(
        row.get("source_confidence")
        or row.get("pnl_source_confidence")
        or row.get("realized_pnl_source_confidence")
        or row.get("source")
        or ""
    ).strip().lower()
    if confidence:
        return _is_confirmed_realized_pnl_source(confidence)
    return default_confirmed


def _is_confirmed_realized_pnl_source(source: str) -> bool:
    normalized = re.sub(r"[^a-z0-9]+", "_", source.lower()).strip("_")
    return normalized in {
        "account",
        "account_confirmed",
        "account_confirmed_closed_position",
        "account_closed_position",
        "closed_position",
        "closed_positions",
        "db_confirmed",
        "settled",
...
def _managed_slot_from_position(
    ...
    *,
    ...
    return GlobalPortfolioManagedSlot(
        ...
        premise_state=_premise_state(position),
        hygiene_bucket=_hygiene_bucket(position),
        market_volume_usd=_decimal_to_float(_market_volume_usd(position)),
        volume_24h_usd=_decimal_to_float(_volume_24h_usd(position)),
        price_bucket=_price_bucket(_decimal_value(position.get("current_price") or position.get("price"))),
        cashout_score=_cashout_score(position),
        price_memory=_price_history_features(position),
        worker_allocation=_worker_allocation(position),
...
def _managed_slot_from_order(
    ...
    *,
    ...
    return GlobalPortfolioManagedSlot(
        ...
        premise_state=_premise_state(order),
        hygiene_bucket=_hygiene_bucket(order, default="needs_target"),
        market_volume_usd=_decimal_to_float(_market_volume_usd(order)),
        volume_24h_usd=_decimal_to_float(_volume_24h_usd(order)),
        price_bucket=_price_bucket(_decimal_value(order.get("price") or order.get("limit_price"))),
        cashout_score=_cashout_score(order),
        price_memory=_price_history_features(order),
        worker_allocation=_worker_allocation(order),
...
def _portfolio_manager_order_rows(direct_truth_snapshot: dict[str, Any]) -> list[Any]:
    rows: list[Any] = []
    seen: set[str] = set()
    direct_open_order_ids = {
        external_id
        for item in _portfolio_list(direct_truth_snapshot, "open_orders")
        if isinstance(item, dict)
        for external_id in (_order_external_id(item),)
        if external_id
    }
    for key in (
        "open_orders",
        ...
        for item in _portfolio_list(direct_truth_snapshot, key):
            if not isinstance(item, dict):
                rows.append(item)
                continue
            external_id = _order_external_id(item)
            if key != "open_orders" and direct_open_order_ids and external_id and external_id not in direct_open_order_ids:
                continue
            identity = "|".join(
...
def _order_external_id(order: dict[str, Any]) -> str | None:
    value = order.get("external_order_id") or order.get("externalOrderId") or order.get("orderID") or order.get("id")
...
def _premise_state(row: dict[str, Any]) -> PremiseState:
    raw = _portfolio_first_text(row, ("premise_state", "thesis_state", "premise_status", "thesis_status"))
    ...
    aliases = {
        ...
        "holds": "intact",
        "strong": "confirmed",
        "sideways": "sideways_valid",
        "sideways_movement": "sideways_valid",
    ...
    if normalized in {"confirmed", "intact", "sideways_valid", "weakening", "invalidated", "unreviewed", "unknown"}:
        return normalized  # type: ignore[return-value]
    if row.get("premise_valid") is False or row.get("thesis_valid") is False:
        return "invalidated"
    return "unreviewed"


def _hygiene_bucket(row: dict[str, Any], *, default: HygieneBucket = "unknown") -> HygieneBucket:
    raw = _portfolio_first_text(row, ("hygiene_bucket", "slot_hygiene_bucket", "management_bucket"))
    normalized = str(raw or "").strip().lower().replace(" ", "_").replace("-", "_")
    if normalized in {
        "targeted",
        "needs_target",
        ...
        return normalized  # type: ignore[return-value]
    if _dict_value(row.get("worker_allocation") or row.get("worker_allocation_json")):
        return "worker_allocated"
    if not _dict_value(row.get("target_stop_rebuy") or row.get("target_policy")) and default == "unknown":
        return "needs_target"
    return default


def _price_history_features(row: dict[str, Any]) -> dict[str, Any]:
    for key in ("price_history_features", "price_memory", "price_memory_json", "feature_snapshot", "features"):
        value = _dict_value(row.get(key))
        if value:
            return value
    return {}


def _grid_backtest_summary(row: dict[str, Any]) -> dict[str, Any]:
    for key in ("grid_backtest_summary", "grid_backtest", "backtest_summary", "backtest_json"):
        value = _dict_value(row.get(key))
        if value:
            return value
    return {}
...
def _worker_allocation(row: dict[str, Any]) -> dict[str, Any]:
    for key in ("worker_allocation", "worker_allocation_json", "strategy_worker", "grid_worker"):
        value = _dict_value(row.get(key))
        if value:
            return value
    return {}
...
def _market_volume_usd(row: dict[str, Any], orderbook: dict[str, Any] | None = None) -> Decimal | None:
    return _first_decimal(
        row,
        (
            "market_volume_usd",
        ...
        nested=orderbook or _dict_value(row.get("direct_orderbook") or row.get("orderbook")),
    )


def _volume_24h_usd(row: dict[str, Any], orderbook: dict[str, Any] | None = None) -> Decimal | None:
    return _first_decimal(
        row,
        (
            "volume_24h_usd",
...
def _liquidity_usd(row: dict[str, Any], orderbook: dict[str, Any]) -> Decimal | None:
    explicit = _first_decimal(
        row,
        ("liquidity_usd", "liquidity", "market_liquidity_usd", "cashout_depth_usd"),
        nested=orderbook,
    )
    return explicit if explicit is not None else _liquidity_capacity_usd(row, orderbook)


def _price_bucket(price: Decimal | None) -> PriceBucket:
    if price is None or price <= 0:
        return "unknown"
    if price < Decimal("0.02"):
        return "sub_2c"
...
def _low_price_score(price: Decimal | None, *, spread_cents: Decimal | None = None) -> int:
    ...
    score = scores[bucket]
    if spread_cents is not None and spread_cents > DEFAULT_GRID_MAX_SPREAD_CENTS:
        score = int(score / 2)
    return score


def _cashout_score(row: dict[str, Any], orderbook: dict[str, Any] | None = None) -> int:
    book = orderbook or _dict_value(row.get("direct_orderbook") or row.get("orderbook"))
    market_volume = _market_volume_usd(row, book)
    volume_24h = _volume_24h_usd(row, book)
    liquidity = _liquidity_usd(row, book)
    spread = _first_decimal(row, ("spread_cents", "spread"), nested=book)
    score = 0
    best_volume = max(value for value in (market_volume, volume_24h, Decimal("0")) if value is not None)
    if best_volume >= DEFAULT_HIGH_VOLUME_USD:
        score += 20
    elif best_volume >= Decimal("50000"):
        score += 12
    elif best_volume >= Decimal("10000"):
        score += 6
    if liquidity is not None:
        if liquidity >= DEFAULT_SCALABLE_DEPTH_USD:
            score += 10
        elif liquidity >= DEFAULT_GRID_MIN_DEPTH_USD:
            score += 5
    if spread is not None:
        if spread <= Decimal("1"):
            score += 6
        elif spread <= DEFAULT_GRID_MAX_SPREAD_CENTS:
            score += 3
        else:
            score -= 6
    return max(0, min(36, score))


def _grid_backtest_score(summary: dict[str, Any]) -> int:
    if not summary:
        return 0
    status = str(summary.get("status") or summary.get("result") or "").lower()
    if status in {"positive_after_costs", "profitable", "grid_profitable", "sideways_grid_qualified"}:
        return 15
    if bool(summary.get("profitable")) or bool(summary.get("positive_after_costs")):
        return 15
    if status in {"not_profitable", "negative_after_costs", "blocked"}:
        return -10
    return 3
...
def _is_external_research_owned_row(row: dict[str, Any]) -> bool:
    raw = _portfolio_row_search_text(row)
    ownership_tokens = {
        "#47",
        "issue47",
        "issue-47",
        "crypto-options-research",
        "future-domain-research-agent",
        "run_crypto_options_live_micro_executor",
        "run_crypto_options_profile_signal_monitor",
    }
    return any(token in raw for token in ownership_tokens) or _is_crypto_options_research_row(row)


def _is_crypto_options_research_row(row: dict[str, Any]) -> bool:
    raw = _portfolio_row_search_text(row)
    compact = re.sub(r"[\s_/-]+", "", raw)
    crypto_terms = ("bitcoin", "btc", "ethereum", "eth")
    updown_terms = ("up or down", "up/down", "updown")
    timed_window = bool(re.search(r"\b\d{1,2}:\d{2}\s*(?:am|pm)?\s*[-–]\s*\d{1,2}:\d{2}", raw))
    slug_5m = bool(re.search(r"\b(?:btc|eth)[-_ ]?updown[-_ ]?5m\b", raw)) or "updown5m" in compact
    title_5m = any(term in raw for term in updown_terms) and any(term in raw for term in crypto_terms) and (
        "5m" in raw or "5-minute" in raw or timed_window
    )
    return slug_5m or title_5m


def _portfolio_row_search_text(row: dict[str, Any]) -> str:
    values: list[Any] = [
        row.get("market_title"),
        row.get("title"),
        row.get("question"),
...
def _attach_price_memory_to_candidate_rows(
    candidate_rows: list[dict[str, Any]],
    direct_truth_snapshot: dict[str, Any],
) -> list[dict[str, Any]]:
    snapshots = _price_memory_snapshots_from_direct_truth(direct_truth_snapshot)
    if not snapshots:
    ...
    for snapshot in snapshots:
        if not isinstance(snapshot, dict):
            continue
        token = _portfolio_token_id(snapshot)
        slug = _portfolio_market_slug(snapshot)
        if token:
            by_token[token] = snapshot
        if slug:
            by_slug[slug] = snapshot
    enriched: list[dict[str, Any]] = []
    for row in candidate_rows:
        item = dict(row)
        snapshot = by_token.get(_portfolio_token_id(item) or "") or by_slug.get(_portfolio_market_slug(item) or "")
        if snapshot:
            features = snapshot.get("features_json") or snapshot.get("price_memory_features")
            validation = snapshot.get("grid_scalp_validation")
            if isinstance(features, dict) and not _dict_value(item.get("price_history_features")):
                item["price_history_features"] = features
            if isinstance(validation, dict) and not _dict_value(item.get("grid_backtest_summary")):
                backtest = validation.get("grid_backtest_summary")
                if isinstance(backtest, dict):
                    item["grid_backtest_summary"] = backtest
        enriched.append(item)
    return enriched


def _price_memory_snapshots_from_direct_truth(direct_truth_snapshot: dict[str, Any]) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    collection = direct_truth_snapshot.get("price_memory_collection")
    if isinstance(collection, dict) and isinstance(collection.get("snapshots"), list):
        rows.extend(dict(item) for item in collection["snapshots"] if isinstance(item, dict))
    for key in ("price_feature_snapshots", "manager_price_feature_snapshots"):
        value = direct_truth_snapshot.get(key)
        if isinstance(value, list):
...
def _score_candidate_row(
    ...
    *,
    ...
    if proposed_side not in {"buy", "sell", "unknown"}:
        proposed_side = "buy"
    ...
    if proposed_side == "buy" and price is not None and price > 0:
        if size is None or size < DEFAULT_MIN_ORDER_SIZE:
            size = DEFAULT_MIN_ORDER_SIZE
        if proposed_notional < DEFAULT_MIN_BUY_NOTIONAL_USD:
            minimum_notional_size = DEFAULT_MIN_BUY_NOTIONAL_USD / price
            size = max(size, minimum_notional_size)
            proposed_notional = price * size
    risk_return = _risk_return_analysis(row, proposed_notional)
    orderbook = _dict_value(row.get("direct_orderbook") or row.get("orderbook"))
    market_volume = _market_volume_usd(row, orderbook)
    volume_24h = _volume_24h_usd(row, orderbook)
    liquidity_usd = _liquidity_usd(row, orderbook)
    spread_cents = _first_decimal(row, ("spread_cents", "spread"), nested=orderbook)
    price_bucket = _price_bucket(price)
    low_price_score = _low_price_score(price, spread_cents=spread_cents)
    cashout_score = _cashout_score(row, orderbook)
    price_history_features = _price_history_features(row)
    grid_backtest_summary = _grid_backtest_summary(row)
    risk_return_flags = list(risk_return["risk_return_flags"])
    if market_volume is None and volume_24h is None:
        risk_return_flags.append("volume_unmodeled")
    elif max(value for value in (market_volume, volume_24h, Decimal("0")) if value is not None) < DEFAULT_HIGH_VOLUME_USD:
        risk_return_flags.append("below_100k_volume_preference")
    rejection_reasons: list[str] = []
    if _is_covered_basketball_row(row):
        rejection_reasons.append("covered_market_excluded")
    if _is_external_research_owned_row(row):
        rejection_reasons.append("external_research_scope_excluded")
    ...
    if token_id and token_id in existing_tags:
        rejection_reasons.append("duplicate_existing_slot")
    if market_slug and market_slug in existing_tags:
        rejection_reasons.append("duplicate_existing_slot")
    ...
    end_date_rejection = _candidate_end_date_rejection_reason(row, now_utc=generated_at_utc)
    if end_date_rejection:
        rejection_reasons.append(end_date_rejection)
    ...
    if proposed_side == "buy" and proposed_notional < DEFAULT_MIN_BUY_NOTIONAL_USD:
        rejection_reasons.append("minimum_buy_notional_unresolved")
    if proposed_notional > DEFAULT_PER_POSITION_CAP_USD:
    ...
    if best_volume >= DEFAULT_HIGH_VOLUME_USD:
        ...
        score += 5
    elif risk_return["strategy_style"] in {"quick_trade", "grid_candidate"}:
        score -= 5
    if price_history_features:
        score += 5
    score += _grid_backtest_score(grid_backtest_summary)
    ...
    if risk_return["sizing_tier"] == "scalable_candidate":
        ...
        score -= 5
    if "expected_return_unmodeled" in risk_return_flags:
        score -= 8
    if "slippage_unmodeled" in risk_return_flags:
        score -= 8
    if "scale_price_impact_unmodeled" in risk_return_flags:
        score -= 3
    if "volume_unmodeled" in risk_return_flags and risk_return["strategy_style"] in {"quick_trade", "grid_candidate"}:
        score -= 5
    if risk_return["strategy_style"] in {"quick_trade", "grid_candidate"} and risk_return["payoff_velocity_score"] >= 20:
        score += 5
    if risk_return["strategy_style"] == "long_thesis" and (
        risk_return["payoff_velocity_score"] == 0
        or ((risk_return["expected_hold_days"] or 0) >= 90 and risk_return["payoff_velocity_score"] < 15)
    ):
        score -= 5
    ...
    return GlobalPortfolioCandidate(
        ...
        proposed_price=_decimal_to_float(price),
        proposed_size=_decimal_to_float(size),
        proposed_notional_usd=_decimal_to_float(proposed_notional),
        market_volume_usd=_decimal_to_float(market_volume),
        volume_24h_usd=_decimal_to_float(volume_24h),
        liquidity_usd=_decimal_to_float(liquidity_usd),
        price_bucket=price_bucket,
        low_price_score=low_price_score,
        cashout_score=cashout_score,
        horizon=_time_horizon(row),
        ...
        risk_return_flags=sorted(set(risk_return_flags)),
        score=score,
        status=status,
...
def _concentration_tags(row: dict[str, Any]) -> list[str]:
    ...
    if isinstance(tags, list):
        resolved.extend(str(item).strip().lower() for item in tags if str(item).strip())

    text = " ".join(
        str(value or "").lower()
        for value in (
            _portfolio_title(row),
            _portfolio_market_slug(row),
            row.get("event_slug"),
            row.get("event_title"),
            row.get("asset"),
            row.get("underlying"),
    ...
    if "microstrategy" in text or "mstr" in text:
        resolved.extend(["issuer:microstrategy", "family:microstrategy-bitcoin-sales"])
    if "bitcoin" in text or "btc" in text:
        resolved.append("asset:bitcoin")
    if "openai" in text:
        resolved.append("issuer:openai")
    if "claude" in text or "anthropic" in text:
        resolved.append("issuer:anthropic")
    if "iran" in text:
        resolved.append("region:iran")
    if "hormuz" in text:
        resolved.append("region:strait-of-hormuz")
    if "cuba" in text:
        resolved.append("region:cuba")
    if "world cup" in text or "fifa" in text:
        resolved.append("event:fifa-world-cup-2026")
    if "brazilian presidential" in text or "lula da silva" in text or "flávio bolsonaro" in text or "flavio bolsonaro" in text:
        resolved.append("event:brazil-presidential-2026")
    if "best ai model" in text and ("june" in text or "end of june" in text):
        resolved.append("event:best-ai-model-june-2026")
    if "democratic presidential nomination" in text and "2028" in text:
        resolved.append("event:2028-democratic-presidential-nomination")
    if "james bond" in text:
        resolved.append("event:next-james-bond-actor")
...
def _candidate_end_date_rejection_reason(row: dict[str, Any], *, now_utc: datetime) -> str | None:
    if bool(row.get("closed") or row.get("resolved") or row.get("archived")):
        return "market_expired_or_closed"
    status = _portfolio_first_text(row, ("status", "market_status", "event_status"))
    if status and status.lower() in {"closed", "resolved", "ended", "final", "inactive", "archived"}:
        return "market_expired_or_closed"
    end_text = _portfolio_first_text(
        row,
        (
            "end_date",
            "endDate",
    ...
    if not end_text:
        return "candidate_end_date_missing"
    try:
        end_at = _parse_datetime(end_text)
    except (TypeError, ValueError):
        return "candidate_end_date_invalid"
    if end_at <= now_utc:
        return "market_expired_or_closed"
    return None
...
def _has_minimum_order_proof(snapshot: GlobalPortfolioExecutionGateSnapshot) -> bool:
    ...
    if size is None or min_size is None or size < min_size:
        return False
    if order_type == "market" and (notional is None or min_buy_notional is None or notional < min_buy_notional):
        return False
    return True


def _global_portfolio_minimum_order_policy_evidence(proof: dict[str, Any]) -> dict[str, Any]:
    side = str(proof.get("side") or "").strip().lower()
    order_type = str(proof.get("order_type") or "limit").strip().lower()
    price = _numeric(proof.get("price") or proof.get("limit_price"))
    size = _numeric(proof.get("size"))
    ...
    if notional is None and price is not None and size is not None:
        notional = price * size
    share_minimum_applies = order_type == "limit"
    notional_minimum_applies = order_type == "market"
    blockers: list[str] = []
    if share_minimum_applies and size is not None and min_size is not None and size < min_size:
        blockers.append("minimum_limit_order_size_not_met")
    if notional_minimum_applies and notional is not None and min_buy_notional is not None and notional < min_buy_notional:
        blockers.append("minimum_market_notional_not_met")
    return {
        ...
        "required_min_size": min_size if share_minimum_applies else None,
        "required_market_notional_usd": min_buy_notional if notional_minimum_applies else None,
        "share_minimum_applies": share_minimum_applies,
        "notional_minimum_applies": notional_minimum_applies,
        "order_type_minimum_rule": "market_min_notional" if notional_minimum_applies else "limit_min_shares",
        "minimum_rule_status": "passed" if not blockers else "blocked",
        "minimum_rule_blockers": blockers,
    }
...
def _build_execution_gate_diagnostics(
    ...
    *,
    ...
    if order_type != "limit":
        minimum_blockers.append("minimum_order_proof_order_type_not_supported_for_portfolio_manager")
    if price is None:
    ...
    elif order_type == "market" and min_buy_notional is not None and notional < min_buy_notional:
        minimum_blockers.append("minimum_order_proof_buy_notional_below_minimum")
    minimum_entry = _diagnostic_entry(
    ...
    minimum_entry["minimum_order_policy"] = _global_portfolio_minimum_order_policy_evidence(proof)
    gates["minimum_order_compliance"] = minimum_entry
