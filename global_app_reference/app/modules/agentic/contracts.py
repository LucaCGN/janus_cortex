...
class LLMRevisionRequest(BaseModel):
    ...
    db_context_sources: list[dict[str, Any]] = Field(default_factory=list)
    db_stat_context_trace: dict[str, Any] = Field(default_factory=dict)
...
class LiveStrategyWorkerRequest(BaseModel):
    ...
    order_sizing_mode: str | None = None
    max_buy_notional_usd: float | None = Field(default=None, gt=0.0)
    max_order_buy_notional_usd: float | None = Field(default=None, gt=0.0)
    event_cap_usd: float | None = Field(default=None, gt=0.0, le=25.0)
    side_budget_mode: str | None = None
    max_same_side_exposure_pct: float | None = Field(default=None, ge=0.0, le=1.0)
