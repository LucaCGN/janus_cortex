...
MINIMUM_ORDER_POLICY_SCHEMA_VERSION = "polymarket_order_type_minimum_policy_v1"
...
def evaluate_strategy_plan(
    ...
    *,
    ...
    economic_buy_keys: dict[tuple[str, str | None, str, float], dict[str, Any]] = {}
    ...
    priority_aggregation_result = _order_intents_from_live_signal_aggregation(
        plan,
        market_state=market_state,
        portfolio_state=portfolio_state,
        dry_run=dry_run,
        existing_intents=[],
        max_remaining=max_intents,
    )
    exit_priority_active = any(_intent_is_exit(intent) for intent in priority_aggregation_result["intents"])
    if exit_priority_active:
        intents.extend(priority_aggregation_result["intents"])
        blockers.extend(priority_aggregation_result["blockers"])
    ...
    for strategy in plan.active_strategies:
        if len(intents) >= max_intents:
            ...
            continue
        if exit_priority_active:
            strategy_side = str(_extract_order_payload(strategy.entry_rules).get("side") or "buy").strip().lower()
            if strategy_side in {"", "buy", "rebuy"}:
                blockers.append(
                    {
                        "strategy_id": strategy.strategy_id,
                        "reason": "exit_intent_priority_blocks_entry",
                    }
                )
                continue
        ...
        strategy_side = str(order_payload.get("side") or "buy").strip().lower()
        live_entry_blocker = _live_game_entry_blocker(market_state)
        if strategy_side in {"", "buy", "rebuy"} and live_entry_blocker is not None:
            blockers.append({"strategy_id": strategy.strategy_id, **live_entry_blocker})
            continue
        ...
        notional = price * size
        min_buy_notional = _safe_float(sizing_metadata.get("min_buy_notional_usd"))
        if min_buy_notional is None:
            min_buy_notional = MIN_BUY_NOTIONAL_USD
        minimum_order_policy = _minimum_order_policy_evidence(
            side=side,
        ...
        if "minimum_limit_order_size_not_met" in minimum_order_policy["minimum_rule_blockers"]:
            blockers.append(
                {
                    "strategy_id": strategy.strategy_id,
            ...
            continue
        if "minimum_market_notional_not_met" in minimum_order_policy["minimum_rule_blockers"]:
            blockers.append(
                {
                    "strategy_id": strategy.strategy_id,
        ...
        duplicate_buy_blocker = _economic_duplicate_buy_blocker(
            seen_keys=economic_buy_keys,
            token_id=str(order_payload["token_id"]),
            outcome_id=str(order_payload["outcome_id"]),
            side=side,
            price=price,
            strategy_id=strategy.strategy_id,
            sleeve_id=str(sleeve_metadata.get("sleeve_id") or strategy.strategy_id),
            cycle_id=_clean_text(order_payload.get("cycle_id") or strategy.entry_rules.get("cycle_id")),
            allow_parallel=_allows_economic_duplicate_buy(strategy.entry_rules),
        )
        if duplicate_buy_blocker is not None:
            blockers.append(duplicate_buy_blocker)
            continue
        intents.append(
            OrderIntent(
...
def _live_game_entry_blocker(market_state: dict[str, Any]) -> dict[str, Any] | None:
    game = market_state.get("game") if isinstance(market_state.get("game"), dict) else {}
    status = _first_float(game, ("game_status", "gameStatus", "status"))
    if status is None:
        status = _first_float(market_state, ("game_status", "gameStatus", "status"))
    if status is not None:
        status_int = int(status)
        if status_int != 2:
            return {
                "reason": "game_not_live_no_entry",
                "game_status": status_int,
                "game_status_text": _clean_text(game.get("game_status_text") or game.get("gameStatusText")),
            }

    period = _first_float(game, ("period", "game_period", "quarter"))
    if period is None:
        period = _first_float(market_state, ("period", "game_period", "quarter"))
    has_explicit_scoreboard_context = any(
        key in game
        for key in (
            "game_start_time",
            "game_status_text",
    ...
    if has_explicit_scoreboard_context and period is not None and int(period) <= 0:
        return {
            "reason": "game_not_live_no_entry",
            "period": int(period),
            "game_status_text": _clean_text(game.get("game_status_text") or game.get("gameStatusText")),
...
def _intent_is_exit(intent: OrderIntent) -> bool:
    if str(intent.side or "").strip().lower() != "sell":
        return False
    metadata = intent.metadata if isinstance(intent.metadata, dict) else {}
    candidate = metadata.get("aggregation_candidate") if isinstance(metadata.get("aggregation_candidate"), dict) else {}
    signal_type = str(candidate.get("signal_type") or "").strip().lower()
    trigger_source = str(candidate.get("trigger_source") or "").strip().lower()
    if signal_type == "reduce" or trigger_source in {"reduce_stop_lifecycle", "paired_microcycle"}:
        return True
    return signal_type == "sell" and metadata.get("source") == "live_signal_aggregation"
...
def _order_intents_from_live_signal_aggregation(
    ...
    *,
    ...
    seen_economic_buy_keys = {
        key: _economic_intent_ref(intent)
        for intent in existing_intents
        if (key := _intent_economic_buy_dedupe_key(intent)) is not None
    }
    ...
    min_buy_notional = _safe_float(sizing_policy.get("min_buy_notional_usd") or sizing_policy.get("minimum_buy_notional_usd"))
    if min_buy_notional is None:
        min_buy_notional = MIN_BUY_NOTIONAL_USD
    max_buy_notional = _safe_float(
        sizing_policy.get("max_order_buy_notional_usd")
        or sizing_policy.get("max_buy_notional_usd")
        or sizing_policy.get("max_notional_usd")
    )
    strategies_by_id = {strategy.strategy_id: strategy for strategy in plan.active_strategies}
    ...
    for index, candidate in enumerate(candidates):
        ...
        size, size_metadata = _candidate_size(
            ...
            strategy=strategy,
            min_size=min_size,
            sizing_mode=str(sizing_policy.get("mode") or "").strip().lower(),
            min_buy_notional_usd=min_buy_notional,
        )
        ...
        if side not in {"buy", "sell"}:
            ...
            continue
        live_entry_blocker = _live_game_entry_blocker(market_state)
        if side == "buy" and live_entry_blocker is not None:
            blockers.append(
                {
                    "scope": "live_signal_aggregation",
                    "strategy_id": strategy_id,
        ...
        notional = price * size
        effective_min_buy_notional = _safe_float(size_metadata.get("min_buy_notional_usd"))
        if effective_min_buy_notional is None:
            effective_min_buy_notional = min_buy_notional
        minimum_order_policy = _minimum_order_policy_evidence(
            side=side,
            order_type="limit",
            price=price,
            size=size,
            min_size=min_size,
            min_buy_notional_usd=effective_min_buy_notional,
        )
        if "minimum_limit_order_size_not_met" in minimum_order_policy["minimum_rule_blockers"]:
            blockers.append(
                {
            ...
            blockers.append(
                {
                    ...
                    "reason": "minimum_buy_notional_not_met",
                    "strategy_id": strategy_id,
                    "minimum_buy_notional_usd": effective_min_buy_notional,
                    "required_notional_usd": round(notional, 6),
                    "minimum_order_policy": minimum_order_policy,
                }
            )
        ...
        if lifecycle_blocker is not None:
            ...
            continue
        unresolved_buy_blocker = _aggregation_candidate_unresolved_buy_blocker(
            portfolio_state,
            side=side,
            token_id=token_id,
            outcome_id=outcome_id,
        ...
        if unresolved_buy_blocker is not None:
            blockers.append(unresolved_buy_blocker)
            continue
        duplicate_buy_blocker = _economic_duplicate_buy_blocker(
            seen_keys=seen_economic_buy_keys,
            token_id=token_id,
            outcome_id=outcome_id,
            side=side,
            price=price,
            ...
            allow_parallel=_candidate_allows_economic_duplicate_buy(candidate, strategy),
            scope="live_signal_aggregation",
        )
        if duplicate_buy_blocker is not None:
            blockers.append(duplicate_buy_blocker)
...
def _aggregation_candidate_unresolved_buy_blocker(
    portfolio_state: dict[str, Any],
    *,
    side: str | None,
    token_id: str | None,
    ...
    pending_matches = _matching_pending_buy_orders(
        portfolio_state,
        token_id=token_id,
        outcome_id=outcome_id,
        strategy_id=strategy_id,
    ...
    if pending_matches:
        return {
            ...
            "token_id": token_id,
            "outcome_id": outcome_id,
            "pending_intents": float(len(pending_matches)),
            "pending_order_ids": [
                item.get("external_order_id") or item.get("order_id")
                for item in pending_matches
                if item.get("external_order_id") or item.get("order_id")
            ],
            "duplicate_scope": "pending_strategy_sleeve_or_token_buy",
        }

    open_order_matches = _matching_direct_open_buy_orders(
        portfolio_state,
        token_id=token_id,
        outcome_id=outcome_id,
    )
    if open_order_matches:
        return {
            "scope": "live_signal_aggregation",
            "reason": "direct_open_buy_order_present",
            "strategy_id": strategy_id,
            "sleeve_id": sleeve_id,
            "token_id": token_id,
            "outcome_id": outcome_id,
            "open_order_count": float(len(open_order_matches)),
            "open_order_ids": [
                item.get("id") or item.get("order_id") or item.get("external_order_id")
                for item in open_order_matches
                if item.get("id") or item.get("order_id") or item.get("external_order_id")
            ],
            "duplicate_scope": "direct_token_buy_order",
        }
    return None


def _matching_pending_buy_orders(
    portfolio_state: dict[str, Any],
    *,
    token_id: str | None,
    outcome_id: str | None,
    ...
    for order in pending_orders:
        ...
        if pending_side and pending_side != "buy":
            continue
        pending_strategy_id = _clean_text(order.get("strategy_id"))
        pending_sleeve_id = _clean_text(order.get("sleeve_id"))
        pending_token_id = _clean_text(order.get("token_id") or order.get("market_token_id") or order.get("asset_id"))
        pending_outcome_id = _clean_text(order.get("outcome_id"))
        if (
            (pending_strategy_id and pending_strategy_id == strategy_id)
            or (pending_sleeve_id and pending_sleeve_id == sleeve_id)
            or (pending_token_id and token_id and pending_token_id == token_id)
            or (pending_outcome_id and outcome_id and pending_outcome_id == outcome_id)
        ):
            matches.append(order)
    return matches


def _matching_direct_open_buy_orders(
    portfolio_state: dict[str, Any],
    *,
    token_id: str | None,
    outcome_id: str | None,
    ...
    for order in orders:
        if not isinstance(order, dict):
            continue
        order_side = str(order.get("side") or order.get("order_side") or "").strip().lower()
        if order_side and order_side != "buy":
            continue
        if _direct_order_matches(order, token_id=token_id or "", outcome_id=outcome_id or ""):
            matches.append(order)
    return matches
...
def _candidate_size(
    ...
    *,
    ...
    if side == "buy" and sizing_mode in {"fixed_min_shares", "fixed_min_size", "minimum_shares"}:
        return min_size, {
            "mode": "fixed_min_shares",
            "requested_size": _safe_float(candidate.get("requested_shares") or candidate.get("size")),
            "resolved_size_source": "operator_fixed_min_shares",
            "min_size": min_size,
...
def _minimum_order_policy_evidence(
    *,
    side: str,
    order_type: str,
    price: float,
    ...
    normalized_order_type = str(order_type or "limit").strip().lower()
    notional = price * size
    share_minimum_applies = normalized_order_type == "limit"
    notional_minimum_applies = normalized_order_type == "market"
    blockers: list[str] = []
    ...
    return {
        "schema_version": MINIMUM_ORDER_POLICY_SCHEMA_VERSION,
        ...
        "required_market_notional_usd": min_buy_notional_usd if notional_minimum_applies else None,
...
def _intent_economic_buy_dedupe_key(intent: OrderIntent) -> tuple[str, str | None, str, float] | None:
    price = _safe_float(intent.price)
    return _economic_buy_dedupe_key(
...
def _economic_buy_dedupe_key(
    *,
    ...
    normalized_side = str(side or "").strip().lower()
    if normalized_side != "buy" or price is None:
        return None
    token_key = _clean_text(token_id) or _clean_text(outcome_id)
    if token_key is None:
        return None
    return (token_key, _clean_text(outcome_id), normalized_side, round(float(price), 4))


def _economic_intent_ref(intent: OrderIntent) -> dict[str, Any]:
    cycle_id = None
    if isinstance(intent.metadata, dict):
        cycle_id = _clean_text(intent.metadata.get("cycle_id"))
    return {
...
def _economic_duplicate_buy_blocker(
    *,
    seen_keys: dict[tuple[str, str | None, str, float], dict[str, Any]],
    token_id: Any,
    outcome_id: Any,
    side: Any,
    price: float | None,
    ...
    scope: str = "strategy_plan",
) -> dict[str, Any] | None:
    key = _economic_buy_dedupe_key(token_id=token_id, outcome_id=outcome_id, side=side, price=price)
    if key is None:
        return None
    current_ref = {
        "strategy_id": strategy_id,
    ...
    if allow_parallel:
        seen_keys.setdefault(key, current_ref)
        return None
    existing = seen_keys.get(key)
    if existing is None:
        seen_keys[key] = current_ref
        return None
    return {
        "scope": scope,
        "reason": "economic_duplicate_buy_intent_blocked",
        "strategy_id": strategy_id,
        "sleeve_id": sleeve_id,
        "token_id": token_id,
        "outcome_id": outcome_id,
        ...
        "price": price,
        "cycle_id": cycle_id,
        "duplicate_scope": "event_token_side_price",
        "duplicate_of_strategy_id": existing.get("strategy_id"),
        "duplicate_of_sleeve_id": existing.get("sleeve_id"),
        "duplicate_of_cycle_id": existing.get("cycle_id"),
    }


def _allows_economic_duplicate_buy(entry_rules: dict[str, Any]) -> bool:
    return any(
        bool(entry_rules.get(key))
        for key in (
            "allow_economic_duplicate_buy",
            "allow_same_price_parallel_buy",
            "parallel_same_band_reviewed",
        )
    )


def _candidate_allows_economic_duplicate_buy(candidate: dict[str, Any], strategy: Any | None) -> bool:
    if strategy is not None and _allows_economic_duplicate_buy(dict(getattr(strategy, "entry_rules", {}) or {})):
        return True
    lifecycle = candidate.get("lifecycle_policy") if isinstance(candidate.get("lifecycle_policy"), dict) else {}
    return any(
        bool(source.get(key))
        for source in (candidate, lifecycle)
        for key in (
            "allow_economic_duplicate_buy",
            "allow_same_price_parallel_buy",
            "parallel_same_band_reviewed",
...
def _resolve_order_size(
    ...
    *,
    ...
    min_buy_notional = _safe_float(policy.get("min_buy_notional_usd") or policy.get("minimum_buy_notional_usd"))
    if min_buy_notional is None:
        min_buy_notional = MIN_BUY_NOTIONAL_USD
    if mode in {"fixed_min_shares", "fixed_min_size", "minimum_shares"}:
    ...
    metadata = {
        ...
        "mode": mode or "operator_minimum_order",
        "min_size": min_size,
        "min_buy_notional_usd": min_buy_notional,
        "min_buy_notional_buffer_usd": min_buy_notional_buffer,
        "effective_min_buy_notional_usd": effective_min_buy_notional,
        "max_buy_notional_usd": _safe_float(
            policy.get("max_order_buy_notional_usd")
            or policy.get("max_buy_notional_usd")
            or policy.get("max_notional_usd")
        ),
