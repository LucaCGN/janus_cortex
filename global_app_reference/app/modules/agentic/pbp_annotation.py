...
def build_pbp_annotation_evidence(
    *,
    ...
    tags.extend(_player_catalyst_tags(rows))
...
def _player_status_text_tag(rows: list[dict[str, Any]]) -> dict[str, Any] | None:
    ...
    return {
        ...
        "severity": "elevated",
        "confidence": 0.55,
        "sleeve_relevance": ["core_hold", "reduce_stop", "grid_scalp", "rebound_snipe"],
...
def _player_catalyst_tags(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    texts = _recent_pbp_texts(rows)
    if not texts:
        return []
    tags: list[dict[str, Any]] = []
    foul_hits = [
        text
        for text in texts
        if re.search(r"\b(5th|fifth|6th|sixth)\s+foul\b", text)
        or "foul trouble" in text
        or "fouled out" in text
    ]
    if foul_hits:
        tags.append(
            {
                ...
                "severity": "elevated",
                "confidence": 0.72,
                "sleeve_relevance": ["rebound_snipe", "core_hold", "reduce_stop", "grid_scalp"],
                "reason": "Recent PBP text indicates high-leverage foul trouble; rebound-snipe and exposure review should consider the catalyst.",
                "evidence": {"matched_rows": foul_hits[:3]},
            }
        )
    rotation_hits = [
        text
        ...
        or "checks in" in text
        or "checks out" in text
    ]
    if rotation_hits:
        tags.append(
            {
                "tag_type": "player_rotation_shock_watch",
                "severity": "routine",
                "confidence": 0.62,
                "sleeve_relevance": ["rebound_snipe", "grid_scalp", "core_hold"],
                "reason": "Recent PBP text indicates a rotation or bench/rest window; price dislocation should be reviewed as context, not direct authority.",
                "evidence": {"matched_rows": rotation_hits[:3]},
            }
        )
    injury_hits = [text for text in texts if "injury" in text or "injured" in text]
    if injury_hits:
        tags.append(
            {
                "tag_type": "player_injury_watch",
                "severity": "critical",
                "confidence": 0.72,
                "sleeve_relevance": ["reduce_stop", "core_hold", "rebound_snipe"],
                "reason": "Recent PBP text indicates injury context; exposure review should escalate if inventory exists.",
                "evidence": {"matched_rows": injury_hits[:3]},
            }
        )
    ejection_hits = [text for text in texts if "ejected" in text or "ejection" in text]
    if ejection_hits:
        tags.append(
            {
                "tag_type": "player_ejection_watch",
                "severity": "critical",
                "confidence": 0.74,
                "sleeve_relevance": ["reduce_stop", "core_hold", "rebound_snipe"],
                "reason": "Recent PBP text indicates ejection context; exposure review should escalate if inventory exists.",
                "evidence": {"matched_rows": ejection_hits[:3]},
            }
        )
    return tags


def _recent_pbp_texts(rows: list[dict[str, Any]]) -> list[str]:
    texts: list[str] = []
    for row in rows[:12]:
        payload = row.get("payload_json") if isinstance(row.get("payload_json"), dict) else {}
        text = " ".join(
