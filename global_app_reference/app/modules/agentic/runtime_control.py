...
class EventControlStopWithPositionsPolicy(BaseModel):
    model_config = ConfigDict(extra="forbid")

    enabled: bool = True
    max_recovery_delay_seconds: float = Field(default=300.0, ge=0.0)
    backup_restart_required: bool = True
    set_sell_targets_if_restart_blocked: bool = True
    sell_targets_must_use_janus_gates: bool = True
    raw_order_bypass_allowed: bool = False
    policy_action: Literal["restart_or_set_sell_targets"] = "restart_or_set_sell_targets"
    reason: str = "stop_with_positions_requires_restart_or_sell_targets"
...
class EventControlConfig(BaseModel):
    ...
    stop_with_positions_policy: EventControlStopWithPositionsPolicy = Field(
        default_factory=EventControlStopWithPositionsPolicy
    )
...
class EventControlUpdateRequest(BaseModel):
    ...
    stop_with_positions_policy: EventControlStopWithPositionsPolicy | None = None
...
def update_event_control_config(
    ...
    *,
    ...
    merged_stop_policy = existing.stop_with_positions_policy.model_copy(
        update=(
            update.stop_with_positions_policy.model_dump(exclude_none=True)
            if update.stop_with_positions_policy is not None
            else {}
        )
    )
    ...
    _validate_stop_with_positions_policy(merged_stop_policy)
    ...
    config = EventControlConfig(
        ...
        stop_with_positions_policy=merged_stop_policy,
    ...
    append_jsonl(
        event_control_root(config.session_date, root=root) / "event_control_updates.jsonl",
        {
            ...
            "stop_with_positions_policy": config.stop_with_positions_policy.model_dump(mode="json"),
...
def _validate_parameters(parameters: EventControlParameters) -> None:
    if parameters.event_cap_usd is not None and parameters.event_cap_usd > 25.0:
        raise EventControlValidationError(
            "event_cap_above_current_live_learning_limit",
            {"event_cap_usd": parameters.event_cap_usd, "maximum_allowed_usd": 25.0},
        )
...
def _validate_stop_with_positions_policy(policy: EventControlStopWithPositionsPolicy) -> None:
    if not policy.enabled:
        return
    if not policy.backup_restart_required and not policy.set_sell_targets_if_restart_blocked:
        raise EventControlValidationError(
            "stop_with_positions_policy_requires_recovery_action",
            {
                "backup_restart_required": policy.backup_restart_required,
                "set_sell_targets_if_restart_blocked": policy.set_sell_targets_if_restart_blocked,
            },
        )
    if policy.raw_order_bypass_allowed:
        raise EventControlValidationError("stop_with_positions_raw_order_bypass_forbidden")
...
__all__ = [
    ...
    "EventControlStopWithPositionsPolicy",
