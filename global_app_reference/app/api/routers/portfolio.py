...
from app.api.models import (
    ...
    PortfolioManagerPriceMemoryCollectRequest,
    PortfolioManagerPriceMemoryRequest,
    PortfolioManagerProfileObservationRequest,
    PortfolioManagerSlotsReconcileRequest,
    PortfolioManagerStrategyWorkerLegPlanRequest,
    PortfolioManagerStrategyWorkerStatusRequest,
    PortfolioManagerStrategyWorkerTickRequest,
    PortfolioManagerStrategyWorkerUpsertRequest,
...
_MINIMUM_ORDER_POLICY_SCHEMA_VERSION = "polymarket_order_type_minimum_policy_v1"
...
def build_portfolio_manager_order_management_preview(
    *,
    ...
    minimum_order_policy = _portfolio_manager_minimum_policy_from_proof(
        dict(plan.gate_snapshot.minimum_order_proof or {})
    )
    ...
    return {
        ...
        "concrete_adapter_proof": {
            ...
            "minimum_order_policy": to_jsonable(minimum_order_policy),
...
def _portfolio_manager_minimum_order_policy_evidence(
    *,
    side: str,
    order_type: str,
    price: Decimal,
    ...
    blockers: list[str] = []
    if share_minimum_applies and size < min_size:
        blockers.append("minimum_limit_order_size_not_met")
    if notional_minimum_applies and notional < min_buy_notional_usd:
        blockers.append("minimum_market_notional_not_met")
        if side == "buy":
            blockers.append("minimum_buy_notional_not_met")
    return {
        "schema_version": _MINIMUM_ORDER_POLICY_SCHEMA_VERSION,
        "side": side,
        ...
        "min_buy_notional_usd": float(min_buy_notional_usd),
        "required_min_size": float(min_size) if share_minimum_applies else None,
        "required_market_notional_usd": float(min_buy_notional_usd) if notional_minimum_applies else None,
...
def _portfolio_manager_minimum_policy_from_proof(proof: dict[str, Any]) -> dict[str, Any] | None:
    side = str(proof.get("side") or "").strip().lower()
    order_type = str(proof.get("order_type") or "limit").strip().lower()
    price = _optional_proof_decimal(proof, "limit_price", "price")
    size = _optional_proof_decimal(proof, "size")
    if not side or price is None or size is None:
        return None
    min_size = _optional_proof_decimal(proof, "min_size", "minimum_size") or Decimal("5")
    min_buy_notional = (
        _optional_proof_decimal(proof, "min_buy_notional_usd", "minimum_buy_notional_usd")
        or Decimal("1")
    )
    return _portfolio_manager_minimum_order_policy_evidence(
        side=side,
        order_type=order_type,
        price=price,
        size=size,
...
def _normalize_portfolio_manager_requested_order(
    *,
    ...
    minimum_order_policy = _portfolio_manager_minimum_order_policy_evidence(
        side=side,
        order_type=order_type,
        price=limit_price,
        size=size,
    ...
    if minimum_order_policy["minimum_rule_blockers"]:
        raise HTTPException(
            status_code=422,
            detail=f"requested_order.minimum_order_policy blocked: {', '.join(minimum_order_policy['minimum_rule_blockers'])}",
        )
    ...
    *,
    ...
    return {
        ...
        "requested_order": {
            ...
            "size": float(size),
            "notional_usd": float(notional),
            "minimum_order_policy": to_jsonable(normalized_order.get("minimum_order_policy")),
        },
        "risk_checks": {
            ...
            "minimum_order_policy_passed": (
                (normalized_order.get("minimum_order_policy") or {}).get("minimum_rule_status") == "passed"
            ),
        },
        "rate_limit": {
    ...
    with connection.cursor() as cursor:
        cursor.execute(
            ...
            INSERT INTO portfolio.manager_budget_snapshots (
                ...
                budget_status,
                budget_source_confidence,
                budget_source_caveats,
                snapshot_json,
                created_at
            ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
            ...
            (
                ...
                budget.equity_usd,
                budget.cash_usd,
                budget.base_sleeve_cap_usd,
                budget.realized_global_pnl_usd,
                budget.dynamic_sleeve_cap_usd,
                ...
                budget.budget_source_confidence,
                Json(to_jsonable(budget.budget_source_caveats)),
        ...
        for slot in board.slots:
            ...
            cursor.execute(
                ...
                DO UPDATE SET
                    ...
                    premise_state = EXCLUDED.premise_state,
                    hygiene_bucket = EXCLUDED.hygiene_bucket,
                    market_volume_usd = EXCLUDED.market_volume_usd,
                    volume_24h_usd = EXCLUDED.volume_24h_usd,
                    price_bucket = EXCLUDED.price_bucket,
                    cashout_score = EXCLUDED.cashout_score,
                    price_memory_json = EXCLUDED.price_memory_json,
                    worker_allocation_json = EXCLUDED.worker_allocation_json,
                ...
                (
                    ...
                    slot.premise_state,
                    slot.hygiene_bucket,
                    slot.market_volume_usd,
                    slot.volume_24h_usd,
                    slot.price_bucket,
                    slot.cashout_score,
                    Json(to_jsonable(slot.price_memory)),
                    Json(to_jsonable(slot.worker_allocation)),
        ...
        for candidate in queue.candidates:
            ...
            cursor.execute(
                ...
                DO UPDATE SET
                    ...
                    price_bucket = EXCLUDED.price_bucket,
                    low_price_score = EXCLUDED.low_price_score,
                    cashout_score = EXCLUDED.cashout_score,
                    horizon = EXCLUDED.horizon,
                    confidence = EXCLUDED.confidence,
                    ...
                    price_history_features = EXCLUDED.price_history_features,
                    grid_backtest_summary = EXCLUDED.grid_backtest_summary,
                ...
                (
                    ...
                    candidate.market_volume_usd,
                    candidate.volume_24h_usd,
                    candidate.liquidity_usd,
                    candidate.price_bucket,
                    candidate.low_price_score,
                    candidate.cashout_score,
                    candidate.horizon,
                    ...
                    Json(to_jsonable(candidate.price_history_features)),
                    Json(to_jsonable(candidate.grid_backtest_summary)),
                    candidate.score,
                    candidate.status,
    ...
    with cursor_dict(connection) as cursor:
        cursor.execute(
            ...
            INSERT INTO portfolio.manager_grid_eligibility_reviews (
                ...
                one_day_range_cents,
                seven_day_range_cents,
                thirty_day_range_cents,
                days_to_resolution,
                stable_thesis,
                spread_cents,
                depth_usd,
                ...
                one_day_backtest_positive,
                three_day_backtest_positive,
                seven_day_backtest_positive,
                recommended_worker_duration_hours,
                backtest_json,
                eligible,
                status,
                blockers,
            ...
            (
                ...
                review.market_slug,
                review.token_id,
                review.validator_profile,
                review.premise_state,
                review.thirty_day_range_percent,
                review.one_day_range_cents,
                review.seven_day_range_cents,
                review.thirty_day_range_cents,
                ...
                review.one_day_backtest_positive,
                review.three_day_backtest_positive,
                review.seven_day_backtest_positive,
                review.recommended_worker_duration_hours,
                Json(to_jsonable(review.backtest_json)),
                review.eligible,
                review.status,
...
def apply_portfolio_manager_price_memory_collection(
    connection: PsycopgConnection,
    *,
    account_id: str | None,
    ...
    now = datetime.now(timezone.utc)
    with cursor_dict(connection) as cursor:
        for snapshot in collection.get("snapshots", []):
            if not isinstance(snapshot, dict):
                continue
            market_title = str(snapshot.get("market_title") or "").strip()
            if not market_title:
                continue
            source = str(snapshot.get("source") or "global_portfolio_price_memory").strip()
            registry_id = _portfolio_sync_uuid_for(
                "portfolio_manager_market_registry",
                source,
                str(snapshot.get("token_id") or ""),
                str(snapshot.get("market_slug") or ""),
            )
            cursor.execute(
                """
                INSERT INTO portfolio.manager_market_registry (
                    registry_id,
                    account_id,
                    market_title,
                    market_slug,
                ...
                (
                    ...
                    snapshot.get("market_volume_usd"),
                    snapshot.get("volume_24h_usd"),
                    snapshot.get("liquidity_usd"),
                    (snapshot.get("registry_json") or {}).get("premise_state", "unreviewed")
                    if isinstance(snapshot.get("registry_json"), dict)
                    else "unreviewed",
                    snapshot.get("slot_state") or "candidate",
                    now,
                    Json(to_jsonable(snapshot.get("registry_json") or snapshot)),
                    now,
                ),
            )
            registry_written += 1
            features = snapshot.get("features_json")
            if not isinstance(features, dict):
                continue
            snapshot_id = str(uuid4())
            cursor.execute(
                """
                INSERT INTO portfolio.manager_price_feature_snapshots (
                    snapshot_id,
                    account_id,
                    market_slug,
                    token_id,
                    ...
                    thirty_day_range_cents,
                    one_day_trend_cents,
                    seven_day_trend_cents,
                    thirty_day_trend_cents,
                    features_json,
                    captured_at
                )
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s);
                ...
                (
                    ...
                    snapshot.get("outcome"),
                    source,
                    snapshot.get("feature_schema_version") or features.get("schema_version"),
                    snapshot.get("one_day_range_cents"),
                    snapshot.get("seven_day_range_cents"),
                    snapshot.get("thirty_day_range_cents"),
                    snapshot.get("one_day_trend_cents"),
                    snapshot.get("seven_day_trend_cents"),
                    snapshot.get("thirty_day_trend_cents"),
                    Json(to_jsonable(features)),
                    now,
                ),
            )
            snapshots_written += 1
    return {
        "registry_rows_upserted": registry_written,
        "price_feature_snapshots_written": snapshots_written,
    }


def apply_portfolio_manager_strategy_worker(
    connection: PsycopgConnection,
    *,
    account_id: str | None,
    slot_id: str | None,
    ...
    spec = build_strategy_worker_spec(worker_config)
    if not spec.dry_run:
        raise HTTPException(status_code=422, detail="strategy workers are dry-run only")
    now = datetime.now(timezone.utc)
    config_json = dict(worker_config)
    config_json.setdefault("reviewed_by", reviewed_by)
    config_json.setdefault("reason", reason)
    with cursor_dict(connection) as cursor:
        cursor.execute(
            """
            INSERT INTO portfolio.manager_strategy_workers (
                worker_id,
                account_id,
                slot_id,
                market_slug,
            ...
            DO UPDATE SET
                ...
                dry_run = EXCLUDED.dry_run,
                premise_state = EXCLUDED.premise_state,
                grid_step_price = EXCLUDED.grid_step_price,
            ...
            (
                ...
                spec.dry_run,
                spec.premise_state,
                spec.grid_step_price,
                spec.review_after_at,
                spec.max_notional_usd,
                Json(to_jsonable(config_json)),
                False,
                False,
                now,
            ),
        )
    return {"worker_id": spec.worker_id, "state": spec.state, "dry_run": spec.dry_run, "applied": True}


def apply_portfolio_manager_strategy_worker_tick(
    connection: PsycopgConnection,
    *,
    tick: dict[str, Any],
) -> dict[str, Any]:
    ledger = tick.get("ledger_event") if isinstance(tick.get("ledger_event"), dict) else {}
    heartbeat = tick.get("heartbeat_json") if isinstance(tick.get("heartbeat_json"), dict) else {}
    worker_id = str(tick.get("worker_id") or ledger.get("worker_id") or "").strip()
    if not worker_id:
        raise HTTPException(status_code=422, detail="worker_id is required for worker tick persistence")
    ledger_id = str(uuid4())
    now = datetime.now(timezone.utc)
    with cursor_dict(connection) as cursor:
        cursor.execute(
            """
            UPDATE portfolio.manager_strategy_workers
            SET
                state = %s,
                last_heartbeat_json = %s,
                order_preparation_allowed = FALSE,
                order_submission_allowed = FALSE,
                updated_at = %s
            WHERE worker_id = %s;
        ...
        cursor.execute(
            """
            INSERT INTO portfolio.manager_strategy_worker_ledger (
                ledger_id,
                worker_id,
                event_type,
                event_status,
                idempotency_key,
                event_json,
                order_preparation_attempted,
                order_submission_attempted,
                recorded_at
            )
            VALUES (%s, %s, %s, %s, %s, %s, FALSE, FALSE, %s)
            ON CONFLICT (worker_id, idempotency_key) WHERE idempotency_key IS NOT NULL
            DO NOTHING;
            """,
            (
                ledger_id,
                worker_id,
                ledger.get("event_type") or "dry_run_tick",
                ledger.get("event_status") or tick.get("status"),
                ledger.get("idempotency_key"),
                Json(to_jsonable(ledger or tick)),
                now,
            ),
        )
    return {"worker_id": worker_id, "ledger_id": ledger_id, "state": tick.get("next_state"), "applied": True}


def fetch_portfolio_manager_strategy_worker_config(
    connection: PsycopgConnection,
    *,
    worker_id: str,
) -> dict[str, Any] | None:
    with cursor_dict(connection) as cursor:
        cursor.execute(
            ...
            SELECT
                ...
                max_notional_usd,
                config_json
            FROM portfolio.manager_strategy_workers
            WHERE worker_id = %s;
            """,
            (worker_id,),
        )
    ...
    if row is None:
        return None
    payload = dict(row.get("config_json") or {})
    payload.update(
        {
            ...
            "grid_step_price": row.get("grid_step_price"),
            "review_after_at": row.get("review_after_at"),
            "max_notional_usd": row.get("max_notional_usd"),
        }
    )
...
def _unresolved_lifecycle_reason(
    row: dict[str, Any],
    *,
    ...
    if lifecycle_status not in _UNKNOWN_LIFECYCLE_STATUSES:
        return None

    status = str(row.get("status") or "").strip().lower()
    external_order_id = str(row.get("external_order_id") or "").strip()
    if lifecycle_status == "direct_flat_status_unknown":
        reason = "direct_flat_open_order_missing_terminal_status"
        evidence_gap = "direct account snapshot is flat, but the local order is still open without fill or terminal status evidence"
        required_evidence = ["terminal_order_status", "linked_trade_row", "direct_clob_trade_match"]
    elif lifecycle_status == "direct_status_unknown":
        if direct_open_order_count is None and direct_open_position_count is None:
            reason = "direct_account_snapshot_unavailable_for_open_order"
            evidence_gap = "direct account snapshot was unavailable, so the local open order cannot be confirmed live, filled, or terminal"
        else:
            reason = "direct_account_snapshot_not_flat_for_open_order"
            evidence_gap = "direct account snapshot is not flat, but this order is not linked to a direct open order or trade"
        required_evidence = ["direct_account_snapshot", "direct_open_order_match", "direct_clob_trade_match"]
    elif lifecycle_status == "local_open_without_external_id":
        reason = "local_open_order_missing_external_id"
        evidence_gap = "local open order has no external order id, so direct CLOB/account reconciliation cannot match it"
        required_evidence = ["external_order_id", "linked_trade_row", "terminal_order_status"]
    else:
        reason = "unknown_order_lifecycle_status"
        evidence_gap = "order lifecycle status is unknown"
        required_evidence = ["terminal_order_status", "linked_trade_row", "direct_clob_trade_match"]
...
def build_order_lifecycle_reconciliation_report(
    ...
    *,
    ...
    unresolved_lifecycle_reason_counts: dict[str, int] = {}
    ...
    for row in rows:
        ...
        unresolved_lifecycle_reason = _unresolved_lifecycle_reason(
            row,
        ...
        if row_lifecycle in _UNKNOWN_LIFECYCLE_STATUSES:
            ...
            if unresolved_lifecycle_reason is not None:
                reason = str(unresolved_lifecycle_reason.get("reason") or "unknown_order_lifecycle_status")
                unresolved_lifecycle_reason_counts[reason] = unresolved_lifecycle_reason_counts.get(reason, 0) + 1
                actor_reasons = actor_bucket["unresolved_lifecycle_reason_counts"]
                actor_reasons[reason] = actor_reasons.get(reason, 0) + 1
        ...
        items.append(
            {
                ...
                "unresolved_lifecycle_reason": unresolved_lifecycle_reason,
            }
        )
    ...
    return {
        ...
        "unresolved_lifecycle_reason_counts": dict(sorted(unresolved_lifecycle_reason_counts.items())),
...
def record_portfolio_manager_grid_eligibility_review(
    ...
    review = build_grid_eligibility_review(
        ...
        validator_profile=payload.validator_profile,
        premise_state=payload.premise_state,
        thirty_day_range_percent=payload.thirty_day_range_percent,
        one_day_range_cents=payload.one_day_range_cents,
        seven_day_range_cents=payload.seven_day_range_cents,
        thirty_day_range_cents=payload.thirty_day_range_cents,
        ...
        one_day_backtest_positive=payload.one_day_backtest_positive,
        three_day_backtest_positive=payload.three_day_backtest_positive,
        seven_day_backtest_positive=payload.seven_day_backtest_positive,
        recommended_worker_duration_hours=payload.recommended_worker_duration_hours,
        backtest_json=payload.backtest_json,
...
@router.post("/manager/price-memory/features")
def build_portfolio_manager_price_memory_features(payload: PortfolioManagerPriceMemoryRequest) -> dict[str, Any]:
    features = build_price_memory_features(payload.price_history)
    backtest = simulate_grid_backtest(
        payload.price_history,
        grid_step_price=payload.grid_step_price,
        leg_size=payload.leg_size,
        starting_inventory=payload.starting_inventory,
        slippage_cents=payload.slippage_cents,
    )
    ...
    return {
        ...
        "token_id": payload.token_id,
        "live_order_impact": "read-only",
        "price_memory_features": to_jsonable(features),
        "grid_backtest_summary": to_jsonable(backtest),
        "grid_scalp_validation": to_jsonable(validation),
        "side_effects": {
            "orders_placed": False,
            "orders_cancelled": False,
            "orders_replaced": False,
...
@router.post("/manager/price-memory/collect")
def collect_portfolio_manager_price_memory(
    payload: PortfolioManagerPriceMemoryCollectRequest,
    connection: PsycopgConnection = Depends(get_db_connection),
) -> dict[str, Any]:
    reviewer = str(payload.reviewed_by or "").strip()
    review_reason = str(payload.reason or "").strip()
    ...
    collection = build_price_memory_collection(
        payload.market_rows,
        price_history_by_token=payload.price_history_by_token,
    ...
    if not payload.dry_run:
        applied.append(
            apply_portfolio_manager_price_memory_collection(
                connection,
                account_id=str(payload.account_id) if payload.account_id is not None else None,
    ...
    return {
        ...
        "account_id": str(payload.account_id) if payload.account_id is not None else None,
        "live_order_impact": "read-only",
        "price_memory_collection": to_jsonable(collection),
        "applied": to_jsonable(applied),
        "side_effects": {
            "orders_placed": False,
            "orders_cancelled": False,
...
@router.get("/manager/strategy-workers")
def list_portfolio_manager_strategy_workers(
    account_id: UUID | None = Query(default=None),
    ...
    return {
        "status": "ok",
        "count": len(rows),
        "workers": to_jsonable(rows),
        "filters": {"account_id": str(account_id) if account_id is not None else None, "state": state},
    }


@router.post("/manager/strategy-workers")
def upsert_portfolio_manager_strategy_worker(
    payload: PortfolioManagerStrategyWorkerUpsertRequest,
    connection: PsycopgConnection = Depends(get_db_connection),
) -> dict[str, Any]:
    reviewer = str(payload.reviewed_by or "").strip()
    review_reason = str(payload.reason or "").strip()
    if not payload.dry_run and (not reviewer or not review_reason):
        raise HTTPException(status_code=422, detail="reviewed_by and reason are required when dry_run=false")
    spec = build_strategy_worker_spec(payload.worker_config)
    if not spec.dry_run:
        raise HTTPException(status_code=422, detail="strategy workers are dry-run only")
    applied: list[dict[str, Any]] = []
    if not payload.dry_run:
        applied.append(
            apply_portfolio_manager_strategy_worker(
                connection,
                account_id=str(payload.account_id) if payload.account_id is not None else None,
                slot_id=payload.slot_id,
    ...
    return {
        ...
        "status": "planned" if payload.dry_run else "applied",
        "live_order_impact": "read-only",
        "strategy_worker": to_jsonable(spec.safe_dict()),
        "applied": to_jsonable(applied),
        "side_effects": {
            "orders_placed": False,
            "orders_cancelled": False,
...
@router.post("/manager/strategy-workers/status")
def summarize_portfolio_manager_strategy_workers(payload: PortfolioManagerStrategyWorkerStatusRequest) -> dict[str, Any]:
    status_payload = build_strategy_worker_status({"workers": payload.workers})
    return {
        "dry_run": payload.dry_run,
        "status": "planned",
        "live_order_impact": "read-only",
        "strategy_worker_status": to_jsonable(status_payload),
        "side_effects": {
            "orders_placed": False,
            "orders_cancelled": False,
            "orders_replaced": False,
...
@router.post("/manager/strategy-workers/tick")
def tick_portfolio_manager_strategy_worker(
    payload: PortfolioManagerStrategyWorkerTickRequest,
    connection: PsycopgConnection = Depends(get_db_connection),
) -> dict[str, Any]:
    reviewer = str(payload.reviewed_by or "").strip()
    review_reason = str(payload.reason or "").strip()
    if not payload.dry_run and (not reviewer or not review_reason):
        raise HTTPException(status_code=422, detail="reviewed_by and reason are required when dry_run=false")
    worker_config = dict(payload.worker_config)
    if not worker_config and payload.worker_id:
        loaded = fetch_portfolio_manager_strategy_worker_config(connection, worker_id=payload.worker_id)
        if loaded is None:
            raise HTTPException(status_code=404, detail="strategy worker not found")
        worker_config = loaded
    if not worker_config:
        raise HTTPException(status_code=422, detail="worker_config or worker_id is required")
    ...
    applied: list[dict[str, Any]] = []
    if not payload.dry_run:
        applied.append(apply_portfolio_manager_strategy_worker_tick(connection, tick=tick))
    return {
        "dry_run": payload.dry_run,
        "status": "planned" if payload.dry_run else "applied",
        "live_order_impact": "read-only",
        "strategy_worker_tick": to_jsonable(tick),
        "applied": to_jsonable(applied),
        "side_effects": {
            "orders_placed": False,
            "orders_cancelled": False,
...
@router.post("/manager/strategy-workers/leg-plan")
def plan_portfolio_manager_strategy_worker_leg(payload: PortfolioManagerStrategyWorkerLegPlanRequest) -> dict[str, Any]:
    ...
    return {
        ...
        "status": "planned",
        "live_order_impact": "read-only",
        "strategy_worker_leg_plan": to_jsonable(plan),
        "side_effects": {
            "orders_placed": False,
            "orders_cancelled": False,
            "orders_replaced": False,
...
def cancel_manual_order(
    ...
    current_status = str(order_row.get("status") or "").strip() or "unknown"
    ...
    if payload.dry_run:
        return ManualOrderResponse(
            ...
            summary={
                ...
                "would_cancel": bool(external_order_id),
                "side_effects": {
                    "orders_cancelled": False,
                    "orders_updated": False,
                    "order_events_inserted": False,
