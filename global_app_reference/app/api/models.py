...
class PortfolioDirectTradeBackfillRequest(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
class PortfolioManagerGridEligibilityReviewRequest(BaseModel):
    ...
    recommended_worker_duration_hours: int | None = Field(default=None, ge=1)
...
class PortfolioManagerPriceMemoryRequest(BaseModel):
    model_config = ConfigDict(extra="forbid")

    account_id: UUID | None = None
    market_slug: str | None = None
    token_id: str | None = None
    price_history: dict[str, Any] | list[Any] = Field(default_factory=dict)
    grid_step_price: float | str = "0.01"
    leg_size: float | str = "5"
    starting_inventory: float | str | None = None
    slippage_cents: float | str = "0"
    premise_state: str = "unreviewed"
    spread_cents: float | str | None = None
    depth_usd: float | str | None = None
    days_to_resolution: int | None = Field(default=None, ge=0)
    dry_run: bool = True


class PortfolioManagerPriceMemoryCollectRequest(BaseModel):
    model_config = ConfigDict(extra="forbid")

    account_id: UUID | None = None
    market_rows: list[dict[str, Any]] = Field(default_factory=list)
    price_history_by_token: dict[str, Any] = Field(default_factory=dict)
    fetch_missing: bool = False
    lookback_days: int = Field(default=30, ge=1, le=30)
    interval: str | None = None
    fidelity: int | None = Field(default=None, ge=1, le=60)
    dry_run: bool = True
    reviewed_by: str | None = None
    reason: str | None = None


class PortfolioManagerStrategyWorkerStatusRequest(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
class PortfolioManagerStrategyWorkerUpsertRequest(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
class PortfolioManagerStrategyWorkerLegPlanRequest(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
class PortfolioManagerStrategyWorkerTickRequest(BaseModel):
    model_config = ConfigDict(extra="forbid")
