...
24. `app/docs/planning/current/final_system/architecture/nba_wnba_parallel_runtime_plan_2026-05-30.md`
25. `app/docs/planning/current/final_system/backlog/premise_to_backlog_map_2026-05-18.md`
26. `app/docs/planning/current/final_system/backlog/live_runtime_scope_map_2026-05-24.md`
27. `app/docs/planning/current/final_system/backlog/live_postgame_learning_backlog.md`
28. `app/docs/planning/current/final_system/obsidian/bootstrap_map.md`
29. `app/docs/planning/current/final_system/obsidian/modular_curation_policy.md`
...
| `janus-portfolio-manager` | 1 hour enforcement loop; daily report; 6 hour deep research cycle | Active Codex global portfolio manager for existing-position actions, new micro-position scouting, Polymarket frontend/profile discovery, one-shot order routing, `$50 +/- realized global PnL` budget enforcement, high-volume/low-price candidate scoring, price-memory collection, and dry-run grid/scalping worker lifecycle. | May trade only through approved portfolio-manager order paths and live gates; no Janus NBA/WNBA or CRYPTO up/down/options authority; strategy-worker surfaces are durable dry-run planning/handoff only until a leg routes through the approved order path. |
...
| Reduce/stop lifecycle enforcement | [#82](https://github.com/LucaCGN/janus_cortex/issues/82) | Turn StrategyPlan stop metadata, Q4/endgame loss state, adverse thesis failure, and final cleanup into deterministic lifecycle evidence and Janus-gated reduce candidates. | May affect live reduce/sell candidates only through Janus gates; no raw exchange bypass. |
| Postgame evidence/LLM/blocker recovery | closed [#83](https://github.com/LucaCGN/janus_cortex/issues/83) | Account-return correction implemented: postgame now separates gross turnover from actual PnL and return-on-peak-risk, while preserving model/blocker evidence routing. Future replay/model gaps route to #80/#81/#44/#63. | Read-only evaluation and issue/docs/task routing; no trading. |
...
| [#44](https://github.com/LucaCGN/janus_cortex/issues/44) | P1 | open | Profit-ratcheted event-budget code path implemented; May 30 account-confirmed positive results are calibration input only until lifecycle quality is clean enough for risk promotion. Live risk readback now requires account/db-confirmed positive closed PnL before any profit-ratcheted addon can increase event caps. |
...
| [#63](https://github.com/LucaCGN/janus_cortex/issues/63) | P0 | open | Parent architecture and implementation route for Janus covered-market runtime. Current May 30 follow-ups are rebound-snipe sleeves, manual-interference rebase, duplicate signal pressure, and direct-evidence timeout hardening. |
| [#82](https://github.com/LucaCGN/janus_cortex/issues/82) | P0 | open | Reduce/stop lifecycle evidence implemented and P0 safety patch applied in `33cbc73`; May 30 adds final cleanup/residual lifecycle hardening after IND/POR left postgame residual evidence despite account-positive PnL. |
| [#83](https://github.com/LucaCGN/janus_cortex/issues/83) | P0 | closed | Postgame account-return correction completed: `actual_pnl_usd`, `peak_cash_at_risk_usd`, turnover, sell/redeem proceeds, and closed-position overrides prevent gross buy turnover from being reported as realized loss. |
...
| [#72](https://github.com/LucaCGN/janus_cortex/issues/72) | P1 | closed | NBA/WNBA pregame research agents formalized as optional priors with expiry and no liveness dependency. |
| [#73](https://github.com/LucaCGN/janus_cortex/issues/73) | P1 | closed | Issue lifecycle anti-stagnation and closure governance completed; future drift should route to focused follow-up tasks. |
...
| [#80](https://github.com/LucaCGN/janus_cortex/issues/80) | P1 | open | WNBA price-history-backed panel/replay parity and probe artifact persistence implemented; issue remains the durable route for expanding persisted sample coverage beyond the current two-target proof. |
| [#81](https://github.com/LucaCGN/janus_cortex/issues/81) | P1 | open | Cheap nano PBP dispatcher path is implemented as non-executable sleeve context. May 30 proved nano transport on SEA/TOR, but SAS/OKC exposed a player-catalyst gap: Wemby foul trouble/rest was not captured as star-status shock evidence. |
