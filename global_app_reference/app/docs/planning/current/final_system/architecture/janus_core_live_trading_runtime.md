...
## 2026-05-30 Parallel Runtime Addendum

The current performance refactor source-truth artifact is:

`app/docs/planning/current/final_system/architecture/nba_wnba_parallel_runtime_plan_2026-05-30.md`

That addendum updates the diagrams and operating model after the May 27-29 live-money windows. Its binding corrections are that duplicate same-sleeve/same-band signals must not consume budget twice, order sizing must preserve room for parallel sleeves, account-return reports must use actual PnL and peak cash at risk rather than gross churn, underwater lots can coexist with distinct lower-band cycles when gates allow, and the `gpt-5.4-nano` PBP path must be visible as evidence-only with deterministic fallback when dispatch is unavailable.
...
As of the 2026-05-30 SEA/TOR postgame hotfix, live-worker sizing separates per-order and per-game risk. For NBA/WNBA live validation the preferred operator mode is `order_sizing_mode=fixed_min_shares`: every buy intent is exactly `min_size` shares, currently the 5-share exchange minimum. `event_cap_usd` is the full game cap used by the event budget snapshot, paired microcycle budget, live signal aggregation, and live-game context. This allows multiple minimum-share orders, distinct grid bands, and balanced both-side participation through `side_budget_mode=balanced_50_50`, without reopening the duplicate same-price buy failure mode. `max_buy_notional_usd` / `max_order_buy_notional_usd` remains available only for legacy notional-capped experiments and should not be used to express NBA/WNBA order size.
...
| [#83](https://github.com/LucaCGN/janus_cortex/issues/83) | Closed follow-up: postgame account-return correction, model-usage readback, and blocker efficacy routing. Future deeper replay/model/risk gaps route to #80/#81/#44/#63. |
