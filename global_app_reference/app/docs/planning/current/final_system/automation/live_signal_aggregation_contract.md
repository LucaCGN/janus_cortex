...
2. Deduplicate signals by source/event/side/sleeve/price band/cycle/cooldown.
...
- `app/modules/agentic/signal_aggregation.py` blocks duplicate same-source/same-side/same-sleeve/same-band/same-cycle budget claims with explicit blocker evidence, while leaving distinct cycles and reviewed same-band multi-lot ladders eligible for normal Janus-gated candidate selection.
- `app/modules/agentic/signal_aggregation.py` emits `live_signal_blocker_scope_v1` detail on blockers, including local/global scope, candidate-blocking status, affected/candidate/unaffected sleeve ids, trigger metadata, affected signal types, and expected next action. Local signal suppressions such as stale, disabled, or low-confidence individual signals do not freeze unrelated fresh candidates; explicit global/event/live-safety blockers remain candidate-blocking.
...
Profit-ratcheted promotion has an additional source-quality gate:

- positive realized PnL can increase event caps only when source confidence is `account_confirmed` or `db_confirmed`;
- open or unrealized PnL never unlocks risk;
- unresolved lifecycle rows, final cleanup reason counts, or residual open-order/position evidence hold cap promotion until readback is clean;
- unresolved losses and adverse exposure still reduce caps through the loss-cut path even when promotion is blocked.

Live tick evidence must expose this decision under `event_risk_budget.risk_promotion_evidence` so postgame review can distinguish profitable calibration input from actual risk-promotion authority.
...
Postgame return accounting must not present gross buy turnover as invested capital. Complete reports must separate `gross_buy_turnover_usd`, `sell_proceeds_usd`, `redeem_proceeds_usd`, `actual_pnl_usd`, `peak_cash_at_risk_usd`, and `return_on_peak_cash_at_risk_pct`. Account-confirmed closed-position PnL overrides partial lifecycle cashflow when settlement/redeem rows are not fully visible in the activity stream. Lifecycle cashflow remains useful for sleeve/order debugging, but it is not the realized return denominator.
