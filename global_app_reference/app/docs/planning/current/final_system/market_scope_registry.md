...
| `crypto` | `up-down-options` | `research / backtest-foundation` | `watch-only` | Issue #47 now has a read-only research module for Polymarket crypto options discovery, data audits, joined panels, and blocked-by-design strategy comparison. Live authority remains absent. |
...
Current #47 research foundation lives in `app/docs/reference/crypto_options_research_module.md`, `app/data/nodes/crypto`, `app/data/nodes/polymarket/crypto`, `app/data/pipelines/crypto/options`, and `codex_tool/run_crypto_options_research.py`. It is artifact-only and keeps `live_trading_authorized=false`.
