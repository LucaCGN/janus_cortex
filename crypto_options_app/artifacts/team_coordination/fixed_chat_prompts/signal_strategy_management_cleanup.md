...
- Latest bounded cleanup artifact, if present:
...
Start from `crypto_options_app/artifacts/reports/signal_strategy_cleanup_batch_latest.md` when present. Claim the open handoff queue item for this artifact, then review the first bounded slice.

The current first slice contains 12 weak `master_hedge_grid_scalping` signal rows marked `NEEDS_VARIANT` and 6 stale shadow-review strategies marked `NEEDS_VARIANT`. For each row: retire it, block it with an explicit reason, or create a V2-V5 only when the variant fixes a concrete blocker or source gap.
