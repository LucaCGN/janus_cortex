...
ApiCreds: Any | None = None
ClobClient: Any | None = None
ClobOrderType: Any | None = None
OrderArgs: Any | None = None
OrderPayload: Any | None = None
POLYGON = DEFAULT_POLYGON_CHAIN_ID


def _load_py_clob_runtime() -> dict[str, Any]:
    global ApiCreds, ClobClient, ClobOrderType, OrderArgs, OrderPayload, POLYGON
    try:
        from py_clob_client_v2.client import ClobClient as ImportedClobClient
        from py_clob_client_v2.clob_types import (
            ApiCreds as ImportedApiCreds,
            OrderArgs as ImportedOrderArgs,
            OrderPayload as ImportedOrderPayload,
            OrderType as ImportedClobOrderType,
        )
        from py_clob_client_v2.constants import POLYGON as ImportedPOLYGON
    except ModuleNotFoundError as exc:
        raise RuntimeError("py_clob_client_v2_missing") from exc
    ApiCreds = ApiCreds or ImportedApiCreds
    ClobClient = ClobClient or ImportedClobClient
    ClobOrderType = ClobOrderType or ImportedClobOrderType
    OrderArgs = OrderArgs or ImportedOrderArgs
    OrderPayload = OrderPayload or ImportedOrderPayload
...
class PolymarketCredentials(BaseModel):
    ...
    def from_env(cls) -> "PolymarketCredentials":
        ...
        chain_id = int(os.getenv("CHAIN_ID", DEFAULT_POLYGON_CHAIN_ID))
...
from crypto_options_app.data_nodes.polymarket_gamma.gamma_client import PolymarketDataClient, OpenPosition, ClosedPosition
