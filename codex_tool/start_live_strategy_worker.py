...
try:
    ...
    def build_live_strategy_worker_start_payload(args):  # type: ignore[no-untyped-def]
        payload = {
    ...
    def main_for_live_strategy_worker_start(description: str) -> None:
        ...
        exit_for_response(
            api_json(
                ...
                "POST",
                LIVE_STRATEGY_WORKER_START_PATH,
                build_live_strategy_worker_start_payload(args),
            )
        )


def main() -> None:
    main_for_live_strategy_worker_start("Start the service-owned Janus live strategy worker.")
