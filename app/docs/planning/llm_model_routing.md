...
| `gpt-5.4-mini` | Routine pregame synthesis, routine StrategyPlanJSON drafting/revision, normal live-monitor reasoning when no unresolved exposure is open, postgame first-pass classification. |
| `gpt-5.5` | Critical reasoning only: final plan review under high uncertainty, open-position stop/hedge decisions, manual-intervention reconciliation, halftime/late-game thesis revision, lane promotion/demotion, architecture or deep development decisions. |
...
- Live System Monitor: `gpt-5.4-nano` for tick summaries, `gpt-5.4-mini` for normal no-position monitoring, `gpt-5.5` for open exposure, manual intervention, missing protection, stale-recovery decisions, or hedge/stop evaluation.
