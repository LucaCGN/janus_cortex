...
- Janus revises the active plan when an order fires, manual intervention is detected, a quarter ends, a price/favorite flip occurs, scoreboard leadership changes, a recent run or garbage-time state is detected, a pregame trigger occurs, a Codex monitor requests review, a player/stat trigger fires, or portfolio truth becomes inconsistent.
