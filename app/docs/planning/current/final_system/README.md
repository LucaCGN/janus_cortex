...
| `backlog/premise_to_backlog_map_2026-05-18.md` | Post-foundation premise-to-issue map for follow-up issues `#30`, `#32`, and `#37-#59`. |
...
- The `janus-portfolio-manager` automation is the Codex global portfolio manager, separate from the master controller and separate from the internal Janus covered-market portfolio/inventory agent for NBA/WNBA. It manages the broader operator/global portfolio, including existing-position target/exit/rebuy maintenance, Polymarket frontend catalog browsing, winning-profile monitoring, proactive trend-following scouting, required per-run action selection, one-shot portfolio order routing, and gated 1c grid/scalping service planning from the closed `#56` foundation. It may trade only through an approved Janus portfolio order-management path or approved independent Polymarket fallback path with direct CLOB/account truth, separate risk budget, minimum-order compliance, ledger/idempotency evidence, reconciliation plan, and kill-switch gates. Base `codex_tools/polymarket` preview/action-planning tooling is implemented, `portfolio-manager-order` can call the approved Janus one-shot order path, and the `#54` Janus portfolio order-management adapter/proof bundle is implemented. Closed `#59` proved the first supervised non-dry-run Janus portfolio-manager call with direct-CLOB reconciliation; every future non-dry-run action still requires same-pass runtime approval, reviewer metadata, fresh direct truth, complete action proof, and reconciliation.
