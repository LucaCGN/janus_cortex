...
- The `janus-portfolio-manager` automation is the Codex global portfolio manager, separate from the master controller and separate from the internal Janus covered-market portfolio/inventory agent for NBA/WNBA. It manages the broader operator/global portfolio, including existing-position target/exit/rebuy maintenance and proactive trend-following scouting in uncovered categories such as geopolitics and economics. It may trade only through an approved Janus portfolio order-management path or approved independent Polymarket fallback path with direct CLOB/account truth, separate risk budget, minimum-order compliance, ledger/idempotency evidence, reconciliation plan, and kill-switch gates. Base `codex_tools/polymarket` preview tooling is implemented, and the `#54` Janus portfolio order-management adapter/proof bundle is implemented. Concrete real-call activation and direct-CLOB reconciliation proof remain blocked on `#59`.
