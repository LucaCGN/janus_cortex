...
| StrategyPlanJSON missing | Data refresh, integrity, planning, issue-backed development, read-only global portfolio. | Live monitor reports strategy-plan gate blockers. | Live strategy execution and live readiness GREEN. |
...
| Global portfolio | Direct/account/API truth available, read-only contract active. | Note missing access as blocker. | Any execution, order preparation, or risk-budget merge with Janus sports testing. |
