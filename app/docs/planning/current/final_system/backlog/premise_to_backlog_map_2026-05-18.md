...
|---|---|---|
| P0 activation blockers | Must be resolved before trusting recurring controller/live-readiness flow or active order-path automations. | `#61` for next NBA min-size live trade routing, `#62` for WNBA min-size live-promotion, and `#59` for global portfolio-manager real-call proof; older portfolio policy/tool split foundations are closed. |
| P1 readiness builders | Needed for NBA/WNBA minimum-size testing, reliable reviews, and portfolio-manager action-readiness. | `#42`, `#44`, `#55`, `#56`; `#50` and `#60` are closed WNBA foundations. |
...
13. Use `#46-#48` for future-domain and profile research without live authority unless promoted by `#54` execution-gate implementation, `#59` activation evidence, and operator-approved gates.
