...
| `codex-global-portfolio-agent` | Global portfolio and uncovered markets | Existing operator/global position target/exit/rebuy management, proactive trend-opportunity scouting, return-receipt tracking, watchlist and lesson capture, gated Janus portfolio order-management or independent Polymarket fallback once the approved execution path exists. | Manage internal Janus NBA/WNBA strategy validation or bypass direct CLOB truth, approved tool paths, separate global-portfolio risk budget, ledger/idempotency, reconciliation, or kill switches. |
