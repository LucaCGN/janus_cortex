...
| JIT-55-03 | #55 | ready | basketball-intelligence / postgame-review | Backfill real price-path and order-lifecycle replay for post-Q1 and post-Q1-stability windows before StrategyPlan template or #69 event-control changes. | JIT-55-02 is deterministic/proxy evidence only for post-Q1 windows: first-live has one Thunder adverse-selection quarantine, pregame resting is treated as expired until order-lifecycle replay proves otherwise, and post-Q1 stability remains blocked by missing stability-window price-path replay. |
