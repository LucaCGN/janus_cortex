...
| JIT-55-05 | #55 | ready | basketball-intelligence / strategy-sleeves | Convert the entry-timing matrix and recommendation pack into StrategyPlan template documentation for when pregame resting is blocked, first-live is allowed for review, and post-Q1 stability is preferred/blocked. | Must remain docs/template guidance only unless explicit Janus/operator gates approve event-control or StrategyPlan default changes. |
