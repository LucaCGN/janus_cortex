...
3. Frontend and profile discovery: navigate the Polymarket frontend catalog and winning-profile pages because the UI exposes live/trending/breaking/new/category context that direct account snapshots do not fully reproduce. Every material run must check saved winning-profile notes and public profile pages for new active positions or recent trades since the last pass, then consider whether any latest trade/position is worth mimicking as a bounded micro-position candidate. Treat UI/profile evidence as discovery only until mapped to direct CLOB/token/orderbook truth.
