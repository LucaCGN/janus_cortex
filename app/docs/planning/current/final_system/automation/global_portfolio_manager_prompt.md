...
4. Grid-service incubation: review all ongoing markets traded in the last month for repeated 3-5% position swings that may support 1c sell/rebuy grids. Use `codex_tools/polymarket preview-grid-service` for candidates and `codex_tools/polymarket plan-grid-service-spawn` for gated service-spawn proof. Do not prepare or submit any order unless the global-portfolio execution gates and per-leg service policy are explicitly satisfied.
