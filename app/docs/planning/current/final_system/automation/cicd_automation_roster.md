...
| `oversight-devloop` | heartbeat | 30m | active | #73, all active P0/P1 | Dev-loop oversight and anti-stagnation. Splits/closes/routes issues, checks dirty worktree and queue locks. No trading or worker starts. |
| `oversight-portfolio` (`janus-loop-oversight-console`) | heartbeat | 1h | active | #56, #59 | Portfolio-manager oversight only. Reviews strategy/action/rationale drift. No orders, redemptions, or services. |
...
3. Enable `janus-master-dev` and `oversight-devloop`.
