...
After [#57](https://github.com/LucaCGN/janus_cortex/issues/57), [#58](https://github.com/LucaCGN/janus_cortex/issues/58), and [#60](https://github.com/LucaCGN/janus_cortex/issues/60) are closed, the controller must not continue treating those issues as the active development route or wait for stale settlement/WNBA evidence before doing useful work. In a no-live/no-pregame window with a clean worktree and clear locks, the expected route is an open sprint issue with a bounded slice: [#59](https://github.com/LucaCGN/janus_cortex/issues/59) for portfolio-manager real-call reconciliation proof, [#55](https://github.com/LucaCGN/janus_cortex/issues/55) for entry-timing/event-start expiry research, or [#56](https://github.com/LucaCGN/janus_cortex/issues/56) for active portfolio-manager action planning, frontend/profile discovery enforcement, one-shot portfolio order routing, cross-league basketball scanning, and gated grid/scalping service spawn work. If it cannot claim or work the selected issue, it must record the concrete blocker once.
