...
- After a covered NBA/WNBA game reaches final or settlement, unresolved event-scoped direct CLOB orders, positions, fills, or valuation mismatches block new live-worker enablement and new live-order tests until reconciled or explicitly classified as a documented residual. For the 2026-05-18 Spurs/Thunder test, route this to GitHub `#57`; keep closed `#50` as WNBA passive/shadow baseline history, use `#60` for remaining WNBA active-window capture/audit work, and keep `#55` focused on entry-timing research.
- `#50` is closed and must not receive new routing except direct regression notes. `#60` is the active WNBA passive/shadow capture issue. It advances only through sustained WNBA active-window passive CLOB capture, WNBA tick/trade audit integration, WNBA price-history/shadow summaries with new evidence, or WNBA handoff/report changes. Do not post NBA live, NBA settlement, resolved-market redeem, or global-portfolio updates under `#50` or `#60`.
...
- Once `#57` and `#58` are closed, do not route additional no-change passes to Spurs/Thunder settlement or redeem scaffolding solely because old evidence is flat. If no active or near-term NBA/WNBA readiness route outranks development, check the open sprint issues and claim the highest useful bounded slice: `#59` for portfolio-manager real-call reconciliation proof, `#60` for WNBA active-window capture/readiness evidence, `#55` for entry-timing and event-start expiry research, or `#56` for active global-portfolio action planning, frontend/profile discovery, grid/cross-league scanner work, and gated grid/scalping service spawn planning. Do not prepare, submit, redeem, sign, broadcast, or start per-leg service orders from those routes unless the issue-specific gates explicitly allow it.
