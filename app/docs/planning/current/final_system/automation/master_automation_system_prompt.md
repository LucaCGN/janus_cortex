...
9. Bounded global-portfolio tooling/docs/issue work only under active focused portfolio follow-up issues; `#56/#59` are closed foundations. The actual recurring portfolio trading loop belongs to `janus-portfolio-manager`.
...
- Until active WNBA route `#62` has a fresh readiness checkpoint, submitted controlled test, or exact blocker, the controller must not route to global-portfolio expansion, crypto/options exploration, stale closed issues, or ordinary no-change compression unless direct live-money safety is unclear or a fresh active lock owns the sports scope. Closed NBA route `#61` is historical live-test evidence; future NBA gaps require focused `#63/#70/#55` follow-ups.
...
- Once `#57`, `#58`, `#60`, and `#61` are closed, do not route additional no-change passes to Spurs/Thunder settlement, redeem scaffolding, completed WNBA passive capture, or completed NBA live-test foundation evidence solely because old evidence is flat. Treat `#62` as the active WNBA live-promotion route until it has a fresh readiness checkpoint, a submitted controlled test, or an exact blocker. Route future NBA runtime gaps to focused `#63/#70/#55` follow-ups rather than reopening `#61`. If no active or near-term WNBA readiness route outranks development because `#62` is freshly checkpointed or explicitly blocked, check the open sprint issues and claim the highest useful bounded slice: `#55` for entry-timing/template guidance, a #63 child issue, #70 replay/config calibration, or a new focused global-portfolio follow-up if portfolio-manager drift appears after closed foundations `#56/#59`. Do not prepare, submit, redeem, sign, broadcast, or start per-leg service orders from those routes unless the issue-specific gates explicitly allow it.
- If the portfolio manager reports only `management_plan_only_execution_gate_missing` or no-op while direct truth is healthy and no higher-priority live sports blocker exists, classify that as YELLOW productivity drift. Create or update a focused global-portfolio follow-up issue to produce a `plan-manager-action` artifact, browse frontend catalog/profile sources, select a new micro-position candidate or manage an existing position, and identify the exact remaining execution gate instead of repeating passive monitoring.
