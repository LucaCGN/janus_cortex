...
- WNBA shadow target `#60` is closed as completed after the Toronto at Phoenix passive capture/audit integration. Treat it as regression-check history only; future WNBA live-promotion work needs a new issue or explicit follow-up route.
...
- Do not keep routing active passes to `#60` now that its passive capture/audit acceptance criteria are complete. WNBA remains non-live unless a separate reviewed WNBA StrategyPlan/integrity/fillability/operator gate exists.
