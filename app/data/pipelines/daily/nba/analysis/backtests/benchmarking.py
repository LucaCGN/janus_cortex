...
def build_benchmark_run_result(state_df: pd.DataFrame, request: BacktestRunRequest) -> BenchmarkRunResult:
    ...
    payload["benchmark"] = {
        ...
        "portfolio_config": {
            ...
            "sizing_mode": normalize_portfolio_sizing_mode(request.portfolio_sizing_mode),
            "target_exposure_fraction": normalize_portfolio_target_exposure_fraction(request.portfolio_target_exposure_fraction),
...
def _render_benchmark_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- Portfolio sizing mode: `{(benchmark.get('portfolio_config') or {}).get('sizing_mode')}`",
        f"- Portfolio target exposure fraction: `{_format_num((benchmark.get('portfolio_config') or {}).get('target_exposure_fraction'))}`",
